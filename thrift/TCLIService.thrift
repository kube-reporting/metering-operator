...
enum TProtocolVersion {
  ...
  // V3 add varchar type, primitive type quali***REMOVED***ers
...
// Thrift does not support recursively de***REMOVED***ned types or forward declarations,
// which makes it dif***REMOVED***cult to represent Hive's nested types.
...
// Valid TTypeQuali***REMOVED***ers key names
const string CHARACTER_MAXIMUM_LENGTH = "characterMaximumLength"

// Type quali***REMOVED***er key name for decimal
const string PRECISION = "precision"
const string SCALE = "scale"

union TTypeQuali***REMOVED***erValue {
  1: optional i32 i32Value
  2: optional string stringValue
}

// Type quali***REMOVED***ers for primitive type.
struct TTypeQuali***REMOVED***ers {
  1: required map <string, TTypeQuali***REMOVED***erValue> quali***REMOVED***ers
}
...
struct TPrimitiveTypeEntry {
  ...
  2: optional TTypeQuali***REMOVED***ers typeQuali***REMOVED***ers
}
...
struct TUserDe***REMOVED***nedTypeEntry {
  // The fully quali***REMOVED***ed name of the class implementing this type.
...
union TTypeEntry {
  ...
  6: TUserDe***REMOVED***nedTypeEntry userDe***REMOVED***nedTypeEntry
}

// Type descriptor for columns.
struct TTypeDesc {
  // The "top" type is always the ***REMOVED***rst element of the list.
...
struct TStatus {
  ...
  // If status is ERROR, then the following ***REMOVED***elds may be set
  3: optional string sqlState  // as de***REMOVED***ned in the ISO/IEF CLI speci***REMOVED***cation
...
// A string identi***REMOVED***er. This is interpreted literally.
typedef string TIdenti***REMOVED***er
...
// A search pattern or identi***REMOVED***er. Used as input
// parameter for many of the catalog functions.
typedef string TPatternOrIdenti***REMOVED***er

struct THandleIdenti***REMOVED***er {
  // 16 byte globally unique identi***REMOVED***er
...
struct TSessionHandle {
  1: required THandleIdenti***REMOVED***er sessionId
}
...
struct TOperationHandle {
  1: required THandleIdenti***REMOVED***er operationId
  ...
  // modi***REMOVED***edRowCount is either:
  //
  // 1) The number of rows that were modi***REMOVED***ed by
  //    the DML operation (e.g. number of rows inserted,
  //    number of rows deleted, etc).
  //
  // 2) 0 for operations that don't modify or add rows.
  //
  // 3) < 0 if the operation is capable of modi***REMOVED***ying rows,
  //    but Hive is unable to determine how many rows were
  //    modi***REMOVED***ed. For example, Hive's LOAD DATA command
  //    doesn't generate row count information because
  //    Hive doesn't inspect the data as it is loaded.
  //
  // modi***REMOVED***edRowCount is unset if the operation generates
  // a result set.
  4: optional double modi***REMOVED***edRowCount
}
...
struct TOpenSessionReq {
  ...
  // protocol layer, in which case these ***REMOVED***elds may be
  // left unset.
  2: optional string username
  3: optional string password

  // Con***REMOVED***guration overlay which is applied when the session is
  // ***REMOVED***rst created.
  4: optional map<string, string> con***REMOVED***guration
}
...
struct TOpenSessionResp {
  ...
  // The con***REMOVED***guration settings for this session.
  4: optional map<string, string> con***REMOVED***guration
}
...
// CloseSession()
//
// Closes the speci***REMOVED***ed session and frees any resources
...
// statement has ***REMOVED***nished executing.
struct TExecuteStatementReq {
  ...
  // Con***REMOVED***guration properties that are overlayed on top of the
  // the existing session con***REMOVED***guration before this statement
...
struct TGetSchemasReq {
  ...
  2: optional TIdenti***REMOVED***er catalogName

  // schema name or pattern
  3: optional TPatternOrIdenti***REMOVED***er schemaName
}
...
struct TGetTablesReq {
  ...
  2: optional TPatternOrIdenti***REMOVED***er catalogName

  // Name of the schema or a search pattern.
  3: optional TPatternOrIdenti***REMOVED***er schemaName

  // Name of the table or a search pattern.
  4: optional TPatternOrIdenti***REMOVED***er tableName
...
// GetColumns()
//
// Returns a list of columns in the speci***REMOVED***ed tables.
// The information is returned as a result set which can be fetched
// using the OperationHandle provided in the response.
...
struct TGetColumnsReq {
  ...
  5: optional TPatternOrIdenti***REMOVED***er columnName
}
...
// name: SPECIFIC_NAME
// type: STRING
// desc: The name which uniquely identi***REMOVED***es this function within its schema.
//       In this case this is the fully quali***REMOVED***ed class name of the class
...
struct TGetFunctionsReq {
  ...
  // in the database.
  4: required TPatternOrIdenti***REMOVED***er functionName
}
...
struct TGetPrimaryKeysReq {
  ...
  // Name of the schema.
  3: optional TIdenti***REMOVED***er schemaName

  // Name of the table.
  4: optional TIdenti***REMOVED***er tableName
}
...
struct TGetCrossReferenceReq {
  ...
  2: optional TIdenti***REMOVED***er parentCatalogName

  // Name of the parent schema.
  3: optional TIdenti***REMOVED***er parentSchemaName

  // Name of the parent table.
  4: optional TIdenti***REMOVED***er parentTableName

  // Name of the foreign catalog.
  5: optional TIdenti***REMOVED***er foreignCatalogName

  // Name of the foreign schema.
  6: optional TIdenti***REMOVED***er foreignSchemaName

  // Name of the foreign table.
  7: optional TIdenti***REMOVED***er foreignTableName
}
...
struct TGetOperationStatusResp {
  ...
  // If operationState is ERROR_STATE, then the following ***REMOVED***elds may be set
  // sqlState as de***REMOVED***ned in the ISO/IEF CLI speci***REMOVED***cation
...
// CancelOperation()
//
// Cancels processing on the speci***REMOVED***ed operation handle and
...
//
// Retrieves schema information for the speci***REMOVED***ed operation
...
enum TFetchOrientation {
  ...
  // Return the rowset at the speci***REMOVED***ed fetch offset.
  // NOT SUPPORTED
  FETCH_ABSOLUTE,

  // Get the ***REMOVED***rst rowset in the result set.
  FETCH_FIRST,
