...
type Lexer struct {
	...
	***REMOVED***rstElement bool // Whether current element is the ***REMOVED***rst in array or an object.
	wantSep      byte // A comma or a colon character, which need to occur before a token.

	UseMultipleErrors bool          // If we want to use multiple errors.
	fatalError        error         // Fatal error occurred during lexing. It is usually a syntax error.
	multipleErrors    []*LexerError // Semantic errors occurred during lexing. Marshalling will be continued after ***REMOVED***nding this errors.
}
...
func (r *Lexer) FetchToken() {
	...
	for _, c := range r.Data[r.pos:] {
		...
		case '}', ']':
			if !r.***REMOVED***rstElement && (r.wantSep != ',') {
				r.errSyntax()
			}
			r.wantSep = 0
			r.token.kind = tokenDelim
...
// ***REMOVED***ndStringLen tries to scan into the string literal for ending quote char to determine required size.
// The size will be exact if no escapes are present and may be inexact if there are escaped chars.
func ***REMOVED***ndStringLen(data []byte) (hasEscapes bool, length int) {
	delta := 0
...
func (r *Lexer) fetchString() {
	...
	hasEscapes, length := ***REMOVED***ndStringLen(data)
	if !hasEscapes {
...
// Delim consumes a token and veri***REMOVED***es that it is the given delimiter.
...
// Null veri***REMOVED***es that the next token is null and consumes it.
func (r *Lexer) Null() {
	if r.token.kind == tokenUndef && r.Ok() {
		r.FetchToken()
	}
