...
// EngineOptions contains con***REMOVED***guration parameters for an Engine.
...
func (ng *Engine) exec(ctx context.Context, q *query) (Value, error) {
	...
	// The base context might already be canceled on the ***REMOVED***rst iteration (e.g. during shutdown).
...
func (ng *Engine) execEvalStmt(ctx context.Context, query *query, s *EvalStmt) (Value, error) {
	...
	for ts := s.Start; !ts.After(s.End); ts = ts.Add(s.Interval) {
		...
		case Scalar:
			...
			// as the ***REMOVED***ngerprint for Scalar expressions.
			ss, ok := Seriess[0]
			if !ok {
...
// An evaluator evaluates given expressions at a ***REMOVED***xed timestamp. It is attached to an
...
type evaluator struct {
	...
	***REMOVED***nalizers []func()

	logger log.Logger
}

func (ev *evaluator) close() {
	for _, f := range ev.***REMOVED***nalizers {
		f()
	}
}
...
func (ev *evaluator) matrixSelector(node *MatrixSelector) Matrix {
	...
	ev.***REMOVED***nalizers = append(ev.***REMOVED***nalizers,
...
func (ev *evaluator) VectorAnd(lhs, rhs Vector, matching *VectorMatching) Vector {
	...
	// Add all rhs samples to a map so we can easily ***REMOVED***nd matches later.
	for _, rs := range rhs {
...
func (ev *evaluator) VectorBinop(op itemType, lhs, rhs Vector, matching *VectorMatching, returnBool bool) Vector {
	...
	// For all lhs samples ***REMOVED***nd a respective rhs sample and perform
	...
	for _, ls := range lhs {
		...
		if matching.Card == CardOneToOne {
			...
			} ***REMOVED*** if _, duplicate := insertedSigs[insertSig]; duplicate {
...
func resultMetric(lhs, rhs labels.Labels, op itemType, matching *VectorMatching) labels.Labels {
	...
	for _, ln := range matching.Include {
		// Included labels from the `group_x` modi***REMOVED***er are taken from the "one"-side.
		if v := rhs.Get(ln); v != "" {
			lb.Set(ln, v)
		} ***REMOVED*** {
			lb.Del(ln)
...
// user facing terminology as de***REMOVED***ned in the documentation.
