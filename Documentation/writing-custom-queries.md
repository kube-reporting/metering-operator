...
One thing to note is we replaced `FROM datasource_unready_deployment_replicas` with `{| dataSourceTableName .Report.Inputs.UnreadyDeploymentReplicasDataSourceName |}` and added an `inputs` con***REMOVED***guration to avoid hard coding the table name.
