...
func Default() *GeneratorArgs {
	return &GeneratorArgs{
		...
		GoHeaderFilePath:           ***REMOVED***lepath.Join(DefaultSourceTree(), "k8s.io/gengo/boilerplate/boilerplate.go.txt"),
...
type GeneratorArgs struct {
	...
	// below the bloilerplate, of the format de***REMOVED***ned by this string.
...
func (g *GeneratorArgs) AddFlags(fs *pflag.FlagSet) {
	...
	fs.StringVarP(&g.OutputFileBaseName, "output-***REMOVED***le-base", "O", g.OutputFileBaseName, "Base name (without .go suf***REMOVED***x) for output ***REMOVED***les.")
	fs.StringVarP(&g.GoHeaderFilePath, "go-header-***REMOVED***le", "h", g.GoHeaderFilePath, "File containing boilerplate header text. The string YEAR will be replaced with the current 4-digit year.")
	fs.BoolVar(&g.VerifyOnly, "verify-only", g.VerifyOnly, "If true, only verify existing output, do not write anything.")
	fs.StringVar(&g.GeneratedBuildTag, "build-tag", g.GeneratedBuildTag, "A Go build tag to use to identify ***REMOVED***les generated by this command. Should be unique.")
}

// LoadGoBoilerplate loads the boilerplate ***REMOVED***le passed to --go-header-***REMOVED***le.
...
func (g *GeneratorArgs) NewBuilder() (*parser.Builder, error) {
	b := parser.New()
	// Ignore all auto-generated ***REMOVED***les.
	b.AddBuildTags(g.GeneratedBuildTag)

	for _, d := range g.InputDirs {
		var err error
		if strings.HasSuf***REMOVED***x(d, "/...") {
			err = b.AddDirRecursive(strings.TrimSuf***REMOVED***x(d, "/..."))
		} ***REMOVED*** {
			err = b.AddDir(d)
		}
		if err != nil {
...
func (g *GeneratorArgs) InputIncludes(p *types.Package) bool {
	for _, dir := range g.InputDirs {
		d := dir
		if strings.HasSuf***REMOVED***x(d, "...") {
			d = strings.TrimSuf***REMOVED***x(d, "...")
		}
		if strings.HasPre***REMOVED***x(p.Path, d) {
			return true
		}
	}
	return false
...
// DefaultSourceTree returns the /src directory of the ***REMOVED***rst entry in $GOPATH.
// If $GOPATH is empty, it returns "./". Useful as a default output location.
func DefaultSourceTree() string {
	paths := strings.Split(os.Getenv("GOPATH"), string(***REMOVED***lepath.ListSeparator))
	if len(paths) > 0 && len(paths[0]) > 0 {
		return ***REMOVED***lepath.Join(paths[0], "src")
	}
	return "./"
}
