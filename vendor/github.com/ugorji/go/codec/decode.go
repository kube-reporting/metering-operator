...
type decDriver interface {
	...
	// the decoder, so that is done later, by setting the decNaked.valueType ***REMOVED***eld.
	//
	...
	//
	// extensions should also use readx to decode them, for ef***REMOVED***ciency.
...
type DecodeOptions struct {
	// MapType speci***REMOVED***es type to use during schema-less decoding of a map in the stream.
	// If nil, we use map[interface{}]interface{}
	MapType reflect.Type

	// SliceType speci***REMOVED***es type to use during schema-less decoding of an array in the stream.
	// If nil, we use []interface{}
	SliceType reflect.Type

	// MaxInitLen de***REMOVED***nes the initial length that we "make" a collection (slice, chan or map) with.
	...
	// Instead, we provision up to MaxInitLen, ***REMOVED***ll that up, and start appending after that.
	MaxInitLen int

	// If ErrorIfNoField, return an error when decoding a map
	// from a codec stream into a struct, and no matching struct ***REMOVED***eld is found.
	ErrorIfNoField bool
	...
	//
	// By default, when we see a ***REMOVED***eld that is an interface{...},
	...
	// If the set of keys are ***REMOVED***nite, it may help reduce allocation to
...
type ioDecReader struct {
	...
	// temp byte array re-used internally for ef***REMOVED***ciency during read.
...
func (f *decFnInfo) kErr(rv reflect.Value) {
	f.d.errorf("no decoding function de***REMOVED***ned for kind %v", rv.Kind())
}
...
func (f *decFnInfo) kInterfaceNaked() (rvn reflect.Value) {
	...
	case valueTypeMap:
		...
		// } ***REMOVED*** if d.h.MapType == mapStrIntfTyp { // for json performance
		// }
		if d.mtid == 0 || d.mtid == mapIntfIntfTypId {
			...
			n.ms = n.ms[:l]
		} ***REMOVED*** if d.mtid == mapStrIntfTypId { // for json performance
			l := len(n.ns)
...
func (f *decFnInfo) kInterface(rv reflect.Value) {
	...
	} ***REMOVED*** if f.d.h.InterfaceReset {
		rvn = f.kInterfaceNaked()
		if rvn.IsValid() {
			rv.Set(rvn)
		} ***REMOVED*** {
...
func (f *decFnInfo) kStruct(rv reflect.Value) {
	...
	if ctyp == valueTypeMap {
			...
			for j := 0; j < containerLen; j++ {
				...
				if k := fti.indexForEncName(rvkencname); k > -1 {
					...
					if dd.TryDecodeAsNil() {
						...
						d.decodeValue(si.***REMOVED***eld(rv, true), nil)
					}
				} ***REMOVED*** {
	...
	} ***REMOVED*** if ctyp == valueTypeArray {
		...
		hasLen := containerLen >= 0
		for j, si := range fti.s***REMOVED***p {
			if hasLen {
				if j == containerLen {
					break
				}
			} ***REMOVED*** if dd.CheckBreak() {
				break
			}
			if cr != nil {
		...
		if containerLen > len(fti.s***REMOVED***p) {
			// read remaining values and throw away
			for j := len(fti.s***REMOVED***p); j < containerLen; j++ {
				if cr != nil {
					cr.sendContainerState(containerArrayElem)
				}
	...
	if containerLenS > 0 { // hasLen
		if f.seq == seqTypeChan {
			...
			if containerLenS > rvcap {
				...
				numToRead = rvlen
			} ***REMOVED*** if containerLenS != rvlen {
				if f.seq == seqTypeSlice {
			...
			} ***REMOVED*** if truncated { // slice was truncated, as chan NOT in this block
				for ; j < containerLenS; j++ {
		...
		for ; !dd.CheckBreak(); j++ {
			if f.seq == seqTypeChan {
				...
				// if inde***REMOVED***nite, etc, then expand the slice if necessary
				var decodeIntoBlank bool
				if j >= rvlen {
					if f.seq == seqTypeArray {
						d.arrayCannotExpand(rvlen, j+1)
						decodeIntoBlank = true
					} ***REMOVED*** { // if f.seq == seqTypeSlice
...
func (f *decFnInfo) kMap(rv reflect.Value) {
	...
	if !f.d.h.MapValueReset {
		...
		// if interface, mapGet = true if !DecodeNakedAlways (***REMOVED*** false)
		// if builtin, mapGet = false
		// ***REMOVED*** mapGet = true
		if vtypeKind == reflect.Ptr {
			mapGet = true
		} ***REMOVED*** if vtypeKind == reflect.Interface {
			if !f.d.h.InterfaceReset {
				mapGet = true
			}
		} ***REMOVED*** if !isImmutableKind(vtypeKind) {
			mapGet = true
		}
	}
...
type Decoder struct {
	...
	// Try to put things that go together to ***REMOVED***t within a cache line (8 words).
...
//
// For ef***REMOVED***ciency, Users are encouraged to pass in a memory buffered reader
// (eg bu***REMOVED***o.Reader, bytes.Buffer).
...
// NewDecoderBytes returns a Decoder which ef***REMOVED***ciently decodes directly
...
// Con***REMOVED***gurations exist on the Handle to override defaults
...
//     by updating the ***REMOVED***rst n elements, where n is length of the stream.
//   - A slice can be decoded from a stream map, by decoding as if
//     it contains a sequence of key-value pairs.
//   - A struct can be decoded from a stream map, by updating matching ***REMOVED***elds.
//   - A struct can be decoded from a stream array,
//     by updating ***REMOVED***elds as they occur in the struct (by index).
//
...
func (d *Decoder) getDecFn(rt reflect.Type, checkFastpath, checkCodecSelfer bool) (fn *decFn) {
	...
	***REMOVED***.ti = ti
	...
	// (e.g. type BitSet int64, type MyStruct { / * unexported ***REMOVED***elds * / }, type X []int, etc.
	//
	// We can't check if it's an extension byte here ***REMOVED***rst, because the user may have
	// registered a pointer or non-pointer type, meaning we may have to recurse ***REMOVED***rst
	...
	} ***REMOVED*** if rtid == rawExtTypId {
		fn.f = (*decFnInfo).rawExt
	} ***REMOVED*** if rtid == rawTypId {
		fn.f = (*decFnInfo).raw
	} ***REMOVED*** if d.d.IsBuiltinType(rtid) {
		fn.f = (*decFnInfo).builtin
	} ***REMOVED*** if xfFn := d.h.getExt(rtid); xfFn != nil {
		***REMOVED***.xfTag, ***REMOVED***.xfFn = xfFn.tag, xfFn.ext
		fn.f = (*decFnInfo).ext
	} ***REMOVED*** if supportMarshalInterfaces && d.be && ti.bunm {
		fn.f = (*decFnInfo).binaryUnmarshal
	} ***REMOVED*** if supportMarshalInterfaces && !d.be && d.js && ti.junm {
		//If JSON, we should check JSONUnmarshal before textUnmarshal
		fn.f = (*decFnInfo).jsonUnmarshal
	} ***REMOVED*** if supportMarshalInterfaces && !d.be && ti.tunm {
		...
		if fn.f == nil {
			...
			case reflect.Chan:
				***REMOVED***.seq = seqTypeChan
				fn.f = (*decFnInfo).kSlice
			case reflect.Slice:
				***REMOVED***.seq = seqTypeSlice
				fn.f = (*decFnInfo).kSlice
			case reflect.Array:
				***REMOVED***.seq = seqTypeArray
...
func (d *Decoder) structFieldNotFound(index int, rvkencname string) {
	...
	if d.h.ErrorIfNoField {
		if index >= 0 {
			d.errorf("no matching struct ***REMOVED***eld found when decoding stream array at index %v", index)
			return
		} ***REMOVED*** if rvkencname != "" {
			d.errorf("no matching struct ***REMOVED***eld found when decoding stream map with key " + rvkencname)
			return
		}
	}
	d.swallow()
...
func decInferLen(clen, maxlen, unit int) (rvlen int, truncated bool) {
	...
	if maxlen <= 0 {
		// no maxlen de***REMOVED***ned. Use maximum of 256K memory, with a floor of 4K items.
	...
	// if clen <= 0 {
	// 	rvlen = 0
	// } ***REMOVED*** if maxlen > 0 && clen > maxlen {
	// 	rvlen = maxlen
	// 	truncated = true
	// } ***REMOVED*** {
	// 	rvlen = clen
...
// // Using decReaderT (de***REMOVED***ned just above) caused performance degradation
