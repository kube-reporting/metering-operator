...
The `Report` and `ScheduledReport` custom resources are used to manage the execution and status of reports. Chargeback produces reports derived from usage data sources which can be used in further analysis and ***REMOVED***ltering.
...
A single `Report` resource represents a report which runs the provided query for the speci***REMOVED***ed time range. Once the object is created, Chargeback starts analyzing the data required to perform the report. A report cannot be updated after its creation and must run to completion.
