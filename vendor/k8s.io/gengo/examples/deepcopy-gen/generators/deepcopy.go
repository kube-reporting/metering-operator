...
// de***REMOVED***ned for the given type.  This allows more ef***REMOVED***cient deep copy
...
func (g *genDeepCopy) doMap(t *types.Type, sw *generator.SnippetWriter) {
	...
	if t.Key.IsAssignable() {
		...
		default:
			...
			} ***REMOVED*** if t.Elem.Kind == types.Slice && t.Elem.Elem.Kind == types.Builtin {
				sw.Do("if val==nil { (*out)[key]=nil } ***REMOVED*** {\n", nil)
				sw.Do("(*out)[key] = make($.|raw$, len(val))\n", t.Elem)
				sw.Do("copy((*out)[key], val)\n", nil)
				sw.Do("}\n", nil)
			} ***REMOVED*** if t.Elem.Kind == types.Alias && t.Elem.Underlying.Kind == types.Slice && t.Elem.Underlying.Elem.Kind == types.Builtin {
				sw.Do("(*out)[key] = make($.|raw$, len(val))\n", t.Elem)
				sw.Do("copy((*out)[key], val)\n", nil)
			} ***REMOVED*** if t.Elem.Kind == types.Pointer {
				sw.Do("if val==nil { (*out)[key]=nil } ***REMOVED*** {\n", nil)
...
func (g *genDeepCopy) doSlice(t *types.Type, sw *generator.SnippetWriter) {
	...
	if hasDeepCopyMethod(t.Elem) {
		...
		sw.Do("}\n", nil)
	} ***REMOVED*** if t.Elem.Kind == types.Builtin || t.Elem.IsAssignable() {
		sw.Do("copy(*out, *in)\n", nil)
	} ***REMOVED*** {
		sw.Do("for i := range *in {\n", nil)
		if t.Elem.Kind == types.Slice {
			...
			sw.Do("}\n", nil)
		} ***REMOVED*** if hasDeepCopyMethod(t.Elem) {
			sw.Do("(*out)[i] = (*in)[i].DeepCopy()\n", nil)
			// REVISIT(sttts): the following is removed in master
			//} ***REMOVED*** if t.Elem.IsAssignable() {
			//	sw.Do("(*out)[i] = (*in)[i]\n", nil)
		} ***REMOVED*** if t.Elem.Kind == types.Interface {
			...
			sw.Do("}\n", nil)
		} ***REMOVED*** if t.Elem.Kind == types.Struct {
			sw.Do("(*in)[i].DeepCopyInto(&(*out)[i])\n", nil)
		} ***REMOVED*** {
			sw.Do("(*out)[i] = (*in)[i].DeepCopy()\n", nil)
		}
