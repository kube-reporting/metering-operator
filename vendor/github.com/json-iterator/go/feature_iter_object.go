...
func (iter *Iterator) ReadObjectCB(callback func(*Iterator, string) bool) bool {
	...
	if c == '{' {
		...
		if c == '"' {
			iter.unreadByte()
			***REMOVED***eld := iter.readObjectFieldAsBytes()
			if !callback(iter, *(*string)(unsafe.Pointer(&***REMOVED***eld))) {
				return false
			}
			c = iter.nextToken()
			for c == ',' {
	...
	if c == '{' {
		...
		if c == '"' {
			...
			***REMOVED***eld := iter.ReadString()
			if iter.nextToken() != ':' {
				iter.ReportError("ReadMapCB", "expect : after object ***REMOVED***eld")
				return false
			}
...
func (iter *Iterator) readObjectFieldAsBytes() (ret []byte) {
	...
	if iter.buf[iter.head] != ':' {
		iter.ReportError("readObjectFieldAsBytes", "expect : after object ***REMOVED***eld")
		return
	}
	iter.head++
	if iter.skipWhitespacesWithoutLoadMore() {
