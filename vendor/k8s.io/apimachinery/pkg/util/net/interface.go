...
type Route struct {
	...
	Destination net.IP
	Gateway     net.IP
	// TODO: add more ***REMOVED***elds here if needed
}

// getRoutes obtains the IPv4 routes, and ***REMOVED***lters out non-default routes.
func getRoutes(input io.Reader) ([]Route, error) {
	...
	for {
		...
		***REMOVED***elds := strings.Fields(line)
		dest, err := parseHexToIPv4(***REMOVED***elds[1])
		if err != nil {
			return nil, err
		}
		gw, err := parseHexToIPv4(***REMOVED***elds[2])
		if err != nil {
			return nil, err
		}
...
// parseHexToIPv4 takes the hex IP address string from route ***REMOVED***le and converts it
...
//It uses data from /proc/net/route ***REMOVED***le.
...
func chooseHostInterfaceFromRoute(inFile io.Reader, nw networkInterfacer) (net.IP, error) {
	...
	for _, family := range []AddressFamily{familyIPv4, familyIPv6} {
		...
		for _, route := range routes {
			// TODO: When have IPv6 routes, ***REMOVED***lter here to speed up processing
