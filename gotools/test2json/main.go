...
// with no command speci***REMOVED***ed, test2json expects test output on standard input.
...
//	}
//
// The Time ***REMOVED***eld holds the time the event happened.
// It is conventionally omitted for cached test results.
//
// The Action ***REMOVED***eld is one of a ***REMOVED***xed set of action descriptions:
//
...
//
// The Package ***REMOVED***eld, if present, speci***REMOVED***es the package being tested.
// When the go command runs parallel tests in -json mode, events from
// different tests are interlaced; the Package ***REMOVED***eld allows readers to
// separate them.
//
// The Test ***REMOVED***eld, if present, speci***REMOVED***es the test, example, or benchmark
// function that caused the event. Events for the overall package test
// do not set Test.
//
// The Elapsed ***REMOVED***eld is set for "pass" and "fail" events. It gives the time
// elapsed for the speci***REMOVED***c test or the overall package test that passed or failed.
//
// The Output ***REMOVED***eld is set for Action == "output" and is a portion of the test's output
// (standard output and standard error merged together). The output is
// unmodi***REMOVED***ed except that invalid UTF-8 output from a test is coerced
// into valid UTF-8 by use of replacement characters. With that one exception,
// the concatenation of the Output ***REMOVED***elds of all output events is the exact
...
// and no Test ***REMOVED***eld. If a benchmark logs output or reports a failure
...
// by a ***REMOVED***nal event with Action == "bench" or "fail".
