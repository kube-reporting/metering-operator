...
Each ReportGenerationQuery is typically designed to report on a speci***REMOVED***c resource typically a `pod`, `namespace` or `node` and on a speci***REMOVED***c metric like, `cpu` or `memory`, on a speci***REMOVED***c resource, though some reports correlate many of these together.
Details on what information each report query provides can be found in the [report documentation][report-md].

Let's start with an example, save the following into a ***REMOVED***le called `report.yaml`:
