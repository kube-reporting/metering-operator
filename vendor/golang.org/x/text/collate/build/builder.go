...
// on those speci***REMOVED***ed in the allkeys* ***REMOVED***le as provided by the UCA or CLDR.
type Builder struct {
	index  *trieBuilder
	root   ordering
	locale []*Tailoring
	...
	ctHandle map[string]ctHandle // contraction handles keyed by a concatenation of the suf***REMOVED***xes
	ctElem   map[string]int      // contraction elements keyed by their string representation
}

// A Tailoring builds a collation table based on another collation table.
// The table is de***REMOVED***ned by specifying tailorings to the underlying table.
// See http://unicode.org/reports/tr35/ for an overview of tailoring
// collation tables.  The CLDR contains pre-de***REMOVED***ned tailorings for a variety
...
// A collation element is speci***REMOVED***ed as list of weights: []int{primary, secondary, ...}.
// The entries are typically obtained from a collation element table
// as de***REMOVED***ned in http://www.unicode.org/reports/tr10/#Data_Table_Format.
// Note that the collation elements speci***REMOVED***ed by colelems are only used
...
func (b *Builder) Add(runes []rune, colelems [][]int, variables []int) error {
	...
	for i, ce := range elems {
		...
		} ***REMOVED*** if p > 1 { // 1 is a special primary value reserved for FFFE
			if p <= b.varTop {
...
func (t *Tailoring) setAnchor(anchor string) error {
	...
	a := t.index.***REMOVED***nd(anchor)
	if a == nil {
		a = t.index.newEntry(anchor, nil)
		a.implicit = true
		a.modi***REMOVED***ed = true
		for _, r := range []rune(anchor) {
			e := t.index.***REMOVED***nd(string(r))
			e.lock = true
		}
	}
	t.anchor = a
...
// speci***REMOVED***cation.  See Insert for an example.
// SetAnchor supports the following logical reset positions:
// <***REMOVED***rst_tertiary_ignorable/>, <last_teriary_ignorable/>, <***REMOVED***rst_primary_ignorable/>,
...
// to the extend elements as de***REMOVED***ned in LDML.  A non-empty value for extend
...
func (t *Tailoring) Insert(level colltab.Level, str, extend string) error {
	...
	e := t.index.***REMOVED***nd(str)
	if e == nil {
		e = t.index.newEntry(str, nil)
	} ***REMOVED*** if e.logical != noAnchor {
	...
	a := t.anchor
	// Find the ***REMOVED***rst element after the anchor which differs at a level smaller or
	...
	if len(e.elems) == 0 && e.logical == noAnchor {
		if e.implicit {
			for _, r := range e.runes {
				e.elems = append(e.elems, o.getWeight(o.***REMOVED***nd(string(r)))...)
			}
		} ***REMOVED*** if e.before {
...
func (o *ordering) addExtension(e *entry) {
	if ex := o.***REMOVED***nd(e.extend); ex != nil {
		e.elems = append(e.elems, ex.elems...)
	} ***REMOVED*** {
		for _, r := range []rune(e.extend) {
			e.elems = append(e.elems, o.***REMOVED***nd(string(r)).elems...)
		}
	}
	e.extend = ""
}
...
func (o *ordering) verifyWeights(a, b *entry, level colltab.Level) error {
	...
	if a.elems[0].w[level] >= b.elems[0].w[level] {
		...
		log.Println(err)
		// TODO: return the error instead, or better, ***REMOVED***x the conflicting entry by making room.
	}
	return nil
}
...
func (o *ordering) patchNorm() {
	...
	for _, e := range o.ordered {
		...
		if nfd != e.str {
			if e0 := o.***REMOVED***nd(nfd); e0 != nil && !e0.modi***REMOVED***ed {
				e0.elems = e.elems
			} ***REMOVED*** if e.modi***REMOVED***ed && !equalCEArrays(o.genColElems(nfd), e.elems) {
	...
	for _, e := range o.ordered {
		nfd := norm.NFD.String(e.str)
		if e.modi***REMOVED***ed || nfd == e.str {
			continue
		}
		if e0 := o.***REMOVED***nd(nfd); e0 != nil {
			e.elems = e0.elems
		} ***REMOVED*** {
			...
			if norm.NFD.LastBoundary([]byte(nfd)) == 0 {
				...
				for i := 1; i < len(r); i++ {
					...
					if e0 := o.***REMOVED***nd(s); e0 != nil && e0.modi***REMOVED***ed {
						head = s
					} ***REMOVED*** {
						tail += string(r[i])
					}
...
// Print prints the tables for b and all its Tailorings as a Go ***REMOVED***le
...
func reproducibleFromNFKD(e *entry, exp, nfkd []rawCE) bool {
	...
	for i, ce := range exp {
		...
		if _, err := makeCE(ce); err != nil {
			// Simply return false. The error will be caught ***REMOVED***where.
			return false
		}
	}
	return true
...
func (b *Builder) processContractions(o *ordering) {
	...
	for _, r := range starters {
		l := cm[r]
		// Compute suf***REMOVED***x strings. There are 31 different contraction suf***REMOVED***x
