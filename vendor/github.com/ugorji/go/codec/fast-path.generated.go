...
//
// We de***REMOVED***ne the functions and register then in this single ***REMOVED***le
// so as not to pollute the encode.go and decode.go, and create a dependency in there.
// This ***REMOVED***le can be omitted without causing a build failure.
//
...
func (_ fastpathT) DecSliceIntfV(v []interface{}, checkNil bool, canChange bool, d *Decoder) (_ []interface{}, changed bool) {
	...
	if containerLenS > 0 {
		...
		if containerLenS > cap(v) {
			...
			x2read = len(v)
		} ***REMOVED*** if containerLenS != len(v) {
			if canChange {
				v = v[:containerLenS]
				changed = true
			}
