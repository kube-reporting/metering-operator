...
func addConversionFuncs(scheme *runtime.Scheme) error {
	...
	// Add ***REMOVED***eld label conversions for kinds having selectable nothing but ObjectMeta ***REMOVED***elds.
	for _, k := range []string{
		...
		err = scheme.AddFieldLabelConversionFunc("v1", kind,
			func(label, value string) (string, string, error) {
				...
				default:
					return "", "", fmt.Errorf("***REMOVED***eld label %q not supported for %q", label, kind)
				}
			},
		)
		if err != nil {
	...
	// Add ***REMOVED***eld conversion funcs.
	err = scheme.AddFieldLabelConversionFunc("v1", "Pod",
		func(label, value string) (string, string, error) {
			...
			default:
				return "", "", fmt.Errorf("***REMOVED***eld label not supported: %s", label)
			}
		},
	)
	if err != nil {
...
func Convert_extensions_ReplicaSet_to_v1_ReplicationController(in *extensions.ReplicaSet, out *ReplicationController, s conversion.Scope) error {
	...
	if err := Convert_extensions_ReplicaSetSpec_to_v1_ReplicationControllerSpec(&in.Spec, &out.Spec, s); err != nil {
		***REMOVED***eldErr, ok := err.(****REMOVED***eld.Error)
		if !ok {
			return err
		}
		...
		out.Annotations[NonConvertibleAnnotationPre***REMOVED***x+"/"+***REMOVED***eldErr.Field] = reflect.ValueOf(***REMOVED***eldErr.BadValue).String()
	}
...
func Convert_v1_PodStatusResult_To_api_PodStatusResult(in *PodStatusResult, out *api.PodStatusResult, s conversion.Scope) error {
	...
	// Move the annotation to the internal repr. ***REMOVED***eld
...
func Convert_api_PodSpec_To_v1_PodSpec(in *api.PodSpec, out *PodSpec, s conversion.Scope) error {
	...
	if in.SecurityContext != nil {
		// the host namespace ***REMOVED***elds have to be handled here for backward compatibility
		// with v1.0.0
...
func Convert_v1_PodSpec_To_api_PodSpec(in *PodSpec, out *api.PodSpec, s conversion.Scope) error {
	...
	// If both are speci***REMOVED***ed, ServiceAccountName (the new ***REMOVED***eld) wins.
	if in.ServiceAccountName == "" {
		out.ServiceAccountName = in.DeprecatedServiceAccount
	}

	// the host namespace ***REMOVED***elds have to be handled specially for backward compatibility
	// with v1.0.0
	...
	return scheme.AddFieldLabelConversionFunc("v1", "Event",
		func(label, value string) (string, string, error) {
			...
			case "involvedObject.kind",
				...
				"involvedObject.***REMOVED***eldPath",
				"reason",
				"source",
				"type",
				"metadata.namespace",
