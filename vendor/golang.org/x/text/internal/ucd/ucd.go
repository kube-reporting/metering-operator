	...
	for p.scanner.Scan() {
		...
		if b[0] == '@' {
			if p.partHandler != nil {
				p.***REMOVED***eld = append(p.***REMOVED***eld, bytes.TrimSpace(b[1:]))
				p.partHandler(p)
				p.***REMOVED***eld = p.***REMOVED***eld[:0]
			}
			p.comment = nil
		...
		for {
			i := bytes.IndexByte(b, ';')
			if i == -1 {
				p.***REMOVED***eld = append(p.***REMOVED***eld, bytes.TrimSpace(b))
				break
			}
			p.***REMOVED***eld = append(p.***REMOVED***eld, bytes.TrimSpace(b[:i]))
			b = b[i+1:]
		}
		if !p.keepRanges {
...
func (p *Parser) getRange(i int) (***REMOVED***rst, last rune) {
	...
	if i == 0 && len(p.***REMOVED***eld) > 1 && bytes.HasSuf***REMOVED***x(p.***REMOVED***eld[1], []byte("First>")) {
		if p.parsedRange {
...
func (p *Parser) Enum(i int, enum ...string) string {
	...
	p.setError(errUnde***REMOVED***nedEnum)
	return ""
}
