...
// ConvertToVersion converts in to the provided outVersion without copying the input ***REMOVED***rst, which
...
// SetField puts the value of src, into ***REMOVED***eldName, which must be a member of v.
// The value of src must be assignable to the ***REMOVED***eld.
func SetField(src interface{}, v reflect.Value, ***REMOVED***eldName string) error {
	***REMOVED***eld := v.FieldByName(***REMOVED***eldName)
	if !***REMOVED***eld.IsValid() {
		return fmt.Errorf("couldn't ***REMOVED***nd %v ***REMOVED***eld in %#v", ***REMOVED***eldName, v.Interface())
	}
	srcValue := reflect.ValueOf(src)
	if srcValue.Type().AssignableTo(***REMOVED***eld.Type()) {
		***REMOVED***eld.Set(srcValue)
		return nil
	}
	if srcValue.Type().ConvertibleTo(***REMOVED***eld.Type()) {
		***REMOVED***eld.Set(srcValue.Convert(***REMOVED***eld.Type()))
		return nil
	}
	return fmt.Errorf("couldn't assign/convert %v to %v", srcValue.Type(), ***REMOVED***eld.Type())
}

// Field puts the value of ***REMOVED***eldName, which must be a member of v, into dest,
// which must be a variable to which this ***REMOVED***eld's value can be assigned.
func Field(v reflect.Value, ***REMOVED***eldName string, dest interface{}) error {
	***REMOVED***eld := v.FieldByName(***REMOVED***eldName)
	if !***REMOVED***eld.IsValid() {
		return fmt.Errorf("couldn't ***REMOVED***nd %v ***REMOVED***eld in %#v", ***REMOVED***eldName, v.Interface())
	}
	...
	if ***REMOVED***eld.Type().AssignableTo(destValue.Type()) {
		destValue.Set(***REMOVED***eld)
		return nil
	}
	if ***REMOVED***eld.Type().ConvertibleTo(destValue.Type()) {
		destValue.Set(***REMOVED***eld.Convert(destValue.Type()))
		return nil
	}
	return fmt.Errorf("couldn't assign/convert %v to %v", ***REMOVED***eld.Type(), destValue.Type())
}

// ***REMOVED***eldPtr puts the address of ***REMOVED***eldName, which must be a member of v,
// into dest, which must be an address of a variable to which this ***REMOVED***eld's
// address can be assigned.
func FieldPtr(v reflect.Value, ***REMOVED***eldName string, dest interface{}) error {
	***REMOVED***eld := v.FieldByName(***REMOVED***eldName)
	if !***REMOVED***eld.IsValid() {
		return fmt.Errorf("couldn't ***REMOVED***nd %v ***REMOVED***eld in %#v", ***REMOVED***eldName, v.Interface())
	}
	...
	***REMOVED***eld = ***REMOVED***eld.Addr()
	if ***REMOVED***eld.Type().AssignableTo(v.Type()) {
		v.Set(***REMOVED***eld)
		return nil
	}
	if ***REMOVED***eld.Type().ConvertibleTo(v.Type()) {
		v.Set(***REMOVED***eld.Convert(v.Type()))
		return nil
	}
	return fmt.Errorf("couldn't assign/convert %v to %v", ***REMOVED***eld.Type(), v.Type())
}
