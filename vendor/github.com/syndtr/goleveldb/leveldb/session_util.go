...
func (s *session) addFileRef(fd storage.FileDesc, ref int) int {
	ref += s.***REMOVED***leRef[fd.Num]
	if ref > 0 {
		s.***REMOVED***leRef[fd.Num] = ref
	} ***REMOVED*** if ref == 0 {
		delete(s.***REMOVED***leRef, fd.Num)
	} ***REMOVED*** {
...
func (s *session) setVersion(v *version) {
	s.vmu.Lock()
	defer s.vmu.Unlock()
	// Hold by session. It is important to call this ***REMOVED***rst before releasing
	// current version, otherwise the still used ***REMOVED***les might get released.
	v.incref()
	if s.stVersion != nil {
...
// Get current unused ***REMOVED***le number.
func (s *session) nextFileNum() int64 {
	return atomic.LoadInt64(&s.stNextFileNum)
}

// Set current unused ***REMOVED***le number to num.
...
// synchronization.
func (s *session) ***REMOVED***llRecord(r *sessionRecord, snapshot bool) {
...
// Create a new manifest ***REMOVED***le; need external synchronization.
func (s *session) newManifest(rec *sessionRecord, v *version) (err error) {
	...
	s.***REMOVED***llRecord(rec, true)
	v.***REMOVED***llRecord(rec)
