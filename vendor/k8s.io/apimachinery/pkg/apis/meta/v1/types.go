...
type ListMeta struct {
	...
	// list may not be possible if the server con***REMOVED***guration has changed or more than a few minutes have
	// passed. The resourceVersion ***REMOVED***eld returned when using this continue value will be identical to
	// the value in the ***REMOVED***rst response.
...
type ListOptions struct {
	...
	// is server de***REMOVED***ned, clients may only use the continue value from a previous query result with
	// identical query parameters (except for the value of continue) and the server may reject a continue
	// value it does not recognize. If the speci***REMOVED***ed continue value is no longer valid whether due to
	// expiration (generally ***REMOVED***ve to ***REMOVED***fteen minutes) or a con***REMOVED***guration change on the server the server
	// will respond with a 410 ResourceExpired error indicating the client must restart their list without
	// the continue ***REMOVED***eld. This ***REMOVED***eld is not supported when watch is true. Clients may start a watch from
	// the last resourceVersion value returned by the server and not miss any modi***REMOVED***cations.
