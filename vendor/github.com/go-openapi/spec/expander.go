...
// ref is guaranteed to be in root (no need to go to external ***REMOVED***les)
...
func nextRef(startingNode interface{}, startingRef *Ref, ptr *jsonpointer.Pointer) *Ref {
	...
	for _, tok := range ptr.DecodedTokens() {
		...
		if refRef != nil {
			...
			nwURL := nw.GetURL()
			if nwURL.Scheme == "***REMOVED***le" || (nwURL.Scheme == "" && nwURL.Host == "") {
				nwpt := ***REMOVED***lepath.ToSlash(nwURL.Path)
				if ***REMOVED***lepath.IsAbs(nwpt) {
					_, err := os.Stat(nwpt)
					if err != nil {
						nwURL.Path = ***REMOVED***lepath.Join(".", nwpt)
					}
				}
			}
...
// base or refPath could be a ***REMOVED***le path or a URL
...
// base could be a directory or a full ***REMOVED***le path
func normalizePaths(refPath, base string) string {
	refURL, _ := url.Parse(refPath)
	if path.IsAbs(refURL.Path) || ***REMOVED***lepath.IsAbs(refPath) {
		// refPath is actually absolute
		if refURL.Host != "" {
			return refPath
		}
		parts := strings.Split(refPath, "#")
		result := ***REMOVED***lepath.FromSlash(parts[0])
		if len(parts) == 2 {
			result += "#" + parts[1]
		}
		return result
	...
	if !strings.HasPre***REMOVED***x(refPath, "#") {
		// combining paths
		if baseURL.Host != "" {
			baseURL.Path = path.Join(path.Dir(baseURL.Path), refURL.Path)
		} ***REMOVED*** { // base is a ***REMOVED***le
			newBase := fmt.Sprintf("%s#%s", ***REMOVED***lepath.Join(***REMOVED***lepath.Dir(base), ***REMOVED***lepath.FromSlash(refURL.Path)), refURL.Fragment)
			return newBase
		}
...
// relativeBase could be an ABSOLUTE ***REMOVED***le path or an ABSOLUTE URL
...
func (r *schemaLoader) resolveRef(ref *Ref, target interface{}, basePath string) error {
	...
	if (ref.IsRoot() || ref.HasFragmentOnly) && root != nil {
		...
		debugLog("current ref normalized ***REMOVED***le: %s", baseRef.String())
...
// absPath returns the absolute path of a ***REMOVED***le
func absPath(fname string) (string, error) {
	if strings.HasPre***REMOVED***x(fname, "http") {
		return fname, nil
	}
	if ***REMOVED***lepath.IsAbs(fname) {
		return fname, nil
	}
	wd, err := os.Getwd()
	return ***REMOVED***lepath.Join(wd, fname), err
}
...
func ExpandSpec(spec *Swagger, options *ExpandOptions) error {
	...
	if options == nil || !options.SkipSchemas {
		for key, de***REMOVED***nition := range spec.De***REMOVED***nitions {
			var def *Schema
			var err error
			if def, err = expandSchema(de***REMOVED***nition, []string{fmt.Sprintf("#/de***REMOVED***nitions/%s", key)}, resolver, specBasePath); shouldStopOnError(err, resolver.options) {
				return err
			}
			if def != nil {
				spec.De***REMOVED***nitions[key] = *def
			}
		}
	}
...
// notice that it is impossible to reference a json scema in a different ***REMOVED***le other than root
func ExpandSchema(schema *Schema, root interface{}, cache ResolutionCache) error {
	// Only save the root to a tmp ***REMOVED***le if it isn't nil.
	var base string
	if root != nil {
		base, _ = absPath("root")
		if cache == nil {
...
// ExpandSchemaWithBasePath expands the refs in the schema object, base path con***REMOVED***gured through expand options
...
func expandSchema(target Schema, parentRefs []string, resolver *schemaLoader, basePath string) (*Schema, error) {
	...
	if target.ID != "" {
		...
		// remember that basePath has to be a ***REMOVED***le
		refPath := target.ID
		if strings.HasSuf***REMOVED***x(target.ID, "/") {
	...
	var t *Schema
	/* if Ref is found, everything ***REMOVED*** doesn't matter */
	...
	for k := range target.De***REMOVED***nitions {
		t, err := expandSchema(target.De***REMOVED***nitions[k], parentRefs, resolver, basePath)
		if shouldStopOnError(err, resolver.options) {
			return &target, err
		}
		if t != nil {
			target.De***REMOVED***nitions[k] = *t
		}
	}
	return &target, nil
}
...
func transitiveResolver(basePath string, ref Ref, resolver *schemaLoader) (*schemaLoader, error) {
	...
	if !strings.HasPre***REMOVED***x(currentRef.String(), baseRef.String()) {
