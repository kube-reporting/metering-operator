...
func newMatcher(supported []Tag) *matcher {
	...
	for _, ml := range matchLang {
		update(ml.want, ml.have, Con***REMOVED***dence(ml.conf), false)
		if !ml.oneway {
			update(ml.have, ml.want, Con***REMOVED***dence(ml.conf), false)
		}
	}
...
func (m *bestMatch) update(have *haveTag, tag Tag, maxScript scriptID, maxRegion regionID) {
	...
	// Next we prefer smaller distances between regions, as de***REMOVED***ned by regionDist.
