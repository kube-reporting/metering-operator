...
// s3nLocation returns the HDFS path based on an S3 bucket and prefix.
func s3nLocation(bucket, prefix string) string {
	return fmt.Sprintf("s3n://%s/%s", bucket, prefix)
}
