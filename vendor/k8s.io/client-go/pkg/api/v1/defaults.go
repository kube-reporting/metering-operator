...
func SetDefaults_ReplicationController(obj *ReplicationController) {
	...
	// TODO: support templates de***REMOVED***ned ***REMOVED***where when we support them in the API
	if labels != nil {
...
func SetDefaults_Container(obj *Container) {
	if obj.ImagePullPolicy == "" {
		// Ignore error and assume it has been validated ***REMOVED***where
...
func SetDefaults_Service(obj *Service) {
	if obj.Spec.SessionAf***REMOVED***nity == "" {
		obj.Spec.SessionAf***REMOVED***nity = ServiceAf***REMOVED***nityNone
	}
	if obj.Spec.Type == "" {
	...
	// Defaults ExternalTraf***REMOVED***cPolicy ***REMOVED***eld for NodePort / LoadBalancer service
	// to Global for consistency.
	if _, ok := obj.Annotations[BetaAnnotationExternalTraf***REMOVED***c]; ok {
		// Don't default this ***REMOVED***eld if beta annotation exists.
		return
	} ***REMOVED*** if (obj.Spec.Type == ServiceTypeNodePort ||
		obj.Spec.Type == ServiceTypeLoadBalancer) &&
		obj.Spec.ExternalTraf***REMOVED***cPolicy == "" {
		obj.Spec.ExternalTraf***REMOVED***cPolicy = ServiceExternalTraf***REMOVED***cPolicyTypeCluster
	}
}
func SetDefaults_Pod(obj *Pod) {
	// If limits are speci***REMOVED***ed, but requests are not, default requests to limits
	// This is done here rather than a more speci***REMOVED***c defaulting pass on ResourceRequirements
	// because we only want this defaulting semantic to take place on a Pod and not a PodTemplate
	for i := range obj.Spec.Containers {
		// set requests to limits if requests are not speci***REMOVED***ed, but limits are
...
func SetDefaults_Con***REMOVED***gMapVolumeSource(obj *Con***REMOVED***gMapVolumeSource) {
	if obj.DefaultMode == nil {
		perm := int32(Con***REMOVED***gMapVolumeSourceDefaultMode)
		obj.DefaultMode = &perm
	}
}
...
func SetDefaults_LimitRangeItem(obj *LimitRangeItem) {
	...
	if obj.Type == LimitTypeContainer {
		...
		// If a default limit is unspeci***REMOVED***ed, but the max is speci***REMOVED***ed, default the limit to the max
		for key, value := range obj.Max {
			if _, exists := obj.Default[key]; !exists {
				obj.Default[key] = *(value.Copy())
			}
		}
		// If a default limit is speci***REMOVED***ed, but the default request is not, default request to limit
		for key, value := range obj.Default {
			if _, exists := obj.DefaultRequest[key]; !exists {
				obj.DefaultRequest[key] = *(value.Copy())
			}
		}
		// If a default request is not speci***REMOVED***ed, but the min is provided, default request to the min
...
func SetDefaults_Con***REMOVED***gMap(obj *Con***REMOVED***gMap) {
	if obj.Data == nil {
