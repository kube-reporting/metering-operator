...
func (i item) String() string {
	...
	case i.typ == itemError:
		return i.val
	case i.typ == itemIdenti***REMOVED***er || i.typ == itemMetricIdenti***REMOVED***er:
...
const (
	...
	itemEOF
	itemComment
	itemIdenti***REMOVED***er
	itemMetricIdenti***REMOVED***er
	itemLeftParen
	itemRightParen
	itemLeftBrace
	itemRightBrace
...
func (i itemType) desc() string {
	...
	case itemComment:
		return "comment"
	case itemIdenti***REMOVED***er:
		return "identi***REMOVED***er"
	case itemMetricIdenti***REMOVED***er:
		return "metric identi***REMOVED***er"
	case itemString:
		return "string"
	case itemNumber:
		return "number"
...
func lexStatements(l *lexer) stateFn {
	...
	if strings.HasPre***REMOVED***x(l.input[l.pos:], lineComment) {
	...
	case isAlpha(r) || r == ':':
		l.backup()
		return lexKeywordOrIdenti***REMOVED***er
	case r == '(':
...
// was only modi***REMOVED***ed to integrate with our lexer.
...
// lexIdenti***REMOVED***er scans an alphanumeric identi***REMOVED***er. The next character
// is known to be a letter.
func lexIdenti***REMOVED***er(l *lexer) stateFn {
	for isAlphaNumeric(l.next()) {
		// absorb
	}
	l.backup()
	l.emit(itemIdenti***REMOVED***er)
	return lexStatements
}

// lexKeywordOrIdenti***REMOVED***er scans an alphanumeric identi***REMOVED***er which may contain
// a colon rune. If the identi***REMOVED***er is a keyword the respective keyword item
// is scanned.
func lexKeywordOrIdenti***REMOVED***er(l *lexer) stateFn {
Loop:
	for {
		switch r := l.next(); {
		...
		default:
			...
			if kw, ok := key[strings.ToLower(word)]; ok {
				l.emit(kw)
			} ***REMOVED*** if !strings.Contains(word, ":") {
				l.emit(itemIdenti***REMOVED***er)
			} ***REMOVED*** {
				l.emit(itemMetricIdenti***REMOVED***er)
			}
			break Loop
		}
	}
...
// instead because that also classi***REMOVED***es non-Latin digits as digits. See
