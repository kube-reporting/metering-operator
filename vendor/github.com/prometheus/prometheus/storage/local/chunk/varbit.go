...
// of the ***REMOVED***rst two samples of a chunk as the base delta, but uses a "sliding"
...
// Note that there are signi***REMOVED***cant differences, some due to the way Prometheus
...
// - ***REMOVED***rst time (int64):                  8 bytes  bit 0000-0063
// - ***REMOVED***rst value (float64):               8 bytes  bit 0064-0127
// - last time (int64):                   8 bytes  bit 0128-0191
// - last value (float64):                8 bytes  bit 0192-0255
// - ***REMOVED***rst Δt (t1-t0, unsigned):          3 bytes  bit 0256-0279
// - flags (byte)                         1 byte   bit 0280-0287
// - bit offset for next sample           2 bytes  bit 0288-0303
// - ***REMOVED***rst Δv for value encoding 1, otherwise payload
...
//     - last signi***REMOVED***cant bits (1 byte)   1 byte   bit 8168-8175
//
// FLAGS
//
// The two least signi***REMOVED***cant bits of the flags byte de***REMOVED***ne the value encoding
// for the whole chunk, see below. The most signi***REMOVED***cant byte of the flags byte
...
//
// The difference to the 2nd timestamp is saved as ***REMOVED***rst Δt. 3 bytes is enough
...
// VALUE ENCODING
//
// Value encoding can change and is determined by the two least signi***REMOVED***cant bits
...
// of ΔΔv=0 is represented by a single '0' bit for each occurrence. The ***REMOVED***rst Δv
...
// signi***REMOVED***cant bits is saved as (count-1), i.e. a saved value of 0 means 1
// signi***REMOVED***cant bit, a saved value of 1 means 2, and so on. Also, we save the
// numbers of leading zeros and signi***REMOVED***cant bits anew if they drop a
// lot. Otherwise, you can easily be locked in with a high number of signi***REMOVED***cant
// bits.
//
// 3: Direct encoding.
//
// If the sample values are just random, it is most ef***REMOVED***cient to save sample
...
const (
	...
	varbitLastSigni***REMOVED***cantBitsCountOffset = ChunkLen - 3
	...
	varbitMaxTimeDelta = 1 << 24 // What ***REMOVED***ts into a 3-byte timestamp.
)
...
func (c varbitChunk) UnmarshalFromBuf(buf []byte) error {
	if copied := copy(c, buf); copied != cap(c) {
		return fmt.Errorf("insuf***REMOVED***cient bytes copied from buffer during unmarshaling, want %d, got %d", cap(c), copied)
	}
	return nil
}
...
func (c varbitChunk) ***REMOVED***rstValue() model.SampleValue {
...
func (c varbitChunk) ***REMOVED***rstTimeDelta() model.Time {
	// Only the ***REMOVED***rst 3 bytes are actually the timestamp, so get rid of the
...
// ***REMOVED***rstValueDelta returns an unde***REMOVED***ned result if the encoding type is not 1.
func (c varbitChunk) ***REMOVED***rstValueDelta() int32 {
	return int32(binary.BigEndian.Uint32(c[varbitFirstValueDeltaOffset:]))
}

// lastTimeDelta returns an unde***REMOVED***ned result if the chunk is closed already.
...
// not be called with a time that doesn't ***REMOVED***t into 24bit, either.
...
// lastValueDelta returns an unde***REMOVED***ned result if the chunk is closed already.
...
func (c varbitChunk) closed() bool {
	return c[varbitFlagOffset] > 0x7F // Most signi***REMOVED***cant bit set.
}
...
func (c varbitChunk) setZeroDDTRepeats(repeats uint64, offset uint16) {
	...
	default:
		...
		// before. We need to clear the bits ***REMOVED***rst.
...
// ***REMOVED***rst time delta from the provided sample and adds it to the chunk together
// with the provided sample as the last sample.
func (c *varbitChunk) addSecondSample(s model.SamplePair) ([]Chunk, error) {
	***REMOVED***rstTimeDelta := s.Timestamp - c.FirstTime()
	if ***REMOVED***rstTimeDelta < 0 {
		return nil, fmt.Errorf("***REMOVED***rst Δt is less than zero: %v", ***REMOVED***rstTimeDelta)
	}
	if ***REMOVED***rstTimeDelta > varbitMaxTimeDelta {
		// A time delta too great. Still, we can add it as a last sample
		// before overflowing.
		return c.addLastSample(s), nil
	}
	(*c)[varbitFirstTimeDeltaOffset] = byte(***REMOVED***rstTimeDelta >> 16)
	(*c)[varbitFirstTimeDeltaOffset+1] = byte(***REMOVED***rstTimeDelta >> 8)
	(*c)[varbitFirstTimeDeltaOffset+2] = byte(***REMOVED***rstTimeDelta)

	// Also set ***REMOVED***rstTimeDelta as the last time delta to be able to use the
	// normal methods for adding later samples.
	c.setLastTimeDelta(***REMOVED***rstTimeDelta)
...
func (c *varbitChunk) addLaterSample(s model.SamplePair, offset uint16) ([]Chunk, error) {
	...
	// Analyze worst case, does it ***REMOVED***t? If not, set new sample as the last.
	if int(offset)+varbitWorstCaseBitsPerSample[encoding] > ChunkLen*8 {
		return c.addLastSample(s), nil
	}

	// Transcoding/overflow decisions ***REMOVED***rst.
...
func (c varbitChunk) prepForThirdSample(
	...
	var (
		...
		***REMOVED***rstValue               = c.***REMOVED***rstValue()
		***REMOVED***rstValueDelta          = lastValue - ***REMOVED***rstValue
		***REMOVED***rstXOR                 = math.Float64bits(float64(***REMOVED***rstValue)) ^ math.Float64bits(float64(lastValue))
		_, ***REMOVED***rstSigni***REMOVED***cantBits  = countBits(***REMOVED***rstXOR)
		secondXOR                = math.Float64bits(float64(lastValue)) ^ math.Float64bits(float64(newValue))
		_, secondSigni***REMOVED***cantBits = countBits(secondXOR)
	)
	// Now pick an initial encoding and prepare things accordingly.
	// However, never pick an encoding "below" the one initially set.
	switch {
	case encoding == varbitZeroEncoding && lastValue == ***REMOVED***rstValue && lastValue == newValue:
		// Stay at zero encoding.
		// No value to be set.
		// No offset change required.
	case encoding <= varbitIntDoubleDeltaEncoding && isInt32(***REMOVED***rstValueDelta):
		encoding = varbitIntDoubleDeltaEncoding
		binary.BigEndian.PutUint32(
			c[varbitFirstValueDeltaOffset:],
			uint32(int32(***REMOVED***rstValueDelta)),
		)
		c.setLastValueDelta(int32(***REMOVED***rstValueDelta))
		offset += 32
	case encoding == varbitDirectEncoding || ***REMOVED***rstSigni***REMOVED***cantBits+secondSigni***REMOVED***cantBits > 100:
	...
	default:
		...
		offset = c.addXORValue(offset, ***REMOVED***rstValue, lastValue)
	}
...
func (c varbitChunk) addXORValue(offset uint16, lastValue, newValue model.SampleValue) uint16 {
	...
	lastSigni***REMOVED***cantBits := c[varbitLastSigni***REMOVED***cantBitsCountOffset]
	newLeadingBits, newSigni***REMOVED***cantBits := countBits(xor)

	// Short entry if the new signi***REMOVED***cant bits ***REMOVED***t into the same box as the
	// last signi***REMOVED***cant bits.  However, should the new signi***REMOVED***cant bits be
	...
	if newLeadingBits >= lastLeadingBits &&
		newLeadingBits+newSigni***REMOVED***cantBits <= lastLeadingBits+lastSigni***REMOVED***cantBits &&
		lastSigni***REMOVED***cantBits-newSigni***REMOVED***cantBits < 10 {
		offset = c.addOneBitsWithTrailingZero(offset, 1)
		return c.addBitPattern(
			offset,
			xor>>(64-lastLeadingBits-lastSigni***REMOVED***cantBits),
			uint16(lastSigni***REMOVED***cantBits),
		)
	}

	// Long entry.
	c[varbitLastLeadingZerosCountOffset] = newLeadingBits
	c[varbitLastSigni***REMOVED***cantBitsCountOffset] = newSigni***REMOVED***cantBits
	offset = c.addOneBits(offset, 2)
	offset = c.addBitPattern(offset, uint64(newLeadingBits), 5)
	offset = c.addBitPattern(offset, uint64(newSigni***REMOVED***cantBits-1), 6) // Note -1!
	return c.addBitPattern(
		offset,
		xor>>(64-newLeadingBits-newSigni***REMOVED***cantBits),
		uint16(newSigni***REMOVED***cantBits),
	)
}
...
// representable as such. (Check with isSignedIntN ***REMOVED***rst.)
...
type varbitChunkIterator struct {
	...
	// used for pos. len is the offset of the ***REMOVED***rst bit in the chunk that is
	...
	leading, signi***REMOVED***cant uint16
...
func (it *varbitChunkIterator) Scan() bool {
	...
	if it.pos == varbitSecondSampleBitOffset {
		...
		it.dT = it.c.***REMOVED***rstTimeDelta()
		it.t += it.dT
		...
		case varbitIntDoubleDeltaEncoding:
			it.dV = int64(it.c.***REMOVED***rstValueDelta())
...
// ***REMOVED***ndAtOrBefore implements Iterator.
...
func (it *varbitChunkIterator) readXOR() {
	...
	case 2:
		...
		it.signi***REMOVED***cant = uint16(it.readBitPattern(6)) + 1
	default:
		panic("unexpected number of control bits")
	}
	pattern := math.Float64bits(float64(it.v))
	pattern ^= it.readBitPattern(it.signi***REMOVED***cant) << (64 - it.signi***REMOVED***cant - it.leading)
	it.v = model.SampleValue(math.Float64frombits(pattern))
}

// readControlBits reads successive 1-bits and stops after reading the ***REMOVED***rst
