...
// there were insuf***REMOVED***cient bytes in src to complete and introduces an error
...
// copyXOR copies the current rune to dst and modi***REMOVED***es it by applying the XOR
// pattern of the case info. It is the responsibility of the caller to ensure
// that this is a rune with a XOR pattern de***REMOVED***ned.
...
// hasPre***REMOVED***x returns true if src[pSrc:] starts with the given string.
func (c *context) hasPre***REMOVED***x(s string) bool {
	b := c.src[c.pSrc:]
	if len(b) < len(s) {
		return false
	}
...
func upper(c *context) bool {
	...
	e := exceptions[c.info>>exceptionShift:]
	offset := 2 + e[0]&lengthMask // size of header + fold string
	// Get length of ***REMOVED***rst special case mapping.
	n := (e[1] >> lengthBits) & lengthMask
	if ct == cTitle {
		// The ***REMOVED***rst special case mapping is for lower. Set n to the second.
		if n == noChange {
			n = 0
		}
