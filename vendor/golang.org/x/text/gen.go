...
func main() {
	...
	// TODO: create temporary cache directory to load ***REMOVED***les and create and set
	...
	var (
		...
		mib        = generate("./encoding/internal/identi***REMOVED***er", unicode)
...
func generate(pkg string, deps ...*dependency) *dependency {
	...
	go func() {
		defer wg.Done()
		defer all.Done()
		// Wait for dependencies to ***REMOVED***nish.
		for _, d := range deps {
			d.Wait()
		...
		cmd := exec.Command(***REMOVED***lepath.Join(runtime.GOROOT(), "bin", "go"), args...)
		w := &bytes.Buffer{}
		cmd.Stderr = w
		cmd.Stdout = w
...
func copyExported(p string) {
	copyPackage(
		***REMOVED***lepath.Join("internal", "export", path.Base(p)),
		***REMOVED***lepath.Join("..", ***REMOVED***lepath.FromSlash(p[len("golang.org/x"):])),
...
func copyVendored() {
	root := ***REMOVED***lepath.Join(build.Default.GOROOT, ***REMOVED***lepath.FromSlash("src/vendor/golang_org/x"))

	err := ***REMOVED***lepath.Walk(root, func(dir string, info os.FileInfo, err error) error {
		if err != nil || !info.IsDir() || root == dir {
			return err
		}
		src := dir[len(root)+1:]
		const slash = string(***REMOVED***lepath.Separator)
		if c := strings.Split(src, slash); c[0] == "text" {
			...
			src = ***REMOVED***lepath.Join("internal", "export", ***REMOVED***lepath.Base(src))
		}
...
// copyPackage copies relevant ***REMOVED***les from a directory in x/text to the
// destination package directory. The destination package is assumed to have
// the same name. For each copied ***REMOVED***le go:generate lines are removed and
// and package comments are rewritten to the new path.
func copyPackage(dirSrc, dirDst, search, replace string) {
	err := ***REMOVED***lepath.Walk(dirSrc, func(***REMOVED***le string, info os.FileInfo, err error) error {
		base := ***REMOVED***lepath.Base(***REMOVED***le)
		if err != nil || info.IsDir() ||
			!strings.HasSuf***REMOVED***x(base, ".go") ||
			strings.HasSuf***REMOVED***x(base, "_test.go") && !strings.HasPre***REMOVED***x(base, "example") ||
			// Don't process subdirectories.
			***REMOVED***lepath.Dir(***REMOVED***le) != dirSrc {
			return nil
		}
		b, err := ioutil.ReadFile(***REMOVED***le)
		...
		if !bytes.HasPre***REMOVED***x(b, []byte(comment)) {
			b = append([]byte(comment), b...)
		}
		if b, err = format.Source(b); err != nil {
			fmt.Println("Failed to format ***REMOVED***le:", err)
			os.Exit(1)
		}
		***REMOVED***le = ***REMOVED***lepath.Join(dirDst, base)
		vprintf("=== COPY %s\n", ***REMOVED***le)
		return ioutil.WriteFile(***REMOVED***le, b, 0666)
	})
	if err != nil {
		fmt.Println("Copying exported ***REMOVED***les failed:", err)
		os.Exit(1)
	}
}
