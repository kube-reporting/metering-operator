...
type NodeExpansion interface {
	// PatchStatus modi***REMOVED***es the status of an existing node. It returns the copy
	// of the node that the server returns, or an error.
	PatchStatus(nodeName string, data []byte) (*v1.Node, error)
}

// PatchStatus modi***REMOVED***es the status of an existing node. It returns the copy of
