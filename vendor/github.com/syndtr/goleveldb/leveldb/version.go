...
type version struct {
	...
	// Score < 1 means compaction is not strictly needed. These ***REMOVED***elds
...
func (v *version) walkOverlapping(aux tFiles, ikey internalKey, f func(level int, t *tFile) bool, lf func(level int) bool) {
	...
	for level, tables := range v.levels {
		...
		if level == 0 {
			// Level-0 ***REMOVED***les may overlap each other. Find all ***REMOVED***les that
			// overlap ukey.
...
func (v *version) get(aux tFiles, ikey internalKey, ro *opt.ReadOptions, noValue bool) (value []byte, tcomp bool, err error) {
	...
	// Since entries never hop across level, ***REMOVED***nding key/value
	...
	v.walkOverlapping(aux, ikey, func(level int, t *tFile) bool {
		...
		var (
			***REMOVED***key, fval []byte
			ferr        error
		)
		if noValue {
			***REMOVED***key, ferr = v.s.tops.***REMOVED***ndKey(t, ikey, ro)
		} ***REMOVED*** {
			***REMOVED***key, fval, ferr = v.s.tops.***REMOVED***nd(t, ikey, ro)
		}
		...
		if fukey, fseq, fkt, fkerr := parseInternalKey(***REMOVED***key); fkerr == nil {
...
func (v *version) getIterators(slice *util.Range, ro *opt.ReadOptions) (its []iterator.Iterator) {
	...
	for level, tables := range v.levels {
		if level == 0 {
			// Merge all level zero ***REMOVED***les together since they may overlap.
			for _, t := range tables {
				its = append(its, v.s.tops.newIterator(t, slice, ro))
			}
		} ***REMOVED*** if len(tables) != 0 {
...
func (v *version) spawn(r *sessionRecord) *version {
	...
	staging.commit(r)
	return staging.***REMOVED***nish()
}

func (v *version) ***REMOVED***llRecord(r *sessionRecord) {
	for level, tables := range v.levels {
		for _, t := range tables {
	...
	for level, tables := range v.levels {
		for _, t := range tables {
			if v.s.icmp.Compare(t.imax, ikey) <= 0 {
				// Entire ***REMOVED***le is before "ikey", so just add the ***REMOVED***le size
				n += t.size
			} ***REMOVED*** if v.s.icmp.Compare(t.imin, ikey) > 0 {
				// Entire ***REMOVED***le is after "ikey", so ignore
				if level > 0 {
					// Files other than level 0 are sorted by meta->min, so
					// no further ***REMOVED***les in this level will contain data for
					// "ikey".
					break
				}
			} ***REMOVED*** {
		...
		if !v.levels[0].overlaps(v.s.icmp, umin, umax, true) {
			...
			for ; level < maxLevel; level++ {
				if pLevel := level + 1; pLevel >= len(v.levels) {
					return maxLevel
				} ***REMOVED*** if v.levels[pLevel].overlaps(v.s.icmp, umin, umax, false) {
					break
				}
...
func (v *version) computeCompaction() {
	...
	for level, tables := range v.levels {
		...
		size := tables.size()
		if level == 0 {
			// We treat level-0 specially by bounding the number of ***REMOVED***les
			// instead of number of bytes for two reasons:
			//
			// (1) With larger write-buffer sizes, it is nice not to do too
			// many level-0 compaction.
			//
			// (2) The ***REMOVED***les in level-0 are merged on every read and
			// therefore we wish to avoid too many ***REMOVED***les when the individual
			// ***REMOVED***le size is small (perhaps because of a small write-buffer
...
func (p *versionStaging) ***REMOVED***nish() *version {
	// Build new version.
