...
func NewRequest(client HTTPClient, verb string, baseURL *url.URL, versionedAPIPath string, content ContentCon***REMOVED***g, serializers Serializers, backoff BackoffManager, throttle flowcontrol.RateLimiter) *Request {
	if backoff == nil {
...
const (
	// A constant that clients can use to refer in a ***REMOVED***eld selector to the object name ***REMOVED***eld.
	...
	eventInvolvedFieldPath       = "involvedObject.***REMOVED***eldPath"
)

type clientFieldNameToAPIVersionFieldName map[string]string

func (c clientFieldNameToAPIVersionFieldName) ***REMOVED***lterField(***REMOVED***eld, value string) (newField, newValue string, err error) {
	newFieldName, ok := c[***REMOVED***eld]
	if !ok {
		return "", "", fmt.Errorf("%v - %v - no ***REMOVED***eld mapping de***REMOVED***ned", ***REMOVED***eld, value)
	}
...
func (r resourceTypeToFieldMapping) ***REMOVED***lterField(resourceType, ***REMOVED***eld, value string) (newField, newValue string, err error) {
	fMapping, ok := r[resourceType]
	if !ok {
		return "", "", fmt.Errorf("%v - %v - %v - no ***REMOVED***eld mapping de***REMOVED***ned", resourceType, ***REMOVED***eld, value)
	}
	return fMapping.***REMOVED***lterField(***REMOVED***eld, value)
}

type versionToResourceToFieldMapping map[schema.GroupVersion]resourceTypeToFieldMapping

// ***REMOVED***lterField transforms the given ***REMOVED***eld/value selector for the given groupVersion and resource
func (v versionToResourceToFieldMapping) ***REMOVED***lterField(groupVersion *schema.GroupVersion, resourceType, ***REMOVED***eld, value string) (newField, newValue string, err error) {
	rMapping, ok := v[*groupVersion]
	if !ok {
		// no groupVersion overrides registered, default to identity mapping
		return ***REMOVED***eld, value, nil
	}
	newField, newValue, err = rMapping.***REMOVED***lterField(resourceType, ***REMOVED***eld, value)
	if err != nil {
...
var ***REMOVED***eldMappings = versionToResourceToFieldMapping{
...
func (r *Request) FieldsSelectorParam(s ***REMOVED***elds.Selector) *Request {
	if r.err != nil {
		return r
	}
	if s == nil {
	...
	s2, err := s.Transform(func(***REMOVED***eld, value string) (newField, newValue string, err error) {
		return ***REMOVED***eldMappings.***REMOVED***lterField(r.content.GroupVersion, r.resource, ***REMOVED***eld, value)
	})
	if err != nil {
		r.err = err
		return r
	...
	for k, v := range params {
		for _, value := range v {
			...
			if k == metav1.LabelSelectorQueryParam(r.content.GroupVersion.String()) && value == "" {
				...
				// and unspeci***REMOVED***ed string, we don't set it to avoid having
			...
			if k == metav1.FieldSelectorQueryParam(r.content.GroupVersion.String()) {
				if len(value) == 0 {
					...
					// Since there is no way to get the difference between empty
					// and unspeci***REMOVED***ed string, we don't set it to avoid having
					// ***REMOVED***eldSelector= param in every request.
					continue
				}
				// TODO: Filtering should be handled somewhere ***REMOVED***.
				selector, err := ***REMOVED***elds.ParseSelector(value)
				if err != nil {
					r.err = fmt.Errorf("unparsable ***REMOVED***eld selector: %v", err)
					return r
				}
				***REMOVED***lteredSelector, err := selector.Transform(
					func(***REMOVED***eld, value string) (newField, newValue string, err error) {
						return ***REMOVED***eldMappings.***REMOVED***lterField(r.content.GroupVersion, r.resource, ***REMOVED***eld, value)
					})
				if err != nil {
					r.err = fmt.Errorf("untransformable ***REMOVED***eld selector: %v", err)
					return r
				}
				value = ***REMOVED***lteredSelector.String()
			}
...
// underyling object.  This means some useful request info (like the types of ***REMOVED***eld
// selectors in use) will be lost.
// TODO: preserve ***REMOVED***eld selector keys
...
func glogBody(pre***REMOVED***x string, body []byte) {
	if glog.V(8) {
		if bytes.IndexFunc(body, func(r rune) bool {
			...
			glog.Infof("%s:\n%s", pre***REMOVED***x, hex.Dump(body))
		} ***REMOVED*** {
			glog.Infof("%s: %s", pre***REMOVED***x, string(body))
		}
	}
}
