...
// genExpansion produces a ***REMOVED***le for a group client, e.g. ExtensionsClient for the extension group.
...
func (g *genExpansion) GenerateType(c *generator.Context, t *types.Type, w io.Writer) error {
	...
	for _, t := range g.types {
		if _, err := os.Stat(***REMOVED***lepath.Join(g.groupPackagePath, strings.ToLower(t.Name.Name+"_expansion.go"))); os.IsNotExist(err) {
