...
// ------------------------------------------
//
// If an interface{} is passed, we ***REMOVED***rst do a type assertion to see if it is
...
//
// Some streams support inde***REMOVED***nite-length containers, and use a breaking
...
// a length pre***REMOVED***x, or if it used explicit breaks. If length-pre***REMOVED***xed, we assume that
// it has to be binary, and we do not even try to read separators.
//
// Philosophy
// ------------
// On decode, this codec will update containers appropriately:
//    - If struct, update ***REMOVED***elds from stream into ***REMOVED***elds of struct.
//      If ***REMOVED***eld in stream not found in struct, handle appropriately (based on option).
//      If a struct ***REMOVED***eld has no corresponding value in the stream, leave it AS IS.
...
// int ***REMOVED***eld with current value=5, and the ***REMOVED***eld is omitted in the stream, then after
...
// We decided against it because there is no ef***REMOVED***cient way to do it.
// We may introduce it as an option later.
// However, that will require enabling it for both runtime and code generation modes.
//
// To support truncate, we need to do 2 passes over the container:
//   map
//   - ***REMOVED***rst collect all keys (e.g. in k1)
//   - for each key in stream, mark k1 that the key should not be removed
//   - after updating map, do second pass and call delete for all keys in k1 which are not marked
//   struct:
//   - for each ***REMOVED***eld, track the *typeInfo s1
//   - iterate through all s1, and for each one not marked, set value to zero
//   - this involves checking the possible anonymous ***REMOVED***elds which are nil ptrs.
//     too much work.
//
// ------------------------------------------
...
//   - once it has its err ***REMOVED***eld set, it cannot be used again.
//   - panicing will be optional, controlled by const flag.
//   - code should always check error ***REMOVED***rst and return early.
...
const (
	...
	// if resetSliceElemToZeroValue, then on decoding a slice, reset the element to a zero value ***REMOVED***rst.
...
// s***REMOVED***Idx used for tracking where a (***REMOVED***eld/enc)Name is seen in a []*structFieldInfo
type s***REMOVED***Idx struct {
	name  string
	index int
}
...
// so as not to cause an in***REMOVED***nite loop.
const rgetMaxRecursion = 2

// Anecdotally, we believe most types have <= 12 ***REMOVED***elds.
...
type rgetPoolT struct {
	...
	s***REMOVED***s     [rgetPoolTArrayLen]*structFieldInfo
	s***REMOVED***idx   [rgetPoolTArrayLen]s***REMOVED***Idx
}
...
var (
	...
	noFieldNameToStructFieldInfoErr = errors.New("no ***REMOVED***eld name passed to parseStructFieldInfo")
)

var defTypeInfos = NewTypeInfos([]string{"codec", "json"})

// Selfer de***REMOVED***nes methods by which a value can encode or decode itself.
//
...
// This affords storing a map in a speci***REMOVED***c sequence in the stream.
//
...
type BasicHandle struct {
	...
	//
	// If not con***REMOVED***gured, the default TypeInfos is used, which uses struct tag keys: codec, json
	TypeInfos *TypeInfos
...
// Handle is the interface for a speci***REMOVED***c encoding format.
//
// Typically, a Handle is pre-con***REMOVED***gured before ***REMOVED***rst time use,
// and not modi***REMOVED***ed while in use. Such a pre-con***REMOVED***gured Handle
...
// DEPRECATED: Use SetBytesExt or SetInterfaceExt on the Handle instead.
//
// Note that the type must be a named type, and speci***REMOVED***cally not
...
type structFieldInfo struct {
	...
	***REMOVED***eldName string // ***REMOVED***eld name

	// only one of 'i' or 'is' can be set. If 'i' is -1, then 'is' has been set.

	is        []int // (recursive/embedded) ***REMOVED***eld index in struct
	i         int16 // ***REMOVED***eld index in struct
	omitEmpty bool
	toArray   bool // if ***REMOVED***eld is _struct, is the toArray set?
}
...
// rv returns the ***REMOVED***eld of the struct.
// If anonymous, it returns an Invalid
func (si *structFieldInfo) ***REMOVED***eld(v reflect.Value, update bool) (rv2 reflect.Value) {
	if si.i != -1 {
		v = v.Field(int(si.i))
		return v
	}
...
type s***REMOVED***SortedByEncName []*structFieldInfo

func (p s***REMOVED***SortedByEncName) Len() int {
	return len(p)
}

func (p s***REMOVED***SortedByEncName) Less(i, j int) bool {
	return p[i].encName < p[j].encName
}

func (p s***REMOVED***SortedByEncName) Swap(i, j int) {
	p[i], p[j] = p[j], p[i]
}
...
type typeInfo struct {
	s***REMOVED***  []*structFieldInfo // sorted. Used when enc/dec struct to map.
	s***REMOVED***p []*structFieldInfo // unsorted. Used when enc/dec struct to array.
...
func (ti *typeInfo) indexForEncName(name string) int {
	...
	//tis***REMOVED*** := ti.s***REMOVED***
	const binarySearchThreshold = 16
	if s***REMOVED***len := len(ti.s***REMOVED***); s***REMOVED***len < binarySearchThreshold {
		// linear search. faster than binary search in my testing up to 16-***REMOVED***eld structs.
		for i, si := range ti.s***REMOVED*** {
			if si.encName == name {
				return i
			}
		}
		...
		h, i, j := 0, 0, s***REMOVED***len
		for i < j {
			h = i + (j-i)/2
			if ti.s***REMOVED***[h].encName < name {
				i = h + 1
			} ***REMOVED*** {
				j = h
			}
		}
		if i < s***REMOVED***len && ti.s***REMOVED***[i].encName == name {
			return i
		}
	}
	return -1
...
// TypeInfos caches typeInfo for each type on ***REMOVED***rst inspection.
//
// It is con***REMOVED***gured with a set of tag keys, which are used to get
// con***REMOVED***guration for the type.
...
//
// This allows users customize the struct tag keys which contain con***REMOVED***guration
// of their types.
...
func (x *TypeInfos) structTag(t reflect.StructTag) (s string) {
	...
	// this allows seamless support for many con***REMOVED***gured structs.
...
func (x *TypeInfos) get(rtid uintptr, rt reflect.Type) (pti *typeInfo) {
	...
	if rt.Kind() == reflect.Struct {
		...
		vv := rgetT{pv.fNames[:0], pv.encNames[:0], pv.etypes[:1], pv.s***REMOVED***s[:0]}
		x.rget(rt, rtid, omitEmpty, nil, &vv)
		ti.s***REMOVED***p, ti.s***REMOVED*** = rgetResolveSFI(vv.s***REMOVED***s, pv.s***REMOVED***idx[:0])
		rgetPool.Put(pi)
	}
	// s***REMOVED*** = s***REMOVED***p
...
func (x *TypeInfos) rget(rt reflect.Type, rtid uintptr, omitEmpty bool,
	...
	// Read up ***REMOVED***elds and store how to access the value.
	//
	// It uses go's rules for message selectors,
	// which say that the ***REMOVED***eld with the shallowest depth is selected.
	//
	// Note: we consciously use slices, not a map, to simulate a set.
	//       Typically, types have < 16 ***REMOVED***elds,
...
LOOP:
	for j, jlen := 0, rt.NumField(); j < jlen; j++ {
		...
		if f.Anonymous && fkind != reflect.Interface {
			...
			if doInline {
				...
				if ft.Kind() == reflect.Struct {
					// if etypes contains this, don't call rget again (as ***REMOVED***elds are already seen here)
					ftid := reflect.ValueOf(ft).Pointer()
					// We cannot recurse forever, but we need to track other ***REMOVED***eld depths.
					// So - we break if we see a type twice (not the ***REMOVED***rst time).
					// This should be suf***REMOVED***cient to handle an embedded type that refers to its
		...
		// after the anonymous dance: if an unexported ***REMOVED***eld, skip
		...
		} ***REMOVED*** if si.encName == "" {
			si.encName = f.Name
		}
		si.***REMOVED***eldName = f.Name
		...
		pv.s***REMOVED***s = append(pv.s***REMOVED***s, si)
	}
}

// resolves the struct ***REMOVED***eld info got from a call to rget.
// Returns a trimmed, unsorted and sorted []*structFieldInfo.
func rgetResolveSFI(x []*structFieldInfo, pv []s***REMOVED***Idx) (y, z []*structFieldInfo) {
	var n int
	for i, v := range x {
		xn := v.encName //TODO: ***REMOVED***eldName or encName? use encName for now.
		var found bool
		for j, k := range pv {
			if k.name == xn {
				// one of them must be reset to nil, and the index updated appropriately to the other one
				if len(v.is) == len(x[k.index].is) {
				} ***REMOVED*** if len(v.is) < len(x[k.index].is) {
					pv[j].index = i
					if x[k.index] != nil {
						x[k.index] = nil
						n++
		...
		if !found {
			pv = append(pv, s***REMOVED***Idx{xn, i})
		}
	}
	...
	copy(z, y)
	sort.Sort(s***REMOVED***SortedByEncName(z))
	return
}
