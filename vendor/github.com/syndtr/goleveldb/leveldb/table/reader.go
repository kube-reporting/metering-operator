...
func (b *block) entry(offset int) (key, value []byte, nShared, n int, err error) {
	...
	v0, n0 := binary.Uvarint(b.data[offset:])       // Shared pre***REMOVED***x length
...
type blockIter struct {
	...
	key, value    []byte
	offset        int
	// Previous offset, only ***REMOVED***lled by Next.
	prevOffset   int
	prevNode     []int
	prevKeys     []byte
	restartIndex int
...
func (i *blockIter) Next() bool {
	...
	for i.offset < i.offsetRealStart {
		...
		if err != nil {
			i.sErr(i.tr.***REMOVED***xErrCorruptedBH(i.block.bh, err))
			return false
		}
		if n == 0 {
			i.dir = dirEOI
...
type ***REMOVED***lterBlock struct {
	bpool      *util.BufferPool
	data       []byte
	oOffset    int
	baseLg     uint
	***REMOVED***ltersNum int
}

func (b ****REMOVED***lterBlock) contains(***REMOVED***lter ***REMOVED***lter.Filter, offset uint64, key []byte) bool {
	i := int(offset >> b.baseLg)
	if i < b.***REMOVED***ltersNum {
		o := b.data[b.oOffset+i*4:]
		n := int(binary.LittleEndian.Uint32(o))
		m := int(binary.LittleEndian.Uint32(o[4:]))
		if n < m && m <= b.oOffset {
			return ***REMOVED***lter.Contains(b.data[n:m], key)
		} ***REMOVED*** if n == m {
			return false
		}
	}
...
func (b ****REMOVED***lterBlock) Release() {
	b.bpool.Put(b.data)
	b.bpool = nil
	b.data = nil
}
...
func (i *indexIter) Get() iterator.Iterator {
	...
	return i.tr.getDataIterErr(dataBH, slice, i.tr.verifyChecksum, i.***REMOVED***llCache)
}
...
type Reader struct {
	...
	***REMOVED***lter         ***REMOVED***lter.Filter
	...
	dataEnd                   int64
	metaBH, indexBH, ***REMOVED***lterBH blockHandle
	indexBlock                *block
	***REMOVED***lterBlock               ****REMOVED***lterBlock
}
...
func (r *Reader) blockKind(bh blockHandle) string {
	...
	case r.indexBH.offset:
		return "index-block"
	case r.***REMOVED***lterBH.offset:
		if r.***REMOVED***lterBH.length > 0 {
			return "***REMOVED***lter-block"
		}
	}
	return "data-block"
}
...
func (r *Reader) ***REMOVED***xErrCorruptedBH(bh blockHandle, err error) error {
...
func (r *Reader) readBlockCached(bh blockHandle, verifyChecksum, ***REMOVED***llCache bool) (*block, util.Releaser, error) {
	if r.cache != nil {
		var (
			err error
			ch  *cache.Handle
...
func (r *Reader) readFilterBlock(bh blockHandle) (****REMOVED***lterBlock, error) {
	...
	b := &***REMOVED***lterBlock{
		...
		***REMOVED***ltersNum: (m - oOffset) / 4,
	}
	return b, nil
}

func (r *Reader) readFilterBlockCached(bh blockHandle, ***REMOVED***llCache bool) (****REMOVED***lterBlock, util.Releaser, error) {
	if r.cache != nil {
		var (
			err error
			ch  *cache.Handle
		...
		if ch != nil {
			b, ok := ch.Value().(****REMOVED***lterBlock)
			if !ok {
				ch.Release()
				return nil, nil, errors.New("leveldb/table: inconsistent block type")
			}
...
func (r *Reader) getIndexBlock(***REMOVED***llCache bool) (b *block, rel util.Releaser, err error) {
	if r.indexBlock == nil {
		return r.readBlockCached(r.indexBH, true, ***REMOVED***llCache)
	}
	return r.indexBlock, util.NoopReleaser{}, nil
}

func (r *Reader) getFilterBlock(***REMOVED***llCache bool) (****REMOVED***lterBlock, util.Releaser, error) {
	if r.***REMOVED***lterBlock == nil {
		return r.readFilterBlockCached(r.***REMOVED***lterBH, ***REMOVED***llCache)
	}
	return r.***REMOVED***lterBlock, util.NoopReleaser{}, nil
}
...
func (r *Reader) getDataIter(dataBH blockHandle, slice *util.Range, verifyChecksum, ***REMOVED***llCache bool) iterator.Iterator {
	b, rel, err := r.readBlockCached(dataBH, verifyChecksum, ***REMOVED***llCache)
	if err != nil {
...
func (r *Reader) getDataIterErr(dataBH blockHandle, slice *util.Range, verifyChecksum, ***REMOVED***llCache bool) iterator.Iterator {
	r.mu.RLock()
	defer r.mu.RUnlock()
	...
	return r.getDataIter(dataBH, slice, verifyChecksum, ***REMOVED***llCache)
}
...
func (r *Reader) NewIterator(slice *util.Range, ro *opt.ReadOptions) iterator.Iterator {
	...
	***REMOVED***llCache := !ro.GetDontFillCache()
	indexBlock, rel, err := r.getIndexBlock(***REMOVED***llCache)
	if err != nil {
		return iterator.NewEmptyIterator(err)
	}
	index := &indexIter{
...
func (r *Reader) ***REMOVED***nd(key []byte, ***REMOVED***ltered bool, ro *opt.ReadOptions, noValue bool) (rkey, value []byte, err error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	...
	// The ***REMOVED***lter should only used for exact match.
	if ***REMOVED***ltered && r.***REMOVED***lter != nil {
		***REMOVED***lterBlock, frel, ferr := r.getFilterBlock(true)
		if ferr == nil {
			if !***REMOVED***lterBlock.contains(r.***REMOVED***lter, dataBH.offset, key) {
				frel.Release()
				return nil, nil, ErrNotFound
			}
			frel.Release()
		} ***REMOVED*** if !errors.IsCorrupted(ferr) {
			return nil, nil, ferr
		}
	}
	...
	if !data.Seek(key) {
		...
		// The nearest greater-than key is the ***REMOVED***rst key of the next block.
		if !index.Next() {
			if err = index.Error(); err == nil {
				err = ErrNotFound
			}
...
// Find ***REMOVED***nds key/value pair whose key is greater than or equal to the
// given key. It returns ErrNotFound if the table doesn't contain
// such pair.
// If ***REMOVED***ltered is true then the nearest 'block' will be checked against
// '***REMOVED***lter data' (if present) and will immediately return ErrNotFound if
// '***REMOVED***lter data' indicates that such pair doesn't exist.
//
// The caller may modify the contents of the returned slice as it is its
// own copy.
// It is safe to modify the contents of the argument after Find returns.
func (r *Reader) Find(key []byte, ***REMOVED***ltered bool, ro *opt.ReadOptions) (rkey, value []byte, err error) {
	return r.***REMOVED***nd(key, ***REMOVED***ltered, ro, false)
}

// FindKey ***REMOVED***nds key that is greater than or equal to the given key.
// It returns ErrNotFound if the table doesn't contain such key.
// If ***REMOVED***ltered is true then the nearest 'block' will be checked against
// '***REMOVED***lter data' (if present) and will immediately return ErrNotFound if
// '***REMOVED***lter data' indicates that such key doesn't exist.
//
// The caller may modify the contents of the returned slice as it is its
// own copy.
// It is safe to modify the contents of the argument after Find returns.
func (r *Reader) FindKey(key []byte, ***REMOVED***ltered bool, ro *opt.ReadOptions) (rkey []byte, err error) {
	rkey, _, err = r.***REMOVED***nd(key, ***REMOVED***ltered, ro, true)
	return
}
...
func (r *Reader) Get(key []byte, ro *opt.ReadOptions) (value []byte, err error) {
	...
	rkey, value, err := r.***REMOVED***nd(key, false, ro, false)
...
// It also close the ***REMOVED***le if it is an io.Closer.
func (r *Reader) Release() {
	...
	if r.***REMOVED***lterBlock != nil {
		r.***REMOVED***lterBlock.Release()
		r.***REMOVED***lterBlock = nil
	}
	r.reader = nil
	r.cache = nil
	r.bpool = nil
...
// NewReader creates a new initialized table reader for the ***REMOVED***le.
// The ***REMOVED***, cache and bpool is optional and can be nil.
//
// The returned table reader instance is safe for concurrent use.
func NewReader(f io.ReaderAt, size int64, fd storage.FileDesc, cache *cache.NamespaceGetter, bpool *util.BufferPool, o *opt.Options) (*Reader, error) {
	if f == nil {
		return nil, errors.New("leveldb/table: nil ***REMOVED***le")
	}
	...
	for metaIter.Next() {
		...
		if !strings.HasPre***REMOVED***x(key, "***REMOVED***lter.") {
			continue
		}
		fn := key[7:]
		...
		if r.***REMOVED***lter != nil {
			***REMOVED***lterBH, n := decodeBlockHandle(metaIter.Value())
			if n == 0 {
				continue
			}
			r.***REMOVED***lterBH = ***REMOVED***lterBH
			// Update data end.
			r.dataEnd = int64(***REMOVED***lterBH.offset)
			break
		}
	}
	metaIter.Release()
	...
	// Cache index and ***REMOVED***lter block locally, since we don't have global cache.
	if cache == nil {
		...
		if r.***REMOVED***lter != nil {
			r.***REMOVED***lterBlock, err = r.readFilterBlock(r.***REMOVED***lterBH)
			if err != nil {
				if !errors.IsCorrupted(err) {
					return nil, err
				}

				// Don't use ***REMOVED***lter then.
				r.***REMOVED***lter = nil
			}
		}
	}
