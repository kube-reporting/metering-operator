...
func newTableFile(fd storage.FileDesc, size int64, imin, imax internalKey) *tFile {
	...
	// We arrange to automatically compact this ***REMOVED***le after
...
// Returns true if i ***REMOVED***le number is greater than j.
// This used for sort by ***REMOVED***le number in descending order.
...
// Sorts tables by ***REMOVED***le number in descending order.
...
func (tf tFiles) overlaps(icmp *iComparer, umin, umax []byte, unsorted bool) bool {
	...
	if i >= len(tf) {
		// Beginning of range is after all ***REMOVED***les, so no overlap.
		return false
	}
	...
	for i := 0; i < len(tf); {
		...
		if t.overlaps(icmp, umin, umax) {
			if umin != nil && icmp.uCompare(t.imin.ukey(), umin) < 0 {
				...
				i = 0
				continue
			} ***REMOVED*** if umax != nil && icmp.uCompare(t.imax.ukey(), umax) > 0 {
				umax = t.imax.ukey()
...
// given key.
func (t *tOps) ***REMOVED***nd(f *tFile, key []byte, ro *opt.ReadOptions) (rkey, rvalue []byte, err error) {
	ch, err := t.open(f)
	if err != nil {
		return nil, nil, err
	}
...
func (t *tOps) ***REMOVED***ndKey(f *tFile, key []byte, ro *opt.ReadOptions) (rkey []byte, err error) {
	ch, err := t.open(f)
	if err != nil {
		return nil, err
	}
...
// tWriter wraps the table writer. It keep track of ***REMOVED***le descriptor
// and added key range.
type tWriter struct {
	t *tOps
...
// Finalizes the table and returns table ***REMOVED***le.
func (w *tWriter) ***REMOVED***nish() (f *tFile, err error) {
	defer w.close()
	err = w.tw.Close()
	if err != nil {
		return
	...
	f = newTableFile(w.fd, int64(w.tw.BytesLen()), internalKey(w.***REMOVED***rst), internalKey(w.last))
	return
}
