...
// reason about semantics of a swagger speci***REMOVED***cation for use in code generation
// or validation etc.
...
func (s *Spec) initialize() {
	...
	for name, schema := range s.spec.De***REMOVED***nitions {
		s.analyzeSchema(name, schema, "/de***REMOVED***nitions")
	}
	// TODO: after analyzing all things and flattening schemas etc
	// resolve all the collected references to their ***REMOVED***nal representations
...
func (s *Spec) analyzeItems(name string, items *spec.Items, pre***REMOVED***x string) {
	if items == nil {
		return
	}
	refPref := slashpath.Join(pre***REMOVED***x, name)
...
func (s *Spec) analyzeOperation(method, path string, op *spec.Operation) {
	...
	pre***REMOVED***x := slashpath.Join("/paths", jsonpointer.Escape(path), strings.ToLower(method))
	for i, param := range op.Parameters {
		refPref := slashpath.Join(pre***REMOVED***x, "parameters", strconv.Itoa(i))
		if param.Ref.String() != "" {
			s.references.addParamRef(refPref, &param)
		}
		s.analyzeItems("items", param.Items, refPref)
	...
	if op.Responses != nil {
		if op.Responses.Default != nil {
			refPref := slashpath.Join(pre***REMOVED***x, "responses", "default")
		...
		for k, res := range op.Responses.StatusCodeResponses {
			refPref := slashpath.Join(pre***REMOVED***x, "responses", strconv.Itoa(k))
			if res.Ref.String() != "" {
...
func (s *Spec) analyzeSchema(name string, schema spec.Schema, pre***REMOVED***x string) {
	refURI := slashpath.Join(pre***REMOVED***x, jsonpointer.Escape(name))
	schRef := SchemaRef{
		Name:   name,
		Schema: &schema,
	...
	for k, v := range schema.De***REMOVED***nitions {
		s.analyzeSchema(k, v, slashpath.Join(refURI, "de***REMOVED***nitions"))
	}
...
// SecurityDe***REMOVED***nitionsFor gets the matching security de***REMOVED***nitions for a set of requirements
func (s *Spec) SecurityDe***REMOVED***nitionsFor(operation *spec.Operation) map[string]spec.SecurityScheme {
	...
	for _, v := range requirements {
		if de***REMOVED***nition, ok := s.spec.SecurityDe***REMOVED***nitions[v.Name]; ok {
			if de***REMOVED***nition != nil {
				result[v.Name] = *de***REMOVED***nition
			}
		}
	}
	return result
...
func mapKeyFromParam(param *spec.Parameter) string {
	return fmt.Sprintf("%s#%s", param.In, ***REMOVED***eldNameFromParam(param))
}

func ***REMOVED***eldNameFromParam(param *spec.Parameter) string {
...
// ParametersFor the speci***REMOVED***ed operation id
...
// ParamsFor the speci***REMOVED***ed method and path. Aggregates them with the defaults etc, so it's all the params that
...
// Operations gathers all the operations speci***REMOVED***ed in the spec document
...
// RequiredConsumes gets all the distinct consumes that are speci***REMOVED***ed in the speci***REMOVED***cation document
func (s *Spec) RequiredConsumes() []string {
	return s.structMapKeys(s.consumes)
}

// RequiredProduces gets all the distinct produces that are speci***REMOVED***ed in the speci***REMOVED***cation document
func (s *Spec) RequiredProduces() []string {
	return s.structMapKeys(s.produces)
}

// RequiredSecuritySchemes gets all the distinct security schemes that are speci***REMOVED***ed in the swagger spec
...
// SchemasWithAllOf returns schema references to all schemas that are de***REMOVED***ned
// with an allOf key
...
// AllDe***REMOVED***nitions returns schema references for all the de***REMOVED***nitions that were discovered
func (s *Spec) AllDe***REMOVED***nitions() (result []SchemaRef) {
...
// AllDe***REMOVED***nitionReferences returns json refs for all the discovered schemas
func (s *Spec) AllDe***REMOVED***nitionReferences() (result []string) {
