...
var (
	...
	// However, it can be modi***REMOVED***ed to avoid periodic resync to break the
	// TCP connection.
...
func NewNamedReflector(name string, lw ListerWatcher, expectedType interface{}, store Store, resyncPeriod time.Duration) *Reflector {
	...
	r := &Reflector{
		...
		metrics:       newReflectorMetrics(makeValidPromethusMetricLabel(fmt.Sprintf("reflector_"+name+"_%d", reflectorSuf***REMOVED***x))),
		listerWatcher: lw,
		store:         store,
...
// getDefaultReflectorName walks back through the call stack until we ***REMOVED***nd a caller from outside of the ignoredPackages
...
func getDefaultReflectorName(ignoredPackages ...string) string {
	...
	for i := 1; i < maxStack; i++ {
		...
		if hasPackage(***REMOVED***le, ignoredPackages) {
			continue
		}
...
func trimPackagePre***REMOVED***x(***REMOVED***le string) string {
	if l := strings.LastIndex(***REMOVED***le, "k8s.io/client-go/pkg/"); l >= 0 {
		return ***REMOVED***le[l+len("k8s.io/client-go/"):]
	}
