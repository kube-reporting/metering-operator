...
func (p *sessionRecord) readUvarintMayEOF(***REMOVED***eld string, r io.ByteReader, mayEOF bool) uint64 {
	if p.err != nil {
		return 0
	}
	...
	if err != nil {
		if err == io.ErrUnexpectedEOF || (mayEOF == false && err == io.EOF) {
			p.err = errors.NewErrCorrupted(storage.FileDesc{}, &ErrManifestCorrupted{***REMOVED***eld, "short read"})
		} ***REMOVED*** if strings.HasPre***REMOVED***x(err.Error(), "binary:") {
			p.err = errors.NewErrCorrupted(storage.FileDesc{}, &ErrManifestCorrupted{***REMOVED***eld, err.Error()})
		} ***REMOVED*** {
			p.err = err
		}
		return 0
...
func (p *sessionRecord) readUvarint(***REMOVED***eld string, r io.ByteReader) uint64 {
	return p.readUvarintMayEOF(***REMOVED***eld, r, false)
}

func (p *sessionRecord) readVarint(***REMOVED***eld string, r io.ByteReader) int64 {
	x := int64(p.readUvarintMayEOF(***REMOVED***eld, r, false))
	if x < 0 {
		p.err = errors.NewErrCorrupted(storage.FileDesc{}, &ErrManifestCorrupted{***REMOVED***eld, "invalid negative value"})
	}
	return x
}

func (p *sessionRecord) readBytes(***REMOVED***eld string, r byteReader) []byte {
	if p.err != nil {
		return nil
	}
	n := p.readUvarint(***REMOVED***eld, r)
	if p.err != nil {
		return nil
	}
	x := make([]byte, n)
...
func (p *sessionRecord) readLevel(***REMOVED***eld string, r io.ByteReader) int {
	if p.err != nil {
		return 0
	}
	x := p.readUvarint(***REMOVED***eld, r)
	if p.err != nil {
		return 0
	}
	return int(x)
...
func (p *sessionRecord) decode(r io.Reader) error {
	br, ok := r.(byteReader)
	if !ok {
		br = bu***REMOVED***o.NewReader(r)
	}
	p.err = nil
	for p.err == nil {
		rec := p.readUvarintMayEOF("***REMOVED***eld-header", br, true)
		if p.err != nil {
			if p.err == io.EOF {
				return nil
			}
		...
		case recNextFileNum:
			x := p.readVarint("next-***REMOVED***le-num", br)
			if p.err == nil {
				p.setNextFileNum(x)
			}
		case recSeqNum:
