...
type memStorage struct {
	...
	***REMOVED***les map[uint64]*memFile
	meta  FileDesc
}
...
func NewMemStorage() Storage {
	return &memStorage{
		***REMOVED***les: make(map[uint64]*memFile),
	}
}
...
func (ms *memStorage) Open(fd FileDesc) (Reader, error) {
	...
	ms.mu.Lock()
	defer ms.mu.Unlock()
	if m, exist := ms.***REMOVED***les[packFile(fd)]; exist {
		if m.open {
...
func (ms *memStorage) Remove(fd FileDesc) error {
	...
	ms.mu.Lock()
	defer ms.mu.Unlock()
	if _, exist := ms.***REMOVED***les[x]; exist {
		delete(ms.***REMOVED***les, x)
		return nil
	}
...
func (ms *memStorage) Rename(oldfd, newfd FileDesc) error {
	...
	ms.mu.Lock()
	defer ms.mu.Unlock()
	oldm, exist := ms.***REMOVED***les[oldx]
	if !exist {
		return os.ErrNotExist
	}
	newm, exist := ms.***REMOVED***les[newx]
	if (exist && newm.open) || oldm.open {
		return errFileOpen
	}
	delete(ms.***REMOVED***les, oldx)
	ms.***REMOVED***les[newx] = oldm
	return nil
}
