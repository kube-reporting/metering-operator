...
TMP_DIR="$(mktemp -d)"
# We set these in here so that when we run integration.sh, which runs e2e-test-runner.sh, they're set to the same files for both the deploy-ci.sh script, and the
