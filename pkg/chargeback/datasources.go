...
func (c *Chargeback) handlePromsumDataSource(logger log.FieldLogger, dataSource *cbTypes.ReportDataSource) error {
	...
	if storageSpec.Hive != nil {
		...
		tableParams, tableProperties, err = hive.ExecuteCreateS3Table(c.hiveQueryer, tableName, storageSpec.S3.Bucket, storageSpec.S3.Prefix, promsumHiveColumns, false)
		if err != nil {
			return err
		}
	} else {
...
func (c *Chargeback) handleAWSBillingDataSource(logger log.FieldLogger, dataSource *cbTypes.ReportDataSource) error {
	...
	tableParams, tableProperties, err := CreateAWSUsageTable(c.hiveQueryer, tableName, source.Bucket, source.Prefix, manifests)
	if err != nil {
		return err
	}
