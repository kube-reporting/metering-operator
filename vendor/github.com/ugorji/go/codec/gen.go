...
// Use of this source code is governed by a MIT license found in the LICENSE ***REMOVED***le.
...
//    - Extensions, only if Extensions are con***REMOVED***gured.
//
...
//
// The following ***REMOVED***eld types are supported:
//     array: [n]T
//     slice: []T
//     map: map[K]V
...
//
// In addition, code in this ***REMOVED***le manages the generation of fast-path implementations of
...
// This allows us support ANY value, including any named types, speci***REMOVED***cally those which
...
//
// codecgen will panic if the ***REMOVED***le was generated with an old version of the library in use.
//
// Note:
...
const (
	...
	// ***REMOVED*** make codecDecodeSelferFromMap{LenPre***REMOVED***x,CheckBreak} so that conditionals
...
const (
	...
	genStructMapStyleLenPre***REMOVED***x
...
type genRunner struct {
	...
	cpfx   string // codec package pre***REMOVED***x
	unsafe bool   // is unsafe to be used in generated code?

	tm map[reflect.Type]struct{} // types for which enc/dec must be generated
	ts []reflect.Type            // types for which enc/dec must be generated

	xs string // top level variable/constant suf***REMOVED***x
	hn string // fn helper type name

	ti *TypeInfos
	// rr *rand.Rand // random generator for ***REMOVED***le-speci***REMOVED***c types
}

// Gen will write a complete go ***REMOVED***le containing Selfer implementations for each
// type passed. All the types must be in the same package.
//
// Library users: *DO NOT USE IT DIRECTLY. IT WILL CHANGE CONTINOUSLY WITHOUT NOTICE.*
func Gen(w io.Writer, buildTags, pkgName, uid string, useUnsafe bool, ti *TypeInfos, typ ...reflect.Type) {
	// All types passed to this method do not have a codec.Selfer method implemented directly.
	// codecgen already checks the AST and skips any types that de***REMOVED***ne the codec.Selfer methods.
...
`)
	...
	x.line("_, ***REMOVED***le, _, _ := runtime.Caller(0)")
	x.line(`err := fmt.Errorf("codecgen version mismatch: current: %v, need %v. Re-generate ***REMOVED***le: %v", `)
	x.linef(`%v, %sGenVersion, ***REMOVED***le)`, GenVersion, x.cpfx)
	x.line("panic(err)")
	x.linef("}")
	...
	if tpkg != "" && tpkg != x.bp && tpkg != x.cp && tname != "" && tname[0] >= 'A' && tname[0] <= 'Z' {
		if _, ok := x.im[tpkg]; !ok {
			...
			if idx := strings.LastIndex(tpkg, "/"); idx < 0 {
				...
				x.imc++
				x.imn[tpkg] = "pkg" + strconv.FormatUint(x.imc, 10) + "_" + genGoIdenti***REMOVED***er(tpkg[idx+1:], false)
			}
		}
	}
	switch t.Kind() {
...
func (x *genRunner) genTypeNamePrim(t reflect.Type) (n string) {
	if t.Name() == "" {
		return t.String()
	} ***REMOVED*** if genImportPath(t) == "" || genImportPath(t) == genImportPath(x.tc) {
		return t.Name()
	} ***REMOVED*** {
...
func (x *genRunner) selfer(encode bool) {
	...
	if genUseOneFunctionForDecStructMap {
		...
		x.out(fnSigPfx)
		x.line(") codecDecodeSelfFromMapLenPre***REMOVED***x(l int, d *" + x.cpfx + "Decoder) {")
		x.genRequiredMethodVars(false)
		x.decStructMap(genTopLevelVarName, "l", reflect.ValueOf(t0).Pointer(), t0, genStructMapStyleLenPre***REMOVED***x)
		x.line("}")
		x.line("")
...
func (x *genRunner) encVar(varname string, t reflect.Type) {
	...
	if checkNil {
		x.linef("if %s == nil { r.EncodeNil() } ***REMOVED*** { ", varname)
	}
	switch t.Kind() {
	case reflect.Ptr:
		switch t.Elem().Kind() {
...
func (x *genRunner) enc(varname string, t reflect.Type) {
	...
	if x.checkForSelfer(t, varname) {
		...
		} ***REMOVED*** { // varname is of type T
			if t.Implements(selferTyp) {
				x.line(varname + ".CodecEncodeSelf(e)")
				return
			} ***REMOVED*** if tptr.Implements(selferTyp) {
	...
	if t == rawTyp {
		x.linef("} ***REMOVED*** { z.EncRaw(%v)", varname)
		return
	}
	if t == rawExtTyp {
		x.linef("} ***REMOVED*** { r.EncodeRawExt(%v, e)", varname)
		return
	}
	...
	if t == timeTyp {
		...
		x.linef("} ***REMOVED*** if %s := z.TimeRtidIfBinc(); %s != 0 { ", vrtid, vrtid)
		x.linef("r.EncodeBuiltin(%s, %s)", vrtid, varname)
	}
	// only check for extensions if the type is named, and has a packagePath.
	if genImportPath(t) != "" && t.Name() != "" {
		// ***REMOVED***rst check if extensions are con***REMOVED***gued, before doing the interface conversion
		x.linef("} ***REMOVED*** if z.HasExtensions() && z.EncExt(%s) {", varname)
	}
	if tk == reflect.Array || tk == reflect.Struct { // varname is of type *T
		if t.Implements(binaryMarshalerTyp) || tptr.Implements(binaryMarshalerTyp) {
			x.linef("} ***REMOVED*** if %sm%s { z.EncBinaryMarshal(%v) ", genTempVarPfx, mi, varname)
		}
		if t.Implements(jsonMarshalerTyp) || tptr.Implements(jsonMarshalerTyp) {
			x.linef("} ***REMOVED*** if !%sm%s && z.IsJSONHandle() { z.EncJSONMarshal(%v) ", genTempVarPfx, mi, varname)
		} ***REMOVED*** if t.Implements(textMarshalerTyp) || tptr.Implements(textMarshalerTyp) {
			x.linef("} ***REMOVED*** if !%sm%s { z.EncTextMarshal(%v) ", genTempVarPfx, mi, varname)
		}
	} ***REMOVED*** { // varname is of type T
		if t.Implements(binaryMarshalerTyp) {
			x.linef("} ***REMOVED*** if %sm%s { z.EncBinaryMarshal(%v) ", genTempVarPfx, mi, varname)
		} ***REMOVED*** if tptr.Implements(binaryMarshalerTyp) {
			x.linef("} ***REMOVED*** if %sm%s { z.EncBinaryMarshal(&%v) ", genTempVarPfx, mi, varname)
		}
		if t.Implements(jsonMarshalerTyp) {
			x.linef("} ***REMOVED*** if !%sm%s && z.IsJSONHandle() { z.EncJSONMarshal(%v) ", genTempVarPfx, mi, varname)
		} ***REMOVED*** if tptr.Implements(jsonMarshalerTyp) {
			x.linef("} ***REMOVED*** if !%sm%s && z.IsJSONHandle() { z.EncJSONMarshal(&%v) ", genTempVarPfx, mi, varname)
		} ***REMOVED*** if t.Implements(textMarshalerTyp) {
			x.linef("} ***REMOVED*** if !%sm%s { z.EncTextMarshal(%v) ", genTempVarPfx, mi, varname)
		} ***REMOVED*** if tptr.Implements(textMarshalerTyp) {
			x.linef("} ***REMOVED*** if !%sm%s { z.EncTextMarshal(&%v) ", genTempVarPfx, mi, varname)
		}
	}
	x.line("} ***REMOVED*** {")
	...
	case reflect.Slice:
		...
		// ***REMOVED*** write encode function in-line.
		// - if elements are primitives or Selfers, call dedicated function on each member.
		// - ***REMOVED*** call Encoder.encode(XXX) on it.
		if rtid == uint8SliceTypId {
			x.line("r.EncodeStringBytes(codecSelferC_RAW" + x.xs + ", []byte(" + varname + "))")
		} ***REMOVED*** if fastpathAV.index(rtid) != -1 {
			g := x.newGenV(t)
...
func (x *genRunner) encStruct(varname string, rtid uintptr, t reflect.Type) {
	// Use knowledge from struct***REMOVED***eldinfo (mbs, encodable ***REMOVED***elds. Ignore omitempty. )
	// replicate code in kStruct i.e. for each ***REMOVED***eld, deref type to non-pointer, and call x.enc on it

	// if t === type currently running selfer on, do for all
	ti := x.ti.get(rtid, t)
	i := x.varsfx()
	sepVarname := genTempVarPfx + "sep" + i
	num***REMOVED***eldsvar := genTempVarPfx + "q" + i
	ti2arrayvar := genTempVarPfx + "r" + i
	struct2arrvar := genTempVarPfx + "2arr" + i

	x.line(sepVarname + " := !z.EncBinary()")
	x.linef("%s := z.EncBasicHandle().StructToArray", struct2arrvar)
	tis***REMOVED*** := ti.s***REMOVED***p // always use sequence from ***REMOVED***le. decStruct expects same thing.
	// due to omitEmpty, we need to calculate the
	// number of non-empty things we write out ***REMOVED***rst.
	// This is required as we need to pre-determine the size of the container,
	// to support length-pre***REMOVED***xing.
	x.linef("var %s [%v]bool", num***REMOVED***eldsvar, len(tis***REMOVED***))
	x.linef("_, _, _ = %s, %s, %s", sepVarname, num***REMOVED***eldsvar, struct2arrvar)
	x.linef("const %s bool = %v", ti2arrayvar, ti.toArray)
	nn := 0
	for j, si := range tis***REMOVED*** {
		if !si.omitEmpty {
			nn++
			continue
		}
		...
		x.linef("%s[%v] = %s", num***REMOVED***eldsvar, j, omitline)
	}
	x.linef("var %snn%s int", genTempVarPfx, i)
	x.linef("if %s || %s {", ti2arrayvar, struct2arrvar) // if ti.toArray {
	x.line("r.EncodeArrayStart(" + strconv.FormatInt(int64(len(tis***REMOVED***)), 10) + ")")
	x.linef("} ***REMOVED*** {") // if not ti.toArray
	x.linef("%snn%s = %v", genTempVarPfx, i, nn)
	x.linef("for _, b := range %s { if b { %snn%s++ } }", num***REMOVED***eldsvar, genTempVarPfx, i)
	x.linef("r.EncodeMapStart(%snn%s)", genTempVarPfx, i)
	x.linef("%snn%s = %v", genTempVarPfx, i, 0)
	// x.line("r.EncodeMapStart(" + strconv.FormatInt(int64(len(tis***REMOVED***)), 10) + ")")
	...
	for j, si := range tis***REMOVED*** {
		...
		// if the type of the ***REMOVED***eld is a Selfer, or one of the ones

		x.linef("if %s || %s {", ti2arrayvar, struct2arrvar) // if ti.toArray
		if labelUsed {
			x.line("if " + isNilVarName + " { r.EncodeNil() } ***REMOVED*** { ")
		}
		x.linef("z.EncSendContainerState(codecSelfer_containerArrayElem%s)", x.xs)
		if si.omitEmpty {
			x.linef("if %s[%v] {", num***REMOVED***eldsvar, j)
		}
		...
		x.linef("} ") // end if/***REMOVED*** ti.toArray
	}
	x.linef("if %s || %s {", ti2arrayvar, struct2arrvar) // if ti.toArray {
...
func (x *genRunner) decVar(varname string, t reflect.Type, canBeNil bool) {
	...
	if canBeNil {
		...
		if t.Kind() == reflect.Ptr {
			...
			// if varname is a ***REMOVED***eld of a struct (has a dot in it),
...
func (x *genRunner) dec(varname string, t reflect.Type) {
	...
	if t == rawTyp {
		x.linef("} ***REMOVED*** { *%v = z.DecRaw()", varname)
		return
	}
	if t == rawExtTyp {
		x.linef("} ***REMOVED*** { r.DecodeExt(%v, 0, nil)", varname)
		return
	}
	...
	if genImportPath(t) != "" && t.Name() != "" {
		// ***REMOVED***rst check if extensions are con***REMOVED***gued, before doing the interface conversion
		x.linef("} ***REMOVED*** if z.HasExtensions() && z.DecExt(%s) {", varname)
	}

	if t.Implements(binaryUnmarshalerTyp) || tptr.Implements(binaryUnmarshalerTyp) {
		x.linef("} ***REMOVED*** if %sm%s { z.DecBinaryUnmarshal(%v) ", genTempVarPfx, mi, varname)
	}
	if t.Implements(jsonUnmarshalerTyp) || tptr.Implements(jsonUnmarshalerTyp) {
		x.linef("} ***REMOVED*** if !%sm%s && z.IsJSONHandle() { z.DecJSONUnmarshal(%v)", genTempVarPfx, mi, varname)
	} ***REMOVED*** if t.Implements(textUnmarshalerTyp) || tptr.Implements(textUnmarshalerTyp) {
		x.linef("} ***REMOVED*** if !%sm%s { z.DecTextUnmarshal(%v)", genTempVarPfx, mi, varname)
	}
...
func (x *genRunner) decStructMapSwitch(kName string, varname string, rtid uintptr, t reflect.Type) {
	...
	tis***REMOVED*** := ti.s***REMOVED***p // always use sequence from ***REMOVED***le. decStruct expects same thing.
	x.line("switch (" + kName + ") {")
	for _, si := range tis***REMOVED*** {
		x.line("case \"" + si.encName + "\":")
		var t2 reflect.StructField
		if si.i != -1 {
			t2 = t.Field(int(si.i))
		} ***REMOVED*** {
			//we must accommodate anonymous ***REMOVED***elds, where the embedded ***REMOVED***eld is a nil pointer in the value.
			// t2 = t.FieldByIndex(si.is)
			t2typ := t
			varname3 := varname
			for _, ix := range si.is {
...
func (x *genRunner) decStructMap(varname, lenvarname string, rtid uintptr, t reflect.Type, style genStructMapStyle) {
	...
	// use the scratch buffer to avoid allocation (most ***REMOVED***eld names are < 32).

	x.line("var " + kName + "Slc = z.DecScratchBuffer() // default slice to decode into")

	x.line("_ = " + kName + "Slc")
	switch style {
	case genStructMapStyleLenPre***REMOVED***x:
	...
	default: // 0, otherwise.
		...
		x.line("} ***REMOVED*** { if r.CheckBreak() { break }; }")
	}
...
func (x *genRunner) decStructArray(varname, lenvarname, breakString string, rtid uintptr, t reflect.Type) {
	...
	for _, si := range tis***REMOVED*** {
		...
		x.linef("%sj%s++; if %shl%s { %sb%s = %sj%s > %s } ***REMOVED*** { %sb%s = r.CheckBreak() }",
...
func (x *genRunner) decStruct(varname string, rtid uintptr, t reflect.Type) {
	...
	if genUseOneFunctionForDecStructMap {
		...
		x.line("} ***REMOVED*** if " + genTempVarPfx + "l" + i + " > 0 { ")
		x.line("x.codecDecodeSelfFromMapLenPre***REMOVED***x(" + genTempVarPfx + "l" + i + ", d)")
		x.line("} ***REMOVED*** {")
		x.line("x.codecDecodeSelfFromMapCheckBreak(" + genTempVarPfx + "l" + i + ", d)")
	}
	x.line("}")

	// ***REMOVED*** if container is array
	x.linef("} ***REMOVED*** if %sct%s == codecSelferValueTypeArray%s {", genTempVarPfx, i, x.xs)
...
func (x *genV) MethodNamePfx(pre***REMOVED***x string, prim bool) string {
	var name []byte
	if pre***REMOVED***x != "" {
		name = append(name, pre***REMOVED***x...)
	}
	if prim {
...
// where PkgPath returns the full path, including the vendoring pre-***REMOVED***x that should have been stripped.
// We strip it here.
func genImportPath(t reflect.Type) (s string) {
	...
	if genCheckVendor {
		...
		} ***REMOVED*** if strings.HasPre***REMOVED***x(s, vendorStart) {
			s = s[len(vendorStart):]
		}
	}
	return
...
// A go identi***REMOVED***er is (letter|_)[letter|number|_]*
func genGoIdenti***REMOVED***er(s string, checkFirstChar bool) string {
	b := make([]byte, 0, len(s))
	t := make([]byte, 4)
	var n int
	for i, r := range s {
...
func genInternalSortType(s string, elem bool) string {
	for _, v := range [...]string{"int", "uint", "float", "bool", "string"} {
		if strings.HasPre***REMOVED***x(s, v) {
			if elem {
...
func genInternalInit() {
	...
	// keep as slice, so it is in speci***REMOVED***c iteration order.
	...
	funcs := make(template.FuncMap)
	// funcs["haspfx"] = strings.HasPre***REMOVED***x
	funcs["encmd"] = genInternalEncCommandAsString
	funcs["decmd"] = genInternalDecCommandAsString
	funcs["zerocmd"] = genInternalZeroValue
	funcs["haspre***REMOVED***x"] = strings.HasPre***REMOVED***x
...
// genInternalGoFile is used to generate source ***REMOVED***les from templates.
