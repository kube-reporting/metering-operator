...
func (r *schemaLoader) resolveRef(currentRef, ref *Ref, node, target interface{}) error {
	...
	if strings.HasPre***REMOVED***x(refURL.String(), "#") {
	...
	if refURL.Scheme != "" && refURL.Host != "" {
		// most de***REMOVED***nitely take the red pill
...
func ExpandSpec(spec *Swagger) error {
	...
	for key, de***REMOVED***ntition := range spec.De***REMOVED***nitions {
		var def *Schema
		var err error
		if def, err = expandSchema(de***REMOVED***ntition, []string{"#/de***REMOVED***nitions/" + key}, resolver); err != nil {
			return err
		}
...
func expandSchema(target Schema, parentRefs []string, resolver *schemaLoader) (schema *Schema, err error) {
	...
	for k, _ := range target.De***REMOVED***nitions {
		if t, err = expandSchema(target.De***REMOVED***nitions[k], parentRefs, resolver); err != nil {
			return
		}
