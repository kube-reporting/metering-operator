...
// Package runtime de***REMOVED***nes conversions between generic types and structs to map query strings
// to struct objects.
package runtime
...
// A cluster scoped resource specifying namespace empty works ***REMOVED***ne and specifying a particular
...
func DefaultMetaV1FieldSelectorConversion(label, value string) (string, string, error) {
	...
	case "metadata.namespace":
		return label, value, nil
	default:
		return "", "", fmt.Errorf("%q is not a known ***REMOVED***eld selector: only %q, %q", label, "metadata.name", "metadata.namespace")
	}
}
