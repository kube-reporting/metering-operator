...
func HTTPWrappersForCon***REMOVED***g(con***REMOVED***g *Con***REMOVED***g, rt http.RoundTripper) (http.RoundTripper, error) {
	...
	case con***REMOVED***g.HasTokenAuth():
		rt = NewBearerAuthRoundTripper(con***REMOVED***g.BearerToken, rt)
	...
	if len(con***REMOVED***g.CacheDir) > 0 {
		rt = NewCacheRoundTripper(con***REMOVED***g.CacheDir, rt)
	}
	return rt, nil
}
...
func NewCacheRoundTripper(cacheDir string, rt http.RoundTripper) http.RoundTripper {
	d := diskv.New(diskv.Options{
		BasePath: cacheDir,
		TempDir:  ***REMOVED***lepath.Join(cacheDir, ".diskv-temp"),
	})
...
func (rt *impersonatingRoundTripper) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	for k, vv := range rt.impersonate.Extra {
		for _, v := range vv {
			req.Header.Add(ImpersonateUserExtraHeaderPre***REMOVED***x+k, v)
		}
	}
