...
func (*Driver) Open(connString string) (driver.Conn, error) {
	...
	con***REMOVED***g := parseCon***REMOVED***gFromQuery(info.Query())
	switch con***REMOVED***g.auth {
	case "sasl":
	...
	return &Connection{client: service, session: session.SessionHandle, con***REMOVED***g: con***REMOVED***g}, nil
}
