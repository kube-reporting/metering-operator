...
var (
	inputVersions = flag.StringSlice("input", []string{
		...
		"settings/",
		"networking/",
	}, "group/versions that client-gen will generate clients for. At most one version per group is allowed. Speci***REMOVED***ed in the format \"group1/version1,group2/version2...\".")
...
func versionToPath(gvPath string, group string, version string) (path string) {
	...
	if group == "api" {
		path = ***REMOVED***lepath.Join(*basePath, "../api", version)
	} ***REMOVED*** {
		path = ***REMOVED***lepath.Join(*basePath, gvPath, group, version)
	}
	return
}
