...
type LookupPatchMeta interface {
	...
	// Get the type name of the ***REMOVED***eld
	Name() string
}
...
func (s PatchMetaFromStruct) LookupPatchMetadataForStruct(key string) (LookupPatchMeta, PatchMeta, error) {
	***REMOVED***eldType, ***REMOVED***eldPatchStrategies, ***REMOVED***eldPatchMergeKey, err := forkedjson.LookupPatchMetadataForStruct(s.T, key)
	if err != nil {
		return nil, PatchMeta{}, err
	}

	return PatchMetaFromStruct{T: ***REMOVED***eldType},
		PatchMeta{
			patchStrategies: ***REMOVED***eldPatchStrategies,
			patchMergeKey:   ***REMOVED***eldPatchMergeKey,
		}, nil
}
