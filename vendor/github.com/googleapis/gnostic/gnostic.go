...
func getOpenAPIVersionFromInfo(info interface{}) int {
	...
	if ok && strings.HasPre***REMOVED***x(swagger, "2.0") {
		return openAPIv2
	}
	openapi, ok := compiler.MapValueForKey(m, "openapi").(string)
	if ok && strings.HasPre***REMOVED***x(openapi, "3.0") {
		return openAPIv3
	}
	return openAPIvUnknown
}
...
const (
	pluginPre***REMOVED***x    = "gnostic-"
	extensionPre***REMOVED***x = "gnostic-x-"
)
...
func (p *pluginCall) perform(document proto.Message, openAPIVersion int, sourceName string) error {
	if p.Name != "" {
		...
		executableName := pluginPre***REMOVED***x + p.Name
		...
		default: // write ***REMOVED***les into a directory named by outputLocation
...
// Write bytes to a named ***REMOVED***le.
// Certain names have special meaning:
//   ! writes nothing
//   - writes to stdout
//   = writes to stderr
// If a directory name is given, the ***REMOVED***le is written there with
...
func newGnostic() *Gnostic {
	g := &Gnostic{}
	// Option ***REMOVED***elds initialize to their default values.
	g.usage = `
Usage: gnostic OPENAPI_SOURCE [OPTIONS]
  OPENAPI_SOURCE is the ***REMOVED***lename or URL of an OpenAPI description to read.
Options:
  --pb-out=PATH       Write a binary proto to the speci***REMOVED***ed location.
  --text-out=PATH     Write a text proto to the speci***REMOVED***ed location.
  --json-out=PATH     Write a json API description to the speci***REMOVED***ed location.
  --yaml-out=PATH     Write a yaml API description to the speci***REMOVED***ed location.
  --errors-out=PATH   Write compilation errors to the speci***REMOVED***ed location.
  ...
  --x-EXTENSION       Use the extension named gnostic-x-EXTENSION
                      to process OpenAPI speci***REMOVED***cation extensions.
...
func (g *Gnostic) readOptions() {
	...
	for i, arg := range os.Args {
		...
		} ***REMOVED*** if m = extensionRegex.FindSubmatch([]byte(arg)); m != nil {
			extensionName := string(m[1])
			extensionHandler := compiler.ExtensionHandler{Name: extensionPre***REMOVED***x + extensionName}
			g.extensionHandlers = append(g.extensionHandlers, extensionHandler)
		} ***REMOVED*** if arg == "--resolve-refs" {
...
// Generate an error message to be written to stderr or a ***REMOVED***le.
...
func (g *Gnostic) readOpenAPIText(bytes []byte) (message proto.Message, err error) {
	...
	if g.openAPIVersion == openAPIv2 {
		...
		message = document
	} ***REMOVED*** if g.openAPIVersion == openAPIv3 {
...
// Read an OpenAPI binary ***REMOVED***le.
func (g *Gnostic) readOpenAPIBinary(data []byte) (message proto.Message, err error) {
	...
	if err == nil && strings.HasPre***REMOVED***x(documentV3.Openapi, "3.0") {
	...
	if err == nil && strings.HasPre***REMOVED***x(documentV2.Swagger, "2.0") {
...
// Perform all actions speci***REMOVED***ed in the command-line options.
func (g *Gnostic) performActions(message proto.Message) (err error) {
	...
	// Call all speci***REMOVED***ed plugins.
...
func (g *Gnostic) main() {
	...
	extension := strings.ToLower(***REMOVED***lepath.Ext(g.sourceName))
	...
	if extension == ".json" || extension == ".yaml" {
		...
		err = errors.New("unknown ***REMOVED***le extension. 'json', 'yaml', and 'pb' are accepted")
		writeFile(g.errorOutputPath, g.errorBytes(err), g.sourceName, "errors")
		os.Exit(-1)
	}
	// Perform actions speci***REMOVED***ed by command options.
