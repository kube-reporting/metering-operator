...
reporting-operator-docker-build: images/reporting-operator/Dockerfile $(REPORTING_OP_BIN_OUT)
...
build-reporting-operator:
	@:$(call check_defined, REPORTING_OPERATOR_BIN_LOCATION, Path to output binary location)
	$(MAKE) update-codegen
	mkdir -p $(dir $@)
