...
//
// Initializing language- or locale-speci***REMOVED***c components usually consists of
// two steps. The ***REMOVED***rst step is to select a display language based on the
// preferred languages of the user and the languages supported by an application.
// The second step is to create the language-speci***REMOVED***c services based on
...
// An application should use a Matcher to ***REMOVED***nd the best supported language based
...
//
// In this case, the best match is Danish, as Danish is suf***REMOVED***ciently a match to
...
//
// Selecting language-speci***REMOVED***c services
//
// One should always use the Tag returned by the Matcher to create an instance
// of any of the language-speci***REMOVED***c services provided by the text repository.
...
// Using the returned Tag also allows user-de***REMOVED***ned settings, such as collation
// order or numbering system to be transparently passed as options.
//
// If you have language-speci***REMOVED***c data in your application, however, it will in
// most cases suf***REMOVED***ce to use the index returned by the matcher to identify
// the user language.
// The following loop provides an alternative in case this is not suf***REMOVED***cient:
//
