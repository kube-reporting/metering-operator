...
//   'marker' + "key=value".
//
// Values are optional; "" is the default.  A tag can be speci***REMOVED***ed more than
...
func ExtractCommentTags(marker string, lines []string) map[string][]string {
	...
	for _, line := range lines {
		...
		if !strings.HasPre***REMOVED***x(line, marker) {
			continue
		}
...
// to be returned.  If the key has multiple values, the ***REMOVED***rst one will be used.
