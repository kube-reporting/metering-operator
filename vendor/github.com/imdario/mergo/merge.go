...
func deepMerge(dst, src reflect.Value, visited map[uintptr]*visit, depth int) (err error) {
	...
	case reflect.Interface:
		if src.IsNil() {
			break
		} ***REMOVED*** if dst.IsNil() {
			if dst.CanSet() && isEmptyValue(dst) {
				dst.Set(src)
			}
		} ***REMOVED*** if err = deepMerge(dst.Elem(), src.Elem(), visited, depth+1); err != nil {
			return
		}
	default:
...
// Merge sets ***REMOVED***elds' values in dst from src if they have a zero
// value of their type.
