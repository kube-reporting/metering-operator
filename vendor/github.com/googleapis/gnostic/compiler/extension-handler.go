...
// ExtensionHandler describes a binary that is called by the compiler to handle speci***REMOVED***cation extensions.
...
func (extensionHandlers *ExtensionHandler) handle(in interface{}, extensionName string) (*any.Any, error) {
	if extensionHandlers.Name != "" {
		...
		if len(response.Error) != 0 {
			message := fmt.Sprintf("Errors when parsing: %+v for ***REMOVED***eld %s by vendor extension handler %s. Details %+v", in, extensionName, extensionHandlers.Name, strings.Join(response.Error, ","))
