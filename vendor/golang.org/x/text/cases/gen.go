...
func parse(***REMOVED***le string, f func(p *ucd.Parser)) {
	ucd.Parse(gen.OpenUCDFile(***REMOVED***le), f)
}
...
func parseUCD() []runeInfo {
	...
	parse("SpecialCasing.txt", func(p *ucd.Parser) {
		...
		// the language-speci***REMOVED***c case mappers. Rune 0x03A3 is the only one with
		// a conditional formatting that is not language-speci***REMOVED***c. However,
		// dealing with this letter is tricky, especially in a streaming
		// context, so we deal with it in the Caser for Greek speci***REMOVED***cally.
		ri := get(p.Rune(0))
		if p.String(4) == "" {
			ri.HasSpecial = true
	...
	parse("auxiliary/WordBreakProperty.txt", func(p *ucd.Parser) {
		...
		switch p.String(1) { // TODO: of***REMOVED***cially we need to canonicalize.
		case "MidLetter", "MidNumLet", "Single_Quote":
			ri.BreakCat = breakMid
			if !ri.CaseIgnorable {
				// ***REMOVED***nalSigma relies on the fact that all breakMid runes are
...
// xorData contains byte-wise XOR data for the least signi***REMOVED***cant bytes of a
...
// from or-ing it with the least-signi***REMOVED***cant bit of the rune, creating large
...
// as well as the modi***REMOVED***ed values.
func makeSparse(vals []uint64) ([]uint16, int) {
	...
	for i, v := range values {
		if v != 0 {
			// Try if the unmodi***REMOVED***ed value is equal to the previous.
			if v == previous {
				continue
			}
...
func (s *sparseCompacter) Handler() string {
	// The sparse global variable and its lookup method is de***REMOVED***ned in gen_trieval.go.
...
// implementation. Each property is marked with an identi***REMOVED***er that is referred
...
func verifyProperties(chars []runeInfo) {
	for i, c := range chars {
		...
		// A.1: modi***REMOVED***er never changes on lowercase. [ltLower]
		...
		if len(d) > 0 {
			if d[0] == 'I' || d[0] == 'J' {
				// A.2.1: we expect at least an ASCII character and a modi***REMOVED***er.
				if len(d) < 3 {
					log.Fatalf("%U: length of decomposition was %d; want >= 3", r, len(d))
				}

				// All subsequent runes are modi***REMOVED***ers and all have the same CCC.
				...
				for _, mr := range runes[1:] {
					...
					// A.2.2: all modi***REMOVED***ers have a CCC of Above or less.
					if ccc == 0 || ccc > above {
						log.Fatalf("%U: CCC of successive rune (%U) was %d; want (0,230]", r, mr, ccc)
					}

					// A.2.3: a sequence of modi***REMOVED***ers all have the same CCC.
					if mc.CCC != ccc {
						log.Fatalf("%U: CCC of follow-up modi***REMOVED***er (%U) was %d; want %d", r, mr, mc.CCC, ccc)
					}

					// A.2.4: for each trailing r, r in [0x300, 0x311] <=> CCC == Above.
					if (ccc == above) != (0x300 <= mr && mr <= 0x311) {
						log.Fatalf("%U: modi***REMOVED***er %U in [U+0300, U+0311] != ccc(%U) == 230", r, mr, mr)
					}
		...
		if unicode.Is(unicode.Greek, r) {
			if b := norm.NFD.PropertiesString(string(r)).Decomposition(); b != nil {
				runes := []rune(string(b))
				// A.6.1: If a Greek rune decomposes and the ***REMOVED***rst rune of the
				// decomposition is greater than U+00FF, the rune is always
				// great and not a modi***REMOVED***er.
				if f := runes[0]; unicode.IsMark(f) || f > 0xFF && !unicode.Is(unicode.Greek, f) {
					log.Fatalf("%U: expected ***REMOVED***rst rune of Greek decomposition to be letter, found %U", r, f)
				}
				// A.6.2: Any follow-up rune in a Greek decomposition is a
				// modi***REMOVED***er of which the ***REMOVED***rst should be gobbled in
				// decomposition.
				for _, m := range runes[1:] {
					switch m {
					case 0x0313, 0x0314, 0x0301, 0x0300, 0x0306, 0x0342, 0x0308, 0x0304, 0x345:
					default:
						log.Fatalf("%U: modi***REMOVED***er %U is outside of expected Greek modi***REMOVED***er set", r, m)
					}
				}
			}
		}
...
func genTablesTest() {
	...
	if n > 0 {
		log.Fatalf("One of the discarded properties does not have a perfect ***REMOVED***lter.")
	}
...
// These functions are just used for veri***REMOVED***cation that their de***REMOVED***nition have not
...
// printProperties prints tables of rune properties from the given UCD ***REMOVED***le.
// A ***REMOVED***lter func f can be given to exclude certain values. A rune r will have
// the indicated property if it is in the generated table or if f(r).
func printProperties(w io.Writer, ***REMOVED***le, property string, f func(r rune) bool) int {
	...
	parse(***REMOVED***le, func(p *ucd.Parser) {
	...
	for r := rune(0); r <= lastRuneForTesting; r++ {
		if !verify[r] && f(r) {
			log.Fatalf("Incorrect ***REMOVED***lter func for property %q.", property)
		}
	}
	return n
}

// The newCaseTrie, sparseValues and sparseOffsets de***REMOVED***nitions below are
// placeholders referred to by gen_trieval.go. The real de***REMOVED***nitions are
