...
// accumulate any single UTF8 sequence. Lines that ***REMOVED***t entirely
...
//
// The pkg string, if present, speci***REMOVED***es the import path to
...
var (
	...
	skipLinePre***REMOVED***x = []byte("?   \t")
	skipLineSuf***REMOVED***x = []byte("\t[no test ***REMOVED***les]\n")
)
...
func (c *converter) handleInputLine(line []byte) {
	...
	// Special case for entirely skipped test binary: "?   \tpkgname\t[no test ***REMOVED***les]\n" is only line.
	// Report it as plain output but remember to say skip in the ***REMOVED***nal summary.
	if bytes.HasPre***REMOVED***x(line, skipLinePre***REMOVED***x) && bytes.HasSuf***REMOVED***x(line, skipLineSuf***REMOVED***x) && len(c.report) == 0 {
		c.result = "skip"
	}
	...
	for _, magic := range updates {
		if bytes.HasPre***REMOVED***x(line, magic) {
			ok = true
			break
		}
	}
	if !ok {
		...
		for bytes.HasPre***REMOVED***x(line, fourSpace) {
			line = line[4:]
			indent++
		}
	...
	action := strings.ToLower(strings.TrimSuf***REMOVED***x(strings.TrimSpace(string(line[4:i])), ":"))
	...
	if line[0] == '-' { // PASS or FAIL report
		...
		if i := strings.Index(name, " ("); i >= 0 {
			if strings.HasSuf***REMOVED***x(name, "s)") {
	...
	if action == "pause" {
		// For a pause, we want to write the pause noti***REMOVED***cation before
...
// and then emits the ***REMOVED***nal overall package-level pass/fail event.
...
// The lineBuffer will call line(x) for any whole line x (including the ***REMOVED***nal newline)
// that ***REMOVED***ts entirely in cap(b). It will handle input lines longer than cap(b) by
// calling part(x) for sections of the line. The line will be split at UTF8 boundaries,
// and the ***REMOVED***nal call to part for a long line includes the ***REMOVED***nal newline.
...
// that might appear as the ***REMOVED***rst ***REMOVED***eld in a benchmark result line.
func isBenchmarkName(b []byte) bool {
	if !bytes.HasPre***REMOVED***x(b, benchmark) {
		return false
	}
...
func trimUTF8(b []byte) int {
	// Scan backward to ***REMOVED***nd non-continuation byte.
