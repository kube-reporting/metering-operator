...
func newTestFromFile(t testutil.T, ***REMOVED***lename string) (*Test, error) {
	content, err := ioutil.ReadFile(***REMOVED***lename)
	if err != nil {
		return nil, err
	}
...
func (t *Test) parseLoad(lines []string, i int) (int, *loadCmd, error) {
	...
	if err != nil {
		return i, nil, raise(i, "invalid step de***REMOVED***nition %q: %s", parts[1], err)
	}
...
func (t *Test) parse(input string) error {
	...
	for i := 0; i < len(lines); i++ {
		...
		case strings.HasPre***REMOVED***x(c, "eval"):
...
// loadCmd is a command that loads sequences of sample values for speci***REMOVED***c
...
// append the de***REMOVED***ned time series to the storage.
...
// and expects a speci***REMOVED***c result.
type evalCmd struct {
	expr       Expr
...
// compareResult compares the result value with the de***REMOVED***ned expectation.
