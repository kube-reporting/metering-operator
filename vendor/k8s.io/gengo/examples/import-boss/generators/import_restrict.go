...
// Packages makes the import-boss package de***REMOVED***nition.
...
type Rule struct {
	...
	// ... must have one of these pre***REMOVED***xes ...
	AllowedPre***REMOVED***xes []string
	// ... and must not have one of these pre***REMOVED***xes.
	ForbiddenPre***REMOVED***xes []string
}

type ***REMOVED***leFormat struct {
	CurrentImports []string

	Rules []Rule
}

func readFile(path string) (****REMOVED***leFormat, error) {
	currentBytes, err := ioutil.ReadFile(path)
	if err != nil {
		return nil, fmt.Errorf("couldn't read %v: %v", path, err)
	}

	var current ***REMOVED***leFormat
...
func writeFile(path string, ff ****REMOVED***leFormat) error {
	raw, err := json.MarshalIndent(ff, "", "\t")
	if err != nil {
		return fmt.Errorf("couldn't format data for ***REMOVED***le %v.\n%#v", path, ff)
	}
...
// This does the actual checking, since it knows the literal destination ***REMOVED***le.
...
func (importRuleFile) listEntireImportTree(f *generator.File, path string) error {
	// If the ***REMOVED***le exists, populate its current imports. This is mostly to help
	// humans ***REMOVED***gure out what they need to ***REMOVED***x.
	if _, err := os.Stat(path); err != nil {
		// Ignore packages which haven't opted in by adding an .import-restrictions ***REMOVED***le.
		return nil
	}
...
// removeLastDir removes the last directory, but leaves the ***REMOVED***le name
// unchanged. It returns the new path and the removed directory. So:
// "a/b/c/***REMOVED***le" -> ("a/b/***REMOVED***le", "c")
func removeLastDir(path string) (newPath, removedDir string) {
	dir, ***REMOVED***le := ***REMOVED***lepath.Split(path)
	dir = strings.TrimSuf***REMOVED***x(dir, string(***REMOVED***lepath.Separator))
	return ***REMOVED***lepath.Join(***REMOVED***lepath.Dir(dir), ***REMOVED***le), ***REMOVED***lepath.Base(dir)
}

// Keep going up a directory until we ***REMOVED***nd an .import-restrictions ***REMOVED***le.
func recursiveRead(path string) (****REMOVED***leFormat, string, error) {
	for {
		if _, err := os.Stat(path); err == nil {
...
func (importRuleFile) VerifyFile(f *generator.File, path string) error {
	...
	if err != nil {
		return fmt.Errorf("error ***REMOVED***nding rules ***REMOVED***le: %v", err)
	}
	...
	for _, r := range rules.Rules {
		...
		if err != nil {
			return fmt.Errorf("regexp `%s` in ***REMOVED***le %q doesn't compile: %v", r.SelectorRegexp, actualPath, err)
		}
		for v := range f.Imports {
			...
			for _, forbidden := range r.ForbiddenPre***REMOVED***xes {
				glog.V(4).Infof("Checking %v against %v\n", v, forbidden)
				if strings.HasPre***REMOVED***x(v, forbidden) {
					return fmt.Errorf("import %v has forbidden pre***REMOVED***x %v", v, forbidden)
				}
			}
			found := false
			for _, allowed := range r.AllowedPre***REMOVED***xes {
				glog.V(4).Infof("Checking %v against %v\n", v, allowed)
				if strings.HasPre***REMOVED***x(v, allowed) {
					found = true
					break
				}
			}
			if !found {
				return fmt.Errorf("import %v did not match any allowed pre***REMOVED***x", v)
			}
		}
	}
...
// importRules produces a ***REMOVED***le with a set for a single type.
