...
func newProtobufPackage(packagePath, packageName string, generateAll bool, omitFieldTypes map[types.Name]struct{}) *protobufPackage {
	pkg := &protobufPackage{
		DefaultPackage: generator.DefaultPackage{
			...
			HeaderText: []byte(
				`
// This ***REMOVED***le was autogenerated by go-to-protobuf. Do not edit it manually!
...
`, packageName)),
	...
	pkg.FilterFunc = pkg.***REMOVED***lterFunc
...
type protobufPackage struct {
	...
	// A list of types to ***REMOVED***lter to; if not speci***REMOVED***ed all types will be included.
	FilterTypes map[types.Name]struct{}

	// If true, omit any gogoprotobuf extensions not de***REMOVED***ned as types.
	OmitGogo bool

	// A list of ***REMOVED***eld types that will be excluded from the output struct
	...
	// to remove synthetic protobuf ***REMOVED***elds.
	OptionalTypeNames map[string]struct{}

	// A list of struct tags to generate onto named struct ***REMOVED***elds
...
func (p *protobufPackage) Clean(outputBase string) error {
	for _, s := range []string{p.ImportPath(), p.OutputPath()} {
		if err := os.Remove(***REMOVED***lepath.Join(outputBase, s)); err != nil && !os.IsNotExist(err) {
			return err
		}
	}
	return nil
...
func (p *protobufPackage) ***REMOVED***lterFunc(c *generator.Context, t *types.Type) bool {
	switch t.Kind {
...
func (p *protobufPackage) ExtractGeneratedType(t *ast.TypeSpec) bool {
	...
	case *ast.StructType:
		for i, f := range s.Fields.List {
			...
			if len(f.Names) > 1 {
				log.Printf("WARNING: struct %s ***REMOVED***eld %d %s: de***REMOVED***ned multiple names but single protobuf tag", t.Name.Name, i, f.Names[0].Name)
				// TODO hard error?
			}
			if p.StructTags == nil {
	...
	default:
		log.Printf("WARNING: unexpected Go AST type de***REMOVED***nition: %#v", t)
	}
...
func (p *protobufPackage) GoPackageName() string {
	return ***REMOVED***lepath.Base(p.PackagePath)
}

func (p *protobufPackage) ImportPath() string {
	return ***REMOVED***lepath.Join(p.PackagePath, "generated.proto")
}

func (p *protobufPackage) OutputPath() string {
	return ***REMOVED***lepath.Join(p.PackagePath, "generated.pb.go")
}
