...
func packageForGroup(gv clientgentypes.GroupVersion, typeList []*types.Type, clientsetPackage string, groupPackageName string, groupGoName string, apiPath string, srcTreePath string, inputPackage string, boilerplate []byte) generator.Package {
	groupVersionClientPackage := strings.ToLower(***REMOVED***lepath.Join(clientsetPackage, "typed", groupPackageName, gv.Version.NonEmpty()))
...
// ***REMOVED***rst ***REMOVED***eld (somegroup) as the name of the group when generating.
//
...
func applyGroupOverrides(universe types.Universe, customArgs *clientgenargs.CustomArgs) {
	...
	for _, gvs := range customArgs.Groups {
		gv := clientgentypes.GroupVersion{
			Group:   gvs.Group,
			Version: gvs.Versions[0], // we only need a version, and the ***REMOVED***rst will do
		}
...
func Packages(context *generator.Context, arguments *args.GeneratorArgs) generator.Packages {
	...
	clientsetPackage := ***REMOVED***lepath.Join(customArgs.ClientsetOutputPath, customArgs.ClientsetName)
