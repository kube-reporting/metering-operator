...
// ***REMOVED***les and to save objects in LevelDB.
//
...
// the ***REMOVED***ngerprint as a big-endian uint64.
...
// TimeRange is used to de***REMOVED***ne a time range and implements
// encoding.BinaryMarshaler and encoding.BinaryUnmarshaler.
...
func (tr *TimeRange) UnmarshalBinary(buf []byte) error {
	r := bytes.NewReader(buf)
	***REMOVED***rst, err := binary.ReadVarint(r)
	if err != nil {
		return err
	}
	...
	tr.First = model.Time(***REMOVED***rst)
