...
// Encoding de***REMOVED***nes which encoding we are using, delta, doubledelta, or varbit
type Encoding byte
...
// caller must make sure nobody ***REMOVED*** will call these methods concurrently,
// either by holding the sole reference to the Desc (usually during loading
// or creation) or by locking the ***REMOVED***ngerprint of the series the Desc
...
// to have locked the ***REMOVED***ngerprint of the series but the chunk does not need to
// be pinned. That's because the ChunkLastTime ***REMOVED***eld in Desc gets populated
// upon completion of the chunk (when it is still pinned, and which happens
// while the series's ***REMOVED***ngerprint is locked). Once that has happened, calling
// LastTime does not require the chunk to be loaded anymore. Before that has
// happened, the chunk is pinned anyway. The ChunkFirstTime ***REMOVED***eld in Desc
...
func NewDesc(c Chunk, ***REMOVED***rstTime model.Time) *Desc {
	...
	return &Desc{
		C:              c,
		rCnt:           1,
		ChunkFirstTime: ***REMOVED***rstTime,
		ChunkLastTime:  model.Earliest,
	}
}

// Add adds a sample pair to the underlying chunk. For safe concurrent access,
// The chunk must be pinned, and the caller must have locked the ***REMOVED***ngerprint of
// the series.
...
// FirstTime returns the timestamp of the ***REMOVED***rst sample in the chunk. This method
...
// concurrent access, this method requires the ***REMOVED***ngerprint of the time series to
// be locked.
...
// the ***REMOVED***ngerprint of the series.
...
func (d *Desc) IsEvicted() bool {
	...
	// pinning the chunk ***REMOVED***rst, so it could be evicted while this method is
	// called.
	d.Lock()
	defer d.Unlock()
...
// ***REMOVED***ngerprint of the series and must have "pre-pinned" the chunk (i.e. ***REMOVED***rst
...
type Chunk interface {
	...
	// any. The ***REMOVED***rst chunk returned might be the same as the original one
...
// Iterator enables ef***REMOVED***cient access to the content of a chunk. It is
...
type Iterator interface {
	...
	// Whether a given timestamp is contained between ***REMOVED***rst and last value
	// in the chunk.
	Contains(model.Time) (bool, error)
	// Scans the next value in the chunk. Directly after the iterator has
	// been created, the next value is the ***REMOVED***rst value in the
	...
	// of the ***REMOVED***nd... methods). It returns model.ZeroSamplePair before any of
...
// NewForEncoding allows con***REMOVED***guring what chunk type you want
