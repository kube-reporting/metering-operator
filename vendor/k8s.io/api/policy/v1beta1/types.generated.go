			...
			if yyr2 || yy2arr2 {
				...
				if yyq2[0] {
					if x.MinAvailable == nil {
						...
						_ = yym4
						if false {
						} ***REMOVED*** if z.HasExtensions() && z.EncExt(x.MinAvailable) {
	...
	for yyj3 := 0; ; yyj3++ {
		...
		case "minAvailable":
			if r.TryDecodeAsNil() {
				...
				_ = yym5
				if false {
				} ***REMOVED*** if z.HasExtensions() && z.DecExt(x.MinAvailable) {
				} ***REMOVED*** if !yym5 && z.IsJSONHandle() {
			...
			if yyr2 || yy2arr2 {
				...
				if yyq2[3] {
					if x.DeleteOptions == nil {
						...
						_ = yym15
						if false {
						} ***REMOVED*** if z.HasExtensions() && z.EncExt(x.DeleteOptions) {
						} ***REMOVED*** {
	...
	for yyj3 := 0; ; yyj3++ {
		...
		case "deleteOptions":
			if r.TryDecodeAsNil() {
				...
				_ = yym11
				if false {
				} ***REMOVED*** if z.HasExtensions() && z.DecExt(x.DeleteOptions) {
				} ***REMOVED*** {
...
func (x codecSelfer1234) encMapstringv1_Time(v map[string]pkg2_v1.Time, e *codec1978.Encoder) {
	...
	for yyk1, yyv1 := range v {
		...
		_ = yym4
		if false {
		} ***REMOVED*** if z.HasExtensions() && z.EncExt(yy3) {
		} ***REMOVED*** if yym4 {
