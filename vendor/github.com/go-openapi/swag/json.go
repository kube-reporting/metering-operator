...
// WriteJSON writes json data, prefers ***REMOVED***nding an appropriate interface to short-circuit the marshaller
...
// ReadJSON reads json data, prefers ***REMOVED***nding an appropriate interface to short-circuit the unmarshaller
...
// ConcatJSON concatenates multiple json objects ef***REMOVED***ciently
func ConcatJSON(blobs ...[]byte) []byte {
	...
	for i, b := range blobs[:last+1] {
		...
		idx = 0
		if a > 0 { // we need to join with a comma for everything beyond the ***REMOVED***rst non-empty item
			if err := buf.WriteByte(comma); err != nil {
				log.Println(err)
			}
			idx = 1 // this is not the ***REMOVED***rst or the last so we want to drop the leading bracket
		}
		...
		} ***REMOVED*** { // last one, strip only the leading bracket
...
func buildnameIndex(tpe reflect.Type, idx, reverseIdx map[string]string) {
	for i := 0; i < tpe.NumField(); i++ {
		...
		if targetDes.Anonymous { // walk embedded structures tree down ***REMOVED***rst
