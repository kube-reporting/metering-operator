...
func NewRESTMapper(groupResources []*APIGroupResources, versionInterfaces meta.VersionInterfacesFunc) meta.RESTMapper {
	...
	// /v1 is special.  It should always come ***REMOVED***rst
	...
	for _, group := range groupResources {
		...
		// Make sure the preferred version comes ***REMOVED***rst
		...
		for _, discoveryVersion := range group.Group.Versions {
			...
			for _, resource := range resources {
				...
				versionMapper.AddSpeci***REMOVED***c(gv.WithKind(resource.Kind), plural, singular, scope)
				// TODO this is producing unsafe guesses that don't actually work, but it matches previous behavior
				versionMapper.Add(gv.WithKind(resource.Kind+"List"), scope)
			}
...
// initialization of the RESTMapper until the ***REMOVED***rst mapping is
// requested.
...
// RESTMapping identi***REMOVED***es a preferred resource mapping for the
// provided group kind.
...
// Make sure it satis***REMOVED***es the interface
