...
const maxMappedFP = 1 << 20 // About 1M ***REMOVED***ngerprints reserved for mapping.

var separatorString = string([]byte{model.SeparatorByte})

// fpMappings maps original ***REMOVED***ngerprints to a map of string representations of
// metrics to the truly unique ***REMOVED***ngerprint.
type fpMappings map[model.Fingerprint]map[string]model.Fingerprint

// fpMapper is used to map ***REMOVED***ngerprints in order to work around ***REMOVED***ngerprint
// collisions.
...
func newFPMapper(fpToSeries *seriesMap, p *persistence) (*fpMapper, error) {
	m := &fpMapper{
		...
		mappingsCounter: prometheus.NewCounter(prometheus.CounterOpts{
			Namespace: namespace,
			Subsystem: subsystem,
			Name:      "***REMOVED***ngerprint_mappings_total",
			Help:      "The total number of ***REMOVED***ngerprints being mapped to avoid collisions.",
		}),
	}
...
// mapFP takes a raw ***REMOVED***ngerprint (as returned by Metrics.FastFingerprint) and
// returns a truly unique ***REMOVED***ngerprint. The caller must have locked the raw
// ***REMOVED***ngerprint.
//
...
func (m *fpMapper) mapFP(fp model.Fingerprint, metric model.Metric) model.Fingerprint {
	...
	if fpAlreadyMapped {
		...
		// 'collisions' required (it is speci***REMOVED***c to fp).
...
// truly unique ***REMOVED***ngerprint for the colliding metric.
func (m *fpMapper) maybeAddMapping(
	...
	if ok {
		...
		log.Infof(
			"Collision detected for ***REMOVED***ngerprint %v, metric %v, mapping to new ***REMOVED***ngerprint %v.",
			fp, collidingMetric, mappedFP,
		)
		return mappedFP
	}
	// This is the ***REMOVED***rst collision for fp.
	mappedFP := m.nextMappedFP()
...
func (m *fpMapper) nextMappedFP() model.Fingerprint {
	...
	if mappedFP > maxMappedFP {
		panic(fmt.Errorf("more than %v ***REMOVED***ngerprints mapped in collision detection", maxMappedFP))
	}
	return mappedFP
}
...
// "ideal" ***REMOVED***ngerprint function, only that it is more expensive than the
