...
const (
	...
	***REMOVED***eldTransport10Len = 10
	***REMOVED***eldTransport11Len = 13
)
...
func parseNFSTransportStats(ss []string, statVersion string) (*NFSTransportStats, error) {
	switch statVersion {
	case statVersion10:
		if len(ss) != ***REMOVED***eldTransport10Len {
			return nil, fmt.Errorf("invalid NFS transport stats 1.0 statement: %v", ss)
		}
	case statVersion11:
		if len(ss) != ***REMOVED***eldTransport11Len {
	...
	ns := make([]uint64, ***REMOVED***eldTransport11Len)
	for i, s := range ss {
		n, err := strconv.ParseUint(s, 10, 64)
		if err != nil {
			return nil, err
