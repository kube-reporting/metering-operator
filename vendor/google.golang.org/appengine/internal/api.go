...
const (
	apiPath             = "/rpc_http"
	defaultTicketSuf***REMOVED***x = "/default.20150612t184001.0"
)
...
// jointContext joins two contexts in a super***REMOVED***cial way.
...
func DefaultTicket() string {
	defaultTicketOnce.Do(func() {
		if IsDevAppServer() {
			defaultTicket = "testapp" + defaultTicketSuf***REMOVED***x
			return
		}
...
func Call(ctx netcontext.Context, service, method string, in, out proto.Message) error {
	...
	if ticket == "" {
		if appid := ctx.Value(&appIDOverrideKey); appid != nil {
			ticket = appid.(string) + defaultTicketSuf***REMOVED***x
		}
	}
