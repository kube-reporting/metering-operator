...
// However, the user can con***REMOVED***gre how to encode/decode bytes.
//
// This library speci***REMOVED***cally supports UTF-8 for encoding and decoding only.
//
...
const (
	...
	//   - If we see ***REMOVED***rst character of null, false or true,
	//     do not validate subsequent characters.
	//   - e.g. if we see a n, assume null and skip next 3 characters,
	//     and do not validate they are ull.
	// P.S. Do not expect a signi***REMOVED***cant decoding boost from this.
...
func (e *jsonEncDriver) sendContainerState(c containerState) {
	...
	if c == containerMapKey {
	...
	} ***REMOVED*** if c == containerMapValue {
		if e.d {
			e.w.writen2(':', ' ')
		} ***REMOVED*** {
			e.w.writen1(':')
		}
	} ***REMOVED*** if c == containerMapEnd {
		if e.d {
			e.dl--
			if e.c != containerMapStart {
				e.writeIndent()
		...
		e.w.writen1('}')
	} ***REMOVED*** if c == containerArrayElem {
	...
	} ***REMOVED*** if c == containerArrayEnd {
		if e.d {
			e.dl--
			if e.c != containerArrayStart {
				e.writeIndent()
...
func (e *jsonEncDriver) EncodeStringBytes(c charEncoding, v []byte) {
	// if encoding raw bytes and RawBytesExt is con***REMOVED***gured, use it to encode
...
func (e *jsonEncDriver) quoteStr(s string) {
	...
	for i := 0; i < len(s); {
		...
		// Both technically valid JSON, but bomb on JSONP, so ***REMOVED***x here.
		if c == '\u2028' || c == '\u2029' {
			if start < i {
				w.writestr(s[start:i])
			}
...
func (x *jsonNum) floatVal() (f float64, parseUsingStrConv bool) {
	...
	//    - The mantissa cannot ***REMOVED***t into a 52 bits of uint64
...
// 	// fast-path: do not enter loop. Just check ***REMOVED***rst (in case no whitespace).
// 	b := d.r.readn1()
// 	if jsonIsWS(b) {
// 		r := d.r
...
func (d *jsonDecDriver) DecodeBool() bool {
	...
	d.d.errorf("json: decode bool: got ***REMOVED***rst char %c", d.tok)
...
func (d *jsonDecDriver) ContainerType() (vt valueType) {
	// check container type by checking the ***REMOVED***rst char
	if d.tok == 0 {
		var b byte
		r := d.r
		for b = r.readn1(); jsonIsWS(b); b = r.readn1() {
...
func (d *jsonDecDriver) decNum(storeBytes bool) {
	// If it is has a . or an e|E, decode as a float; ***REMOVED*** decode as an int.
	if d.tok == 0 {
		var b byte
		r := d.r
		for b = r.readn1(); jsonIsWS(b); b = r.readn1() {
	...
	if !(b == '+' || b == '-' || b == '.' || (b >= '0' && b <= '9')) {
		d.d.errorf("json: decNum: got ***REMOVED***rst char '%c'", b)
		return
	}
	d.tok = 0
...
func (d *jsonDecDriver) DecodeBytes(bs []byte, isstring, zerocopy bool) (bsOut []byte) {
	// if decoding into raw bytes, and the RawBytesExt is con***REMOVED***gured, use it to decode.
	...
	if slen <= cap(bs) {
		bsOut = bs[:slen]
	} ***REMOVED*** if zerocopy && slen <= cap(d.b2) {
		bsOut = d.b2[:slen]
	} ***REMOVED*** {
...
//    - con***REMOVED***gurable way to encode/decode []byte .
...
type JsonHandle struct {
	textEncodingType
	BasicHandle
	// RawBytesExt, if con***REMOVED***gured, is used to encode and decode raw bytes in a custom way.
	// If not con***REMOVED***gured, raw bytes are encoded to/from base64 text.
	...
	// This can be mitigated by con***REMOVED***guring how to encode integers.
	//
	// IntegerAsString interpretes the following values:
	//   - if 'L', then encode integers > 2^53 as a json string.
	//   - if 'A', then encode all integers as a json string
	//             containing the exact integer representation as a decimal.
	//   - ***REMOVED***    encode all integers as a json number (default)
