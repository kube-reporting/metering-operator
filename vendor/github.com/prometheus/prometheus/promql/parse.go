...
func ParseMetricSelector(input string) (m []*labels.Matcher, err error) {
	...
	name := ""
	if t := p.peek().typ; t == itemMetricIdenti***REMOVED***er || t == itemIdenti***REMOVED***er {
		name = p.next().val
	}
...
func (p *parser) parseSeriesDesc() (m labels.Labels, vals []sequenceValue, err error) {
	...
	for {
		...
		} ***REMOVED*** if t == itemIdenti***REMOVED***er && p.peek().val == "stale" {
			p.next()
		...
		// If there are no offset repetitions speci***REMOVED***ed, proceed with the next value.
		if t := p.peek(); t.typ == itemNumber || t.typ == itemBlank || t.typ == itemIdenti***REMOVED***er && t.val == "stale" {
			continue
		} ***REMOVED*** if t.typ == itemEOF {
			break
		} ***REMOVED*** if t.typ != itemADD && t.typ != itemSUB {
...
func (p *parser) stmt() Statement {
	...
	case itemAlert:
		return p.alertStmt()
	case itemIdenti***REMOVED***er, itemMetricIdenti***REMOVED***er:
		return p.recordStmt()
	}
...
func (p *parser) alertStmt() *AlertStmt {
	...
	name := p.expect(itemIdenti***REMOVED***er, ctx)
...
func (p *parser) recordStmt() *RecordStmt {
	...
	name := p.expectOneOf(itemIdenti***REMOVED***er, itemMetricIdenti***REMOVED***er, ctx).val
...
func (p *parser) expr() Expr {
	...
	for {
		...
		returnBool := false
		// Parse bool modi***REMOVED***er.
		if p.peek().typ == itemBool {
			if !op.isComparisonOperator() {
				p.errorf("bool modi***REMOVED***er can only be used on comparison operators")
			}
			p.next()
			returnBool = true
		}
	...
	if lhsBE, ok := lhs.(*BinaryExpr); ok {
		...
		if (precd < 0) || (precd == 0 && op.isRightAssociative()) {
			...
			if lhsBE.Op.isComparisonOperator() && !lhsBE.ReturnBool && balanced.Type() == ValueTypeScalar && lhsBE.LHS.Type() == ValueTypeScalar {
				p.errorf("comparisons between scalars must use BOOL modi***REMOVED***er")
			}
			return &BinaryExpr{
				Op:             lhsBE.Op,
				LHS:            lhsBE.LHS,
...
func (p *parser) unaryExpr() Expr {
	...
	if p.peek().typ == itemLeftBracket {
		...
		if !ok {
			p.errorf("range speci***REMOVED***cation must be preceded by a metric selector, but follows a %T instead", e)
		}
	...
	if p.peek().typ == itemOffset {
		...
		case *MatrixSelector:
			s.Offset = offset
		default:
			p.errorf("offset modi***REMOVED***er must be preceded by an instant or range selector, but follows a %T instead", e)
		}
	}
...
func (p *parser) primaryExpr() Expr {
	...
	case t.typ == itemIdenti***REMOVED***er:
	...
	case t.typ == itemMetricIdenti***REMOVED***er:
...
func (p *parser) labelMatchers(operators ...itemType) []*labels.Matcher {
	...
	for {
		label := p.expect(itemIdenti***REMOVED***er, ctx)
		...
		if p.peek().typ == itemIdenti***REMOVED***er {
			p.errorf("missing comma before next identi***REMOVED***er %q", p.peek().val)
		}
...
//
//		<label_set>
//		<metric_identi***REMOVED***er> [<label_set>]
//
func (p *parser) metric() labels.Labels {
	...
	t := p.peek().typ
	if t == itemIdenti***REMOVED***er || t == itemMetricIdenti***REMOVED***er {
		name = p.next().val
		t = p.peek().typ
	}
...
// offset parses an offset modi***REMOVED***er.
//
//		offset <duration>
//
func (p *parser) offset() time.Duration {
	...
	p.next()
	of***REMOVED*** := p.expect(itemDuration, ctx)

	offset, err := parseDuration(of***REMOVED***.val)
	if err != nil {
		p.error(err)
	}
...
//
//		<metric_identi***REMOVED***er> [<label_matchers>]
//		[<metric_identi***REMOVED***er>] <label_matchers>
//
...
func (p *parser) checkType(node Node) (typ ValueType) {
	...
	case *Call:
		...
		if n.Func.Variadic == 0 {
			...
			} ***REMOVED*** if nargsmax := na + n.Func.Variadic; n.Func.Variadic > 0 && nargsmax < len(n.Args) {
