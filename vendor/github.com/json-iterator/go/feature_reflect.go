...
type pre***REMOVED***x string

func (p pre***REMOVED***x) addToDecoder(decoder ValDecoder, err error) (ValDecoder, error) {
	if err != nil {
...
func (p pre***REMOVED***x) addToEncoder(encoder ValEncoder, err error) (ValEncoder, error) {
	if err != nil {
		return nil, fmt.Errorf("%s: %s", p, err.Error())
	}
	return encoder, err
...
func decoderOfType(cfg *frozenCon***REMOVED***g, typ reflect.Type) (ValDecoder, error) {
	cacheKey := typ
...
func createDecoderOfType(cfg *frozenCon***REMOVED***g, typ reflect.Type) (ValDecoder, error) {
	typeName := typ.String()
	...
	if typ.Kind() == reflect.Slice && typ.Elem().Kind() == reflect.Uint8 {
		sliceDecoder, err := pre***REMOVED***x("[slice]").addToDecoder(decoderOfSlice(cfg, typ))
		if err != nil {
			return nil, err
		}
	...
	case reflect.Struct:
		return pre***REMOVED***x(fmt.Sprintf("[%s]", typeName)).addToDecoder(decoderOfStruct(cfg, typ))
	case reflect.Array:
		return pre***REMOVED***x("[array]").addToDecoder(decoderOfArray(cfg, typ))
	case reflect.Slice:
		return pre***REMOVED***x("[slice]").addToDecoder(decoderOfSlice(cfg, typ))
	case reflect.Map:
		return pre***REMOVED***x("[map]").addToDecoder(decoderOfMap(cfg, typ))
	case reflect.Ptr:
		return pre***REMOVED***x("[optional]").addToDecoder(decoderOfOptional(cfg, typ))
	default:
		return nil, fmt.Errorf("unsupported type: %v", typ)
	}
}

func encoderOfType(cfg *frozenCon***REMOVED***g, typ reflect.Type) (ValEncoder, error) {
	cacheKey := typ
...
func createEncoderOfType(cfg *frozenCon***REMOVED***g, typ reflect.Type) (ValEncoder, error) {
	if typ == jsonRawMessageType {
		return &jsonRawMessageCodec{}, nil
	}
	if typ == jsoniterRawMessageType {
...
func createEncoderOfSimpleType(cfg *frozenCon***REMOVED***g, typ reflect.Type) (ValEncoder, error) {
	typeName := typ.String()
	kind := typ.Kind()
	switch kind {
	case reflect.String:
	...
	case reflect.Interface:
		...
		return &nonEmptyInterfaceCodec{}, nil
	case reflect.Struct:
		return pre***REMOVED***x(fmt.Sprintf("[%s]", typeName)).addToEncoder(encoderOfStruct(cfg, typ))
	case reflect.Array:
		return pre***REMOVED***x("[array]").addToEncoder(encoderOfArray(cfg, typ))
	case reflect.Slice:
		return pre***REMOVED***x("[slice]").addToEncoder(encoderOfSlice(cfg, typ))
	case reflect.Map:
		return pre***REMOVED***x("[map]").addToEncoder(encoderOfMap(cfg, typ))
	case reflect.Ptr:
		return pre***REMOVED***x("[optional]").addToEncoder(encoderOfOptional(cfg, typ))
	default:
		return nil, fmt.Errorf("unsupported type: %v", typ)
	}
}

func decoderOfOptional(cfg *frozenCon***REMOVED***g, typ reflect.Type) (ValDecoder, error) {
	elemType := typ.Elem()
...
func encoderOfOptional(cfg *frozenCon***REMOVED***g, typ reflect.Type) (ValEncoder, error) {
	elemType := typ.Elem()
...
func decoderOfMap(cfg *frozenCon***REMOVED***g, typ reflect.Type) (ValDecoder, error) {
...
func encoderOfMap(cfg *frozenCon***REMOVED***g, typ reflect.Type) (ValEncoder, error) {
	elemType := typ.Elem()
