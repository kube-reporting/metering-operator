...
podTemplate(
    ...
    timestamps {
        ...
        def e2eFinalPodLogsFile = 'e2e-***REMOVED***nal-pod-logs.log'
        ...
        def integrationFinalPodLogsFile = 'integration-***REMOVED***nal-pod-logs.log'
                ...
                try {
                    ansiColor('xterm') {
                        timeout(20) {
                            ...
                            docker run \
                            -i --rm \
                            --env-***REMOVED***le <(env | grep -E 'INSTALL_METHOD|DOCKER|TEST|LOG|DEPLOY|KUBECONFIG|AWS|CHARGEBACK|METERING|PULL_SECRET') \
