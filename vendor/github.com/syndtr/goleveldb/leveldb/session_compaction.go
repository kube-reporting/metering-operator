...
func (s *session) getCompactionRange(sourceLevel int, umin, umax []byte, noLimit bool) *compaction {
	...
	// But we cannot do this for level-0 since level-0 ***REMOVED***les can overlap
	// and we must not pick one ***REMOVED***le and drop another older ***REMOVED***le if the
	// two ***REMOVED***les overlap.
...
func (c *compaction) expand() {
	...
	// changing the number of "sourceLevel+1" ***REMOVED***les we pick up.
	if len(t1) > 0 {
	...
	// Compute the set of grandparent ***REMOVED***les that overlap this compaction
		...
		for c.tPtrs[level] < len(tables) {
			...
			if c.s.icmp.uCompare(ukey, t.imax.ukey()) <= 0 {
				...
				if c.s.icmp.uCompare(ukey, t.imin.ukey()) >= 0 {
					// Key falls in this ***REMOVED***le's range, so de***REMOVED***nitely not base level.
					return false
				}
				break
			}
