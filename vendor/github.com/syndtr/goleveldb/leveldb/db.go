...
func openDB(s *session) (*DB, error) {
	...
	if readOnly {
		...
		// Remove any obsolete ***REMOVED***les.
...
func Open(stor storage.Storage, o *opt.Options) (db *DB, err error) {
	...
	} ***REMOVED*** if s.o.GetErrorIfExist() {
		err = os.ErrExist
		return
	}
...
// os.ErrExist error.
//
// OpenFile uses standard ***REMOVED***le-system backed storage implementation as
...
// Recover recovers and opens a DB with missing or corrupted manifest ***REMOVED***les
// for the given storage. It will ignore any manifest ***REMOVED***les, valid or not.
...
// RecoverFile recovers and opens a DB with missing or corrupted manifest ***REMOVED***les
// for the given path. It will ignore any manifest ***REMOVED***les, valid or not.
// The DB must already exist or it will returns an error.
// Also, Recover will ignore ErrorIfMissing and ErrorIfExist options.
//
// RecoverFile uses standard ***REMOVED***le-system backed storage implementation as described
...
func recoverTable(s *session, o *opt.Options) error {
	...
	// Get all tables and sort it by ***REMOVED***le number.
	...
	if len(fds) > 0 {
		...
		// Mark ***REMOVED***le number as used.
...
func (db *DB) recoverJournal() error {
	// Get all journals and sort it by ***REMOVED***le number.
	...
	var (
		ofd storage.FileDesc // Obsolete ***REMOVED***le.
		rec = &sessionRecord{}
	)
	...
	if len(fds) > 0 {
		...
		for _, fd := range fds {
			...
			// Flush memdb and remove obsolete journal ***REMOVED***le.
			if !ofd.Zero() {
				if mdb.Len() > 0 {
	...
	// Remove the last obsolete journal ***REMOVED***le.
	if !ofd.Zero() {
		db.s.stor.Remove(ofd)
	}
...
//
// Property names:
//	leveldb.num-***REMOVED***les-at-level{n}
//		Returns the number of ***REMOVED***les at level 'n'.
//	leveldb.stats
...
func (db *DB) GetProperty(name string) (value string, err error) {
	...
	const pre***REMOVED***x = "leveldb."
	if !strings.HasPre***REMOVED***x(name, pre***REMOVED***x) {
		return "", ErrNotFound
	}
	p := name[len(pre***REMOVED***x):]

	v := db.s.version()
	defer v.release()

	numFilesPre***REMOVED***x := "num-***REMOVED***les-at-level"
	switch {
	case strings.HasPre***REMOVED***x(p, numFilesPre***REMOVED***x):
		var level uint
		var rest string
		n, _ := fmt.Sscanf(p[len(numFilesPre***REMOVED***x):], "%d%s", &level, &rest)
		if n != 1 {
			err = ErrNotFound
		} ***REMOVED*** {
