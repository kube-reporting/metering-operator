...
// ***REMOVED***ngerprintSeriesPair pairs a ***REMOVED***ngerprint with a memorySeries pointer.
type ***REMOVED***ngerprintSeriesPair struct {
	fp     model.Fingerprint
	series *memorySeries
}

// seriesMap maps ***REMOVED***ngerprints to memory series. All its methods are
...
// based on a pre***REMOVED***lled map, use an explicit initializer.
...
// get returns a memorySeries for a ***REMOVED***ngerprint. Return values have the same
...
func (sm *seriesMap) get(fp model.Fingerprint) (s *memorySeries, ok bool) {
	...
	// ***REMOVED***xed, revert to the usual defer idiom.
	sm.mtx.RUnlock()
	return
}
...
// channel will be closed once all ***REMOVED***ngerprints have been received. Not
// consuming all ***REMOVED***ngerprints from the channel will leak a goroutine. The
// semantics of concurrent modi***REMOVED***cation of seriesMap is the similar as the one
...
func (sm *seriesMap) iter() <-chan ***REMOVED***ngerprintSeriesPair {
	ch := make(chan ***REMOVED***ngerprintSeriesPair)
	go func() {
		sm.mtx.RLock()
		for fp, s := range sm.m {
			sm.mtx.RUnlock()
			ch <- ***REMOVED***ngerprintSeriesPair{fp, s}
			sm.mtx.RLock()
		}
		sm.mtx.RUnlock()
		close(ch)
...
// sortedFPs returns a sorted slice of all the ***REMOVED***ngerprints in the seriesMap.
...
type memorySeries struct {
	...
	// The index (within chunkDescs above) of the ***REMOVED***rst chunk.Desc that
	// points to a non-persisted chunk. If all chunks are persisted, then
	// persistWatermark == len(chunkDescs).
	persistWatermark int
	// The modi***REMOVED***cation time of the series ***REMOVED***le. The zero value of time.Time
	// is used to mark an unknown modi***REMOVED***cation time.
	modTime time.Time
	...
	// chunk on disk that corresponds to the ***REMOVED***rst chunk.Desc in memory. If
	...
	// upon ***REMOVED***rst persisting of a chunk in memory, the offset has to be
	// set).
	chunkDescsOffset int
	// The savedFirstTime ***REMOVED***eld is used as a fallback when the
	// chunkDescsOffset is not 0. It can be used to save the FirstTime of the
	// ***REMOVED***rst chunk before its chunk desc is evicted. In doubt, this ***REMOVED***eld is
	...
	// Whether the current head chunk has already been ***REMOVED***nished.  If true,
	// the current head chunk must not be modi***REMOVED***ed anymore.
...
// case, headChunkClosed is set to false, and ***REMOVED***rstTime and lastTime are both
// set to model.Earliest. The zero value for modTime can be used if the
// modi***REMOVED***cation time of the series ***REMOVED***le is unknown (e.g. if this is a genuinely
// new series).
func newMemorySeries(m model.Metric, chunkDescs []*chunk.Desc, modTime time.Time) (*memorySeries, error) {
	var err error
	***REMOVED***rstTime := model.Earliest
	lastTime := model.Earliest
	if len(chunkDescs) > 0 {
		***REMOVED***rstTime = chunkDescs[0].FirstTime()
	...
	return &memorySeries{
		...
		savedFirstTime:   ***REMOVED***rstTime,
		lastTime:         lastTime,
		persistWatermark: len(chunkDescs),
		modTime:          modTime,
	}, nil
...
//
// The caller must have locked the ***REMOVED***ngerprint of the series.
func (s *memorySeries) add(v model.SamplePair) (int, error) {
	if len(s.chunkDescs) == 0 || s.headChunkClosed {
		newHead := chunk.NewDesc(chunk.New(), v.Timestamp)
		s.chunkDescs = append(s.chunkDescs, newHead)
		s.headChunkClosed = false
	} ***REMOVED*** if s.headChunkUsedByIterator && s.head().RefCount() > 1 {
...
func (s *memorySeries) evictChunkDescs(lenToEvict int) {
	...
	s.savedFirstTime = s.***REMOVED***rstTime()
...
// ***REMOVED***ngerprint of the series.
...
func (s *memorySeries) preloadChunks(
	...
	for _, idx := range indexes {
		...
		cd.Pin(mss.evictRequests) // Have to pin everything ***REMOVED***rst to prevent immediate eviction on chunk loading.
		if cd.IsEvicted() {
...
// must be pinned).
//
// The caller must have locked the ***REMOVED***ngerprint of the memorySeries.
...
func (s *memorySeries) preloadChunksForRange(
	...
	***REMOVED***rstChunkDescTime := model.Latest
	if len(s.chunkDescs) > 0 {
		***REMOVED***rstChunkDescTime = s.chunkDescs[0].FirstTime()
	}
	if s.chunkDescsOffset != 0 && from.Before(***REMOVED***rstChunkDescTime) {
		...
		if s.chunkDescsOffset != -1 && len(cds) != s.chunkDescsOffset {
			return nopIter, fmt.Errorf(
				"unexpected number of chunk descs loaded for ***REMOVED***ngerprint %v: expected %d, got %d",
	...
	if len(s.chunkDescs) == 0 || through.Before(***REMOVED***rstChunkDescTime) {
		return nopIter, nil
	}

	// Find ***REMOVED***rst chunk with start time after "from".
	fromIdx := sort.Search(len(s.chunkDescs), func(i int) bool {
		return s.chunkDescs[i].FirstTime().After(from)
	})
	// Find ***REMOVED***rst chunk with start time after "through".
...
// locked the ***REMOVED***ngerprint of the memorySeries. This method will panic if this
// series has no chunk descriptors.
func (s *memorySeries) head() *chunk.Desc {
	return s.chunkDescs[len(s.chunkDescs)-1]
}

// ***REMOVED***rstTime returns the timestamp of the ***REMOVED***rst sample in the series.
//
// The caller must have locked the ***REMOVED***ngerprint of the memorySeries.
func (s *memorySeries) ***REMOVED***rstTime() model.Time {
...
func (it *memorySeriesIterator) ValueAtOrBeforeTime(t model.Time) model.SamplePair {
	...
	if i == len(it.chunks) {
		// Even the ***REMOVED***rst chunk starts after t.
		return model.ZeroSamplePair
	}
...
func (it *memorySeriesIterator) RangeValues(in metric.Interval) []model.SamplePair {
	// Find the ***REMOVED***rst chunk for which the ***REMOVED***rst sample is within the interval.
	i := sort.Search(len(it.chunks), func(i int) bool {
