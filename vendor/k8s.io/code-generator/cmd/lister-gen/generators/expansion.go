...
// expansionGenerator produces a ***REMOVED***le for a expansion interfaces.
...
func (g *expansionGenerator) GenerateType(c *generator.Context, t *types.Type, w io.Writer) error {
	...
	for _, t := range g.types {
		...
		if _, err := os.Stat(***REMOVED***lepath.Join(g.packagePath, strings.ToLower(t.Name.Name+"_expansion.go"))); os.IsNotExist(err) {
