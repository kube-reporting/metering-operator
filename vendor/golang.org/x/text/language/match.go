...
func newMatcher(supported []Tag, options []MatchOption) *matcher {
	...
	// - don't replace regions, but allow regions to be made more speci***REMOVED***c.
	...
	update := func(want, have uint16, conf Con***REMOVED***dence, force bool) {
...
// better match.
// To determine if the given pair is a better match, it ***REMOVED***rst computes the rough
// con***REMOVED***dence level. If this surpasses the current match, it will replace it and
...
func (m *bestMatch) update(have *haveTag, tag Tag, maxScript scriptID, maxRegion regionID) {
	...
	// Possible solutions:
	// - apply the primary locale rule ***REMOVED***rst to effectively disable region
	//   region distance if groups are de***REMOVED***ned.
	// - express the following errors in terms of grouping (if possible)
	// - ***REMOVED***nd another method of handling the following cases.
	// maximization of legacy: ***REMOVED***nd mo in
	//      "sr-Cyrl, sr-Latn, ro, ro-MD": have ro; want ro-MD (High)
	// region distance French: ***REMOVED***nd fr-US in
	//      "en, fr, fr-CA, fr-CH": have fr; want fr-CA (High)

	// Next we prefer smaller distances between regions, as de***REMOVED***ned by
	// regionDist.
	...
	// to leave it in as the ***REMOVED***nal tie-breaker, though, especially until the
...
// distance in the graph of region containments as de***REMOVED***ned in CLDR. It iterates
