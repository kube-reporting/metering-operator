...
const (
	...
	// series ***REMOVED***les are not synced if the adaptive sync strategy is used.
	...
	// intersect ***REMOVED***ngerprints. The rationale here is that looking up more
	// ***REMOVED***ngerprints has diminishing returns if we already have narrowed down
	// the possible ***REMOVED***ngerprints signi***REMOVED***cantly. It is then easier to simply
	// lookup the metrics for all the ***REMOVED***ngerprints and directly compare them
	// to the matchers. Since a ***REMOVED***ngerprint lookup for an Equal matcher is
...
// SyncStrategy is an enum to select a sync strategy for series ***REMOVED***les.
...
// A syncStrategy is a function that returns whether series ***REMOVED***les should be
...
type MemorySeriesStorage struct {
	...
	fpLocker   ****REMOVED***ngerprintLocker
...
type MemorySeriesStorageOptions struct {
	...
	PersistenceStoragePath     string        // Location of persistence ***REMOVED***les.
	...
	PedanticChecks             bool          // If dirty, perform crash-recovery checks on each series ***REMOVED***le.
	SyncStrategy               SyncStrategy  // Which sync strategy to apply to series ***REMOVED***les.
	MinShrinkRatio             float64       // Minimum ratio a series ***REMOVED***le has to shrink during truncation.
	NumMutexes                 int           // Number of mutexes used for stochastic ***REMOVED***ngerprint locking.
}
...
func NewMemorySeriesStorage(o *MemorySeriesStorageOptions) *MemorySeriesStorage {
	...
	s.targetHeapSizeBytes = prometheus.NewGaugeFunc(
		prometheus.GaugeOpts{
			...
			Help:      "The con***REMOVED***gured target heap size in bytes.",
		},
		func() float64 {
...
func (s *MemorySeriesStorage) Stop() error {
	...
	// One ***REMOVED***nal checkpoint of the series map and the head chunks.
...
// data from earlier than the con***REMOVED***gured start time.
...
// ***REMOVED***ngerprintsForLabelPair returns the ***REMOVED***ngerprints with the given
// LabelPair. If intersectWith is non-nil, the method will only return
// ***REMOVED***ngerprints that are also contained in intersectsWith. If mergeWith is
// non-nil, the found ***REMOVED***ngerprints are added to the given map. The returned map
// is the same as the given one.
func (s *MemorySeriesStorage) ***REMOVED***ngerprintsForLabelPair(
	...
	for _, fp := range s.persistence.***REMOVED***ngerprintsForLabelPair(pair) {
...
func (s *MemorySeriesStorage) candidateFPsForLabelMatchers(
	...
	for ; matcherIdx < len(matchers) && (candidateFPs == nil || len(candidateFPs) > fpEqualMatchThreshold); matcherIdx++ {
		...
		candidateFPs = s.***REMOVED***ngerprintsForLabelPair(
			model.LabelPair{
				Name:  m.Name,
				Value: m.Value,
			},
	...
	for ; matcherIdx < len(matchers) && (candidateFPs == nil || len(candidateFPs) > fpOtherMatchThreshold); matcherIdx++ {
		...
		for _, lv := range lvs {
			s.***REMOVED***ngerprintsForLabelPair(
				model.LabelPair{
					Name:  m.Name,
					Value: lv,
				},
...
FPLoop:
	for fp := range candidateFPs {
		...
		result = append(result, ***REMOVED***ngerprintSeriesPair{fp, series})
	}
	return result, nil
}
...
// metricForRange returns the metric for the given ***REMOVED***ngerprint if the
...
func (s *MemorySeriesStorage) metricForRange(
	...
	series, ok := s.fpToSeries.get(fp)
	if ok {
		if series.lastTime.Before(from) || series.***REMOVED***rstTime().After(through) {
	...
	if from.After(model.Earliest) || through.Before(model.Latest) {
		// The range lookup is relatively cheap, so let's do it ***REMOVED***rst if
		// we have a chance the archived metric is not in the range.
		has, ***REMOVED***rst, last := s.persistence.hasArchivedMetric(fp)
		if !has {
			s.nonExistentSeriesMatches.Inc()
			return nil, nil, false
		}
		if ***REMOVED***rst.After(through) || last.Before(from) {
			return nil, nil, false
		}
	}
...
// is sent to s.throttled. If that happens for the ***REMOVED***rst time, an Error is
// logged that the storage is now throttled. As long as signals continues to be
// sent via s.throttled at least once per minute, nothing ***REMOVED*** is logged. Once
...
func (s *MemorySeriesStorage) getOrCreateSeries(fp model.Fingerprint, m model.Metric) (*memorySeries, error) {
	...
	if !ok {
		...
		if err != nil {
			log.Errorf("Error unarchiving ***REMOVED***ngerprint %v (metric %v): %v", fp, m, err)
			return nil, err
		}
		if unarchived {
			...
			if err == nil && len(cds) == 0 {
				err = fmt.Errorf("unarchived ***REMOVED***ngerprint %v (metric %v) has no chunks on disk", fp, m)
			}
			if err != nil {
...
func (s *MemorySeriesStorage) preloadChunksForRange(
	pair ***REMOVED***ngerprintSeriesPair,
	from model.Time, through model.Time,
) SeriesIterator {
...
func (s *MemorySeriesStorage) handleEvictList() {
	...
	// we would handle the evict list after each ***REMOVED***nished GC cycle, but I
...
//    h(target):    Con***REMOVED***gured target heap size.
//
...
//
// - In the latter case, the persist urgency might be increased. The ***REMOVED***nal value
//   is the following:
//
//            n(toEvict)
//   MAX( p, ------------ )
...
// to the con***REMOVED***gured target size, and how big the pool of chunks to persist is
...
// another ***REMOVED***ngerprint so that we will process all ***REMOVED***ngerprints in a tenth of
// s.dropAfter assuming that the system is doing nothing ***REMOVED***, e.g. if we want
// to drop chunks after 40h, we want to cycle through all ***REMOVED***ngerprints within
// 4h.  The estimation is based on the total number of ***REMOVED***ngerprints as passed
...
// cycleThroughMemoryFingerprints returns a channel that emits ***REMOVED***ngerprints for
// series in memory in a throttled fashion. It continues to cycle through all
// ***REMOVED***ngerprints in memory until s.loopStopping is closed.
	...
	go func() {
		...
		***REMOVED***rstPass := true
		...
		for {
			...
			if ***REMOVED***rstPass && len(fps) > 0 {
				// Start ***REMOVED***rst pass at a random location in the
			...
			if count > 0 {
				...
				log.Infof(
					"Completed %s maintenance sweep through %d in-memory ***REMOVED***ngerprints in %v.",
					msg, count, time.Since(begin),
				)
			}
			***REMOVED***rstPass = false
		}
	}()
...
// cycleThroughArchivedFingerprints returns a channel that emits ***REMOVED***ngerprints
// for archived series in a throttled fashion. It continues to cycle through all
// archived ***REMOVED***ngerprints until s.loopStopping is closed.
func (s *MemorySeriesStorage) cycleThroughArchivedFingerprints() chan model.Fingerprint {
	...
	go func() {
		...
		for {
			archivedFPs, err := s.persistence.***REMOVED***ngerprintsModi***REMOVED***edBefore(
				model.Now().Add(-s.dropAfter),
			)
			if err != nil {
				log.Error("Failed to lookup archived ***REMOVED***ngerprint ranges: ", err)
				s.waitForNextFP(0, 1)
				continue
			}
			// Initial wait, also important if there are no FPs yet.
			...
			if len(archivedFPs) > 0 {
				log.Infof(
					"Completed maintenance sweep through %d archived ***REMOVED***ngerprints in %v.",
...
func (s *MemorySeriesStorage) loop() {
	...
	// checkpointing there will still be suf***REMOVED***cient progress on maintenance.
...
//
// The method ***REMOVED***rst closes the head chunk if it was not touched for the duration
// of headChunkTimeout.
//
...
// ***REMOVED***le does not require a disk seek.)
//
// - If any chunks need to be purged (but not all of them), it purges those
// chunks from memory and rewrites the series ***REMOVED***le on disk, leaving out the
...
// those chunks are simply appended to the existing series ***REMOVED***le (or the ***REMOVED***le is
...
func (s *MemorySeriesStorage) maintainMemorySeries(
	...
	if iOldestNotEvicted == -1 && model.Now().Sub(series.lastTime) > s.headChunkTimeout {
		s.fpToSeries.del(fp)
		s.memorySeries.Dec()
		s.persistence.archiveMetric(fp, series.metric, series.***REMOVED***rstTime(), series.lastTime)
		...
		if oldWatermark < int64(series.lastTime) {
			if !atomic.CompareAndSwapInt64(
				...
				panic("s.archiveHighWatermark modi***REMOVED***ed outside of maintainMemorySeries")
			}
		}
		return
	}
...
// writeMemorySeries (re-)writes a memory series ***REMOVED***le. While doing so, it drops
// chunks older than beforeTime from both the series ***REMOVED***le (if it exists) as well
// as from memory. The provided chunksToPersist are appended to the newly
// written series ***REMOVED***le. If no chunks need to be purged, but chunksToPersist is
// not empty, those chunks are simply appended to the series ***REMOVED***le. If the series
...
func (s *MemorySeriesStorage) writeMemorySeries(
	...
	if !series.***REMOVED***rstTime().Before(beforeTime) {
		...
		if series.chunkDescsOffset == -1 {
			// This is the ***REMOVED***rst chunk persisted for a newly created
...
func (s *MemorySeriesStorage) maintainArchivedSeries(fp model.Fingerprint, beforeTime model.Time) {
	...
	has, ***REMOVED***rstTime, lastTime := s.persistence.hasArchivedMetric(fp)
	if !has || !***REMOVED***rstTime.Before(beforeTime) {
	...
	if err := s.persistence.updateArchivedTimeRange(fp, newFirstTime, lastTime); err != nil {
		s.persistErrors.Inc()
		log.Errorf("Error updating archived time range for ***REMOVED***ngerprint %v: %s", fp, err)
	}
}
...
// quarantineSeries registers the provided ***REMOVED***ngerprint for quarantining. It
...
// dropped.
//
// Quarantining means that the series ***REMOVED***le is moved to the orphaned directory,
...
// provided, the series ***REMOVED***le will not be deleted completely, but moved to the
// orphaned directory with the reason and the metric in a hint ***REMOVED***le. The
...
func (s *MemorySeriesStorage) purgeSeries(fp model.Fingerprint, m model.Metric, quarantineReason error) {
	...
	// Attempt to delete/quarantine the series ***REMOVED***le in any case.
	if quarantineReason == nil {
		// No reason stated, simply delete the ***REMOVED***le.
		if _, err := s.persistence.deleteSeriesFile(fp); err != nil {
			log.
				With("***REMOVED***ngerprint", fp).
				With("metric", m).
				With("error", err).
				Error("Error deleting series ***REMOVED***le.")
		}
		...
		if err := s.persistence.quarantineSeriesFile(fp, quarantineReason, m); err == nil {
			...
			log.
				...
				With("error", err).
				Error("Error quarantining series ***REMOVED***le.")
		}
	}
