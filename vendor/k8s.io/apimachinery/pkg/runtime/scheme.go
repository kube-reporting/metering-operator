...
// Converts the given ***REMOVED***eld label and value for an kind ***REMOVED***eld selector from
// versioned representation to an unversioned one.
func (s *Scheme) ConvertFieldLabel(version, kind, label, value string) (string, string, error) {
	if s.***REMOVED***eldLabelConversionFuncs[version] == nil {
		return "", "", fmt.Errorf("No ***REMOVED***eld label conversion function found for version: %s", version)
	}
	conversionFunc, ok := s.***REMOVED***eldLabelConversionFuncs[version][kind]
	if !ok {
		return "", "", fmt.Errorf("No ***REMOVED***eld label conversion function found for version %s and kind %s", version, kind)
	}
