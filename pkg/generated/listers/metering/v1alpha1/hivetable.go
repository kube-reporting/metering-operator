...
type HiveTableLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.HiveTable, err error)
...
// List lists all HiveTables in the indexer.
func (s *hiveTableLister) List(selector labels.Selector) (ret []*v1alpha1.HiveTable, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.HiveTable))
	})
	return ret, err
}
...
type HiveTableNamespaceLister interface {
	...
	Get(name string) (*v1alpha1.HiveTable, error)
...
// List lists all HiveTables in the indexer for a given namespace.
func (s hiveTableNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.HiveTable, err error) {
...
// Get retrieves the HiveTable from the indexer for a given namespace and name.
func (s hiveTableNamespaceLister) Get(name string) (*v1alpha1.HiveTable, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("hivetable"), name)
	}
	return obj.(*v1alpha1.HiveTable), nil
}
