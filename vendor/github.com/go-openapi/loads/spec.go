...
// JSONDoc loads a json document from either a ***REMOVED***le or a remote url
...
// Expanded expands the ref ***REMOVED***elds in the spec document and returns a new spec document
...
// ResetDe***REMOVED***nitions gives a shallow copy with the models reset
func (d *Document) ResetDe***REMOVED***nitions() *Document {
	defs := make(map[string]spec.Schema, len(d.origSpec.De***REMOVED***nitions))
	for k, v := range d.origSpec.De***REMOVED***nitions {
		defs[k] = v
	}

	d.spec.De***REMOVED***nitions = defs
	return d
}
