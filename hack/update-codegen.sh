...
function generate_group() {
  ...
  ${deepcopygen} --input-dirs ${APIS_PKG}/${GROUP_NAME}/${VERSION} -O zz_generated.deepcopy --go-header-file $SCRIPT_ROOT/vendor/k8s.io/gengo/boilerplate/no-boilerplate.go.txt
  ...
  ${clientgen} --clientset-name="versioned" --input-base ${APIS_PKG} --input ${GROUP_NAME}/${VERSION} --clientset-path ${CLIENTSET_PKG} --go-header-file $SCRIPT_ROOT/vendor/k8s.io/gengo/boilerplate/no-boilerplate.go.txt
  ...
  ${listergen} --input-dirs ${APIS_PKG}/${GROUP_NAME} --input-dirs ${APIS_PKG}/${GROUP_NAME}/${VERSION} --output-package ${LISTERS_PKG} --go-header-file $SCRIPT_ROOT/vendor/k8s.io/gengo/boilerplate/no-boilerplate.go.txt
  ...
  ${informergen} \
    ...
    --go-header-file $SCRIPT_ROOT/vendor/k8s.io/gengo/boilerplate/no-boilerplate.go.txt
}
