...
// A Pro***REMOVED***le represents a set of rules for normalizing and validating strings in
...
// NewIdenti***REMOVED***er creates a new PRECIS pro***REMOVED***le based on the Identi***REMOVED***er string
// class. Pro***REMOVED***les created from this class are suitable for use where safety is
// prioritized over expressiveness like network identi***REMOVED***ers, user accounts, chat
// rooms, and ***REMOVED***le names.
func NewIdenti***REMOVED***er(opts ...Option) *Pro***REMOVED***le {
	return &Pro***REMOVED***le{
		options: getOpts(opts...),
		class:   identi***REMOVED***er,
	}
}

// NewFreeform creates a new PRECIS pro***REMOVED***le based on the Freeform string class.
// Pro***REMOVED***les created from this class are suitable for use where expressiveness is
// prioritized over safety like passwords, and display-elements such as
// nicknames in a chat room.
func NewFreeform(opts ...Option) *Pro***REMOVED***le {
	return &Pro***REMOVED***le{
		options: getOpts(opts...),
		class:   freeform,
	}
...
func (p *Pro***REMOVED***le) NewTransformer() *Transformer {
	var ts []transform.Transformer

	// These transforms are applied in the order de***REMOVED***ned in
...
// TODO: make this a method on pro***REMOVED***le.

func (b *buffers) enforce(p *Pro***REMOVED***le, src []byte, comparing bool) (str []byte, err error) {
	b.src = src
...
func (p *Pro***REMOVED***le) Append(dst, src []byte) ([]byte, error) {
	var buf buffers
...
func processBytes(p *Pro***REMOVED***le, b []byte, key bool) ([]byte, error) {
	var buf buffers
...
// Bytes returns a new byte slice with the result of applying the pro***REMOVED***le to b.
func (p *Pro***REMOVED***le) Bytes(b []byte) ([]byte, error) {
...
func (p *Pro***REMOVED***le) AppendCompareKey(dst, src []byte) ([]byte, error) {
	var buf buffers
...
func processString(p *Pro***REMOVED***le, s string, key bool) (string, error) {
	var buf buffers
...
// String returns a string with the result of applying the pro***REMOVED***le to s.
func (p *Pro***REMOVED***le) String(s string) (string, error) {
	return processString(p, s, false)
}

// CompareKey returns a string that can be used for comparison, hashing, or
// collation.
func (p *Pro***REMOVED***le) CompareKey(s string) (string, error) {
...
// is false.
func (p *Pro***REMOVED***le) Compare(a, b string) bool {
	var buf buffers
...
// underlying pro***REMOVED***le's string class and not disallowed by any pro***REMOVED***le speci***REMOVED***c
// rules.
func (p *Pro***REMOVED***le) Allowed() runes.Set {
...
func (c *checker) span(src []byte, atEOF bool) (n int, err error) {
	for n < len(src) {
		...
		if c.termBits != 0 {
			...
			if c.beforeBits&c.termBits != 0 {
				c.termBits = 0
				c.acceptBits = 0
			} ***REMOVED*** if c.beforeBits&c.acceptBits == 0 {
	...
	if m := c.beforeBits >> ***REMOVED***nalShift; c.beforeBits&m != m || c.termBits != 0 {
		err = errContext
	}
	return n, err
}
