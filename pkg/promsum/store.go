...
// FileStore is a simple implementation of Store which writes files to disk.
...
func Name(rng cb.Range, suffix string) string {
	return fmt.Sprintf("%d-%d-%x.json", rng.Start.Unix(), rng.End.Unix(), suffix)
}
...
func PathWithinRange(path string, rng cb.Range) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("could not determine record range from filename for '%s': %v", path, err)
	}
