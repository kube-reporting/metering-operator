...
var (
	outputFile = flag.String("out", "tables.go", "output ***REMOVED***le")
)
...
// getWidthData calls f for every entry for which it is de***REMOVED***ned.
//
...
func getWidthData(f func(r rune, tag elem, alt rune)) {
	// Set the default values for Uni***REMOVED***ed Ideographs. In line with Annex 11,
	// we encode full ranges instead of the de***REMOVED***ned runes in Uni***REMOVED***ed_Ideograph.
	for _, b := range []struct{ lo, hi rune }{
		{0x4E00, 0x9FFF},   // the CJK Uni***REMOVED***ed Ideographs block,
		{0x3400, 0x4DBF},   // the CJK Uni***REMOVED***ed Ideographs Externsion A block,
