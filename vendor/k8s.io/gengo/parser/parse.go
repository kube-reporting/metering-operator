...
func New() *Builder {
	...
	if c.GOROOT == "" {
		if p, err := exec.Command("which", "go").CombinedOutput(); err == nil {
			...
			glog.Warningf("Warning: $GOROOT not set, and unable to run `which go` to ***REMOVED***nd it: %v\n", err)
		}
	}
...
func (b *Builder) AddDirRecursive(dir string) error {
	...
	fn := func(path string, info os.FileInfo, err error) error {
		if info != nil && info.IsDir() {
			rel := strings.TrimPre***REMOVED***x(path, pre***REMOVED***x)
			if rel != "" {
				// Make a pkg path.
				pkg := ***REMOVED***lepath.Join(string(canonicalizeImportPath(b.buildPackages[dir].ImportPath)), rel)
...
func (b *Builder) typeCheckPackage(pkgPath importPathString) (*tc.Package, error) {
	...
	if !ok {
		return nil, fmt.Errorf("No ***REMOVED***les for pkg %q: %#v", pkgPath, b.parsed)
	}
...
func (b *Builder) ***REMOVED***ndTypesIn(pkgPath importPathString, u *types.Universe) error {
	glog.V(5).Infof("***REMOVED***ndTypesIn %s", pkgPath)
	...
	if !b.userRequested[pkgPath] {
		...
		glog.V(5).Infof("***REMOVED***ndTypesIn %s: package is not user requested", pkgPath)
		return nil
	}
	...
	for _, f := range b.parsed[pkgPath] {
		if strings.HasSuf***REMOVED***x(f.name, "/doc.go") {
