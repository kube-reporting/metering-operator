	...
	for {
		...
		case datastore := <-c.prestoTablePartitionQueue:
			if datastore.Spec.DataStoreSource.AWSBilling == nil {
				logger.Errorf("incorrectly con***REMOVED***gured datastore sent to the presto table worker: %q", datastore.Name)
				continue
			}
...
func (c *Chargeback) updateAWSBillingPartitions(logger log.FieldLogger, datastore *cbTypes.ReportDataStore) error {
	...
	manifestRetriever, err := aws.NewManifestRetriever(datastore.Spec.DataStoreSource.AWSBilling.Source.Bucket, datastore.Spec.DataStoreSource.AWSBilling.Source.Pre***REMOVED***x)
	if err != nil {
		return err
	}
