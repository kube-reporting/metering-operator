...
// See LICENCE ***REMOVED***le for details.

// Package ratelimit provides an ef***REMOVED***cient token bucket implementation
...
// (***REMOVED***llInterval) apart. On each tick a number of tokens (quantum) are
// added to the bucket.
//
...
// ***REMOVED***ve tokens an hour, replenished on the hour".

// Bucket represents a token bucket that ***REMOVED***lls at a predetermined rate.
// Methods on Bucket may be called concurrently.
type Bucket struct {
	clock Clock

	// startTime holds the moment when the bucket was
	// ***REMOVED***rst created and ticks began.
	startTime time.Time
	...
	// ***REMOVED***llInterval holds the interval between each tick.
	***REMOVED***llInterval time.Duration

	// mu guards the ***REMOVED***elds below it.
	mu sync.Mutex
...
// NewBucket returns a new token bucket that ***REMOVED***lls at the
// rate of one token every ***REMOVED***llInterval, up to the given
// maximum capacity. Both arguments must be
// positive. The bucket is initially full.
func NewBucket(***REMOVED***llInterval time.Duration, capacity int64) *Bucket {
	return NewBucketWithClock(***REMOVED***llInterval, capacity, nil)
}

// NewBucketWithClock is identical to NewBucket but injects a testable clock
// interface.
func NewBucketWithClock(***REMOVED***llInterval time.Duration, capacity int64, clock Clock) *Bucket {
	return NewBucketWithQuantumAndClock(***REMOVED***llInterval, capacity, 1, clock)
}

// rateMargin specifes the allowed variance of actual
// rate from speci***REMOVED***ed rate. 1% seems reasonable.
const rateMargin = 0.01

// NewBucketWithRate returns a token bucket that ***REMOVED***lls the bucket
// at the rate of rate tokens per second up to the given
// maximum capacity. Because of limited clock resolution,
// at high rates, the actual rate may be up to 1% different from the
// speci***REMOVED***ed rate.
...
func NewBucketWithRateAndClock(rate float64, capacity int64, clock Clock) *Bucket {
	...
	for quantum := int64(1); quantum < 1<<50; quantum = nextQuantum(quantum) {
		***REMOVED***llInterval := time.Duration(1e9 * float64(quantum) / rate)
		if ***REMOVED***llInterval <= 0 {
			continue
		}
		tb.***REMOVED***llInterval = ***REMOVED***llInterval
		tb.quantum = quantum
		if diff := math.Abs(tb.Rate() - rate); diff/rate <= rateMargin {
			return tb
		}
	}
	panic("cannot ***REMOVED***nd suitable quantum for " + strconv.FormatFloat(rate, 'g', -1, 64))
}

// nextQuantum returns the next quantum to try after q.
// We grow the quantum exponentially, but slowly, so we
// get a good ***REMOVED***t in the lower numbers.
...
// the speci***REMOVED***cation of the quantum size - quantum tokens
// are added every ***REMOVED***llInterval.
func NewBucketWithQuantum(***REMOVED***llInterval time.Duration, capacity, quantum int64) *Bucket {
	return NewBucketWithQuantumAndClock(***REMOVED***llInterval, capacity, quantum, nil)
}
...
func NewBucketWithQuantumAndClock(***REMOVED***llInterval time.Duration, capacity, quantum int64, clock Clock) *Bucket {
	if clock == nil {
		clock = realClock{}
	}
	if ***REMOVED***llInterval <= 0 {
		panic("token bucket ***REMOVED***ll interval is not > 0")
	}
	if capacity <= 0 {
	...
	return &Bucket{
		...
		startTime:       clock.Now(),
		latestTick:      0,
		***REMOVED***llInterval:    ***REMOVED***llInterval,
		capacity:        capacity,
		quantum:         quantum,
...
const in***REMOVED***nityDuration time.Duration = 0x7fffffffffffffff
...
func (tb *Bucket) Take(count int64) time.Duration {
	tb.mu.Lock()
	defer tb.mu.Unlock()
	d, _ := tb.take(tb.clock.Now(), count, in***REMOVED***nityDuration)
	return d
}
...
// Rate returns the ***REMOVED***ll rate of the bucket, in tokens per second.
func (tb *Bucket) Rate() float64 {
	return 1e9 * float64(tb.quantum) / float64(tb.***REMOVED***llInterval)
}
...
func (tb *Bucket) take(now time.Time, count int64, maxWait time.Duration) (time.Duration, bool) {
	...
	endTime := tb.startTime.Add(time.Duration(endTick) * tb.***REMOVED***llInterval)
...
func (tb *Bucket) currentTick(now time.Time) int64 {
	return int64(now.Sub(tb.startTime) / tb.***REMOVED***llInterval)
}
