...
func NewDefaultRESTMapperFromScheme(defaultGroupVersions []schema.GroupVersion, interfacesFunc VersionInterfacesFunc,
	importPathPre***REMOVED***x string, ignoredKinds, rootScoped sets.String, scheme *runtime.Scheme) *DefaultRESTMapper {
	...
	for _, gv := range defaultGroupVersions {
		for kind, oType := range scheme.KnownTypes(gv) {
			...
			if !strings.Contains(oType.PkgPath(), importPathPre***REMOVED***x) || ignoredKinds.Has(kind) {
				continue
			}
