		...
		if pkgNeedsGeneration {
			...
			packages = append(packages,
				&generator.DefaultPackage{
					PackageName: strings.Split(***REMOVED***lepath.Base(pkg.Path), ".")[0],
					PackagePath: path,
					HeaderText:  header,
...
// genDeepCopy produces a ***REMOVED***le with autogenerated deep-copy functions.
...
// if the type does not match. This allows more ef***REMOVED***cient deep copy
// implementations to be de***REMOVED***ned by the type's author.  The correct signature
// for a type T is:
//    func (t T) DeepCopy() T
// or:
//    func (t *T) DeepCopy() *T
...
// if the type is wrong. DeepCopyInto allows more ef***REMOVED***cient deep copy
// implementations to be de***REMOVED***ned by the type's author.  The correct signature
// for a type T is:
...
func isRootedUnder(pkg string, roots []string) bool {
	...
	for _, root := range roots {
		if strings.HasPre***REMOVED***x(pkg, root+"/") {
			return true
		}
	}
	return false
...
func copyableType(t *types.Type) bool {
	...
	if t.Kind == types.Alias {
		// if the underlying built-in is not deepcopy-able, deepcopy is opt-in through de***REMOVED***nition of custom methods.
...
func (g *genDeepCopy) isOtherPackage(pkg string) bool {
	...
	if strings.HasSuf***REMOVED***x(pkg, "\""+g.targetPackage+"\"") {
		return false
	}
	return true
}
...
func (g *genDeepCopy) deepCopyableInterfacesInner(c *generator.Context, t *types.Type) ([]*types.Type, error) {
	...
	var ts []*types.Type
	for _, intf := range intfs {
		t := types.ParseFullyQuali***REMOVED***edName(intf)
		c.AddDir(t.Package)
...
func (g *genDeepCopy) doMap(t *types.Type, sw *generator.SnippetWriter) {
	...
	if ut.Key.IsAssignable() {
		...
		case dc != nil || dci != nil:
			...
			// Note: a DeepCopy exists because it is added if DeepCopyInto is manually de***REMOVED***ned
		...
		case uet.Kind == types.Interface:
			sw.Do("for key, val := range *in {\n", nil)
			sw.Do("if val == nil {(*out)[key]=nil} ***REMOVED*** {\n", nil)
		...
		default:
			...
			} ***REMOVED*** if uet.Kind == types.Slice && underlyingType(uet.Elem).Kind == types.Builtin {
				sw.Do("if val==nil { (*out)[key]=nil } ***REMOVED*** {\n", nil)
				sw.Do("(*out)[key] = make($.|raw$, len(val))\n", uet)
				sw.Do("copy((*out)[key], val)\n", nil)
				sw.Do("}\n", nil)
			} ***REMOVED*** if uet.Kind == types.Pointer {
				sw.Do("if val==nil { (*out)[key]=nil } ***REMOVED*** {\n", nil)
...
func (g *genDeepCopy) doSlice(t *types.Type, sw *generator.SnippetWriter) {
	...
	if deepCopyMethodOrDie(ut.Elem) != nil || deepCopyIntoMethodOrDie(ut.Elem) != nil {
		...
		// Note: a DeepCopyInto exists because it is added if DeepCopy is manually de***REMOVED***ned
		sw.Do("(*in)[i].DeepCopyInto(&(*out)[i])\n", nil)
		sw.Do("}\n", nil)
	} ***REMOVED*** if uet.Kind == types.Builtin || uet.IsAssignable() {
		sw.Do("copy(*out, *in)\n", nil)
	} ***REMOVED*** {
		sw.Do("for i := range *in {\n", nil)
		if uet.Kind == types.Slice || uet.Kind == types.Map || deepCopyMethodOrDie(ut.Elem) != nil || deepCopyIntoMethodOrDie(ut.Elem) != nil {
			...
			sw.Do("}\n", nil)
		} ***REMOVED*** if uet.Kind == types.Interface {
			sw.Do("if (*in)[i] == nil {(*out)[i]=nil} ***REMOVED*** {\n", nil)
			// Note: if t.Elem has been an alias "J" of an interface "I" in Go, we will see it
			// as kind Interface of name "J" here, i.e. generate val.DeepCopyJ(). The golang
			// parser does not give us the underlying interface name. So we cannot do any better.
			...
			sw.Do("(*in)[i].DeepCopyInto((*out)[i])\n", nil)
			sw.Do("}\n", nil)
		} ***REMOVED*** if uet.Kind == types.Struct {
			sw.Do("(*in)[i].DeepCopyInto(&(*out)[i])\n", nil)
		} ***REMOVED*** {
...
func (g *genDeepCopy) doStruct(t *types.Type, sw *generator.SnippetWriter) {
	...
	// Now ***REMOVED***x-up ***REMOVED***elds as needed.
	for _, m := range ut.Members {
		...
		case uft.Kind == types.Interface:
			sw.Do("if in.$.name$ == nil {out.$.name$=nil} ***REMOVED*** {\n", args)
			// Note: if t.Elem has been an alias "J" of an interface "I" in Go, we will see it
			// as kind Interface of name "J" here, i.e. generate val.DeepCopyJ(). The golang
			// parser does not give us the underlying interface name. So we cannot do any better.
...
func (g *genDeepCopy) doPointer(t *types.Type, sw *generator.SnippetWriter) {
	...
	sw.Do("if *in == nil { *out = nil } ***REMOVED*** {\n", t)
	dc, dci := deepCopyMethodOrDie(ut.Elem), deepCopyIntoMethodOrDie(ut.Elem)
	if dc != nil || dci != nil {
		rightPointer := !isReference(ut.Elem)
		if dc != nil {
