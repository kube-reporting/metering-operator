...
func apiScheduledReportV2URLFull(reportName string) string {
	return path.Join(APIV2ScheduledReportsEndpointPre***REMOVED***x, reportName, "full")
}

//for v2 endpoints TableHidden
func apiScheduledReportV2URLTable(reportName string) string {
	return path.Join(APIV2ScheduledReportsEndpointPre***REMOVED***x, reportName, "table")
}
...
func TestAPIV2ScheduledReportsFull(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"report-name-not-speci***REMOVED***ed": {
			reportName:            "",
			apiPath:               APIV2ScheduledReportsEndpointPre***REMOVED***x + "//full" + testFormat,
			expectedStatusCode:    http.StatusBadRequest,
			expectedAPIError:      "the following ***REMOVED***elds are missing or empty: name",
			reportResultsGetter:   &fakeReportResultsGetter{},
			prometheusMetricsRepo: &fakePrometheusMetricsRepo{},
...
func TestAPIV2ScheduledReportsTable(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"report-name-not-speci***REMOVED***ed": {
			reportName:            "",
			apiPath:               APIV2ScheduledReportsEndpointPre***REMOVED***x + "//table" + testFormat,
			expectedStatusCode:    http.StatusBadRequest,
			expectedAPIError:      "the following ***REMOVED***elds are missing or empty: name",
			reportResultsGetter:   &fakeReportResultsGetter{},
			prometheusMetricsRepo: &fakePrometheusMetricsRepo{},
