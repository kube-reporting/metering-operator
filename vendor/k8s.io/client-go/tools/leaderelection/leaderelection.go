...
func (le *LeaderElector) Run() {
	...
	go le.con***REMOVED***g.Callbacks.OnStartedLeading(stop)
	le.renew()
	close(stop)
}
...
// fails to validate.
func RunOrDie(lec LeaderElectionCon***REMOVED***g) {
...
func (le *LeaderElector) acquire() {
	...
	wait.JitterUntil(func() {
		...
		close(stop)
	}, le.con***REMOVED***g.RetryPeriod, JitterFactor, true, stop)
}
...
func (le *LeaderElector) renew() {
	stop := make(chan struct{})
	wait.Until(func() {
		err := wait.Poll(le.con***REMOVED***g.RetryPeriod, le.con***REMOVED***g.RenewDeadline, func() (bool, error) {
...
func (le *LeaderElector) tryAcquireOrRenew() bool {
	...
	if err != nil {
		if !errors.IsNotFound(err) {
			glog.Errorf("error retrieving resource lock %v: %v", le.con***REMOVED***g.Lock.Describe(), err)
			return false
		}
	...
	if le.observedTime.Add(le.con***REMOVED***g.LeaseDuration).After(now.Time) &&
		oldLeaderElectionRecord.HolderIdentity != le.con***REMOVED***g.Lock.Identity() {
	...
	if oldLeaderElectionRecord.HolderIdentity == le.con***REMOVED***g.Lock.Identity() {
...
func (l *LeaderElector) maybeReportTransition() {
	...
	if l.con***REMOVED***g.Callbacks.OnNewLeader != nil {
		go l.con***REMOVED***g.Callbacks.OnNewLeader(l.reportedLeader)
	}
}
