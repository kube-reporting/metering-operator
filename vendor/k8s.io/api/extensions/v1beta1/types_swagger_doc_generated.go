...
var map_AllowedHostPath = map[string]string{
	"":           "de***REMOVED***nes the host volume conditions that will be enabled by a policy for pods to use. It requires the path pre***REMOVED***x to be de***REMOVED***ned.",
	"pathPre***REMOVED***x": "is the path pre***REMOVED***x that the host volume must match. It does not support `*`. Trailing slashes are trimmed when validating the path pre***REMOVED***x with a host path.\n\nExamples: `/foo` would allow `/foo`, `/foo/` and `/foo/bar` `/foo` would not allow `/food` or `/etc/foo`",
}
...
var map_DeploymentSpec = map[string]string{
	...
	"revisionHistoryLimit":    "The number of old ReplicaSets to retain to allow rollback. This is a pointer to distinguish between explicit zero and not speci***REMOVED***ed.",
...
var map_HostPortRange = map[string]string{
	"":    "Host Port Range de***REMOVED***nes a range of host ports that will be enabled by a policy for pods to use.  It requires both the start and end to be de***REMOVED***ned.",
...
var map_NetworkPolicyPeer = map[string]string{
	"podSelector":       "This is a label selector which selects Pods in this namespace. This ***REMOVED***eld follows standard label selector semantics. If present but empty, this selector selects all pods in this namespace.",
	"namespaceSelector": "Selects Namespaces using cluster scoped-labels.  This matches all pods in all namespaces selected by this label selector. This ***REMOVED***eld follows standard label selector semantics. If present but empty, this selector selects all namespaces.",
	"ipBlock":           "IPBlock de***REMOVED***nes policy on a particular IPBlock",
}
...
var map_NetworkPolicyPort = map[string]string{
	"protocol": "Optional.  The protocol (TCP or UDP) which traf***REMOVED***c must match. If not speci***REMOVED***ed, this ***REMOVED***eld defaults to TCP.",
...
var map_PodSecurityPolicySpec = map[string]string{
	...
	"defaultAddCapabilities":          "DefaultAddCapabilities is the default set of capabilities that will be added to the container unless the pod spec speci***REMOVED***cally drops the capability.  You may not list a capabiility in both DefaultAddCapabilities and RequiredDropCapabilities.",
...
var map_ReplicaSet = map[string]string{
	"":         "DEPRECATED - This group version of ReplicaSet is deprecated by apps/v1beta2/ReplicaSet. See the release notes for more information. ReplicaSet represents the con***REMOVED***guration of a ReplicaSet.",
