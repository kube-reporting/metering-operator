...
func (deploy *Deployer) deleteMeteringConfig() error {
...
func (deploy *Deployer) deleteMeteringResources() error {
	err := deploy.deleteMeteringDeployment(filepath.Join(deploy.ManifestLocation, meteringDeploymentFile))
	if err != nil {
		return fmt.Errorf("Failed to delete the metering service account: %v", err)
	}

	err = deploy.deleteMeteringServiceAccount(filepath.Join(deploy.ManifestLocation, meteringServiceAccountFile))
	if err != nil {
		return fmt.Errorf("Failed to delete the metering service account: %v", err)
	}

	err = deploy.deleteMeteringRole(filepath.Join(deploy.ManifestLocation, meteringRoleFile))
	if err != nil {
		return fmt.Errorf("Failed to delete the metering role: %v", err)
	}

	err = deploy.deleteMeteringRoleBinding(filepath.Join(deploy.ManifestLocation, meteringRoleBindingFile))
	if err != nil {
		return fmt.Errorf("Failed to delete the metering role binding: %v", err)
	}

	if deploy.DeleteCRB {
		err = deploy.deleteMeteringClusterRole(filepath.Join(deploy.ManifestLocation, meteringClusterRoleFile))
		if err != nil {
			return fmt.Errorf("Failed to delete the metering cluster role: %v", err)
		}

		err = deploy.deleteMeteringClusterRoleBinding(filepath.Join(deploy.ManifestLocation, meteringClusterRoleBindingFile))
		if err != nil {
