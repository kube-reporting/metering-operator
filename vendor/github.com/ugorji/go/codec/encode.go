...
// AsSymbolFlag de***REMOVED***nes what should be encoded as symbols.
type AsSymbolFlag uint8

const (
	// AsSymbolDefault is default.
	// Currently, this means only encode struct ***REMOVED***eld names as symbols.
	...
	// AsSymbolStructFieldName means encode struct ***REMOVED***eld names as symbols.
...
type EncodeOptions struct {
	...
	//       then the map keys are ***REMOVED***rst sorted in natural order and then written
	//       with corresponding map values to the strema.
	//     - If there is no natural sort order, then the map keys will ***REMOVED***rst be
	...
	// references itself either directly or through one of its ***REMOVED***elds (iteratively).
	//
	...
	// AsSymbols de***REMOVED***nes what should be encoded as symbols.
	//
	// Encoding as symbols can reduce the encoded size signi***REMOVED***cantly.
	//
...
type ioEncWriter struct {
	...
	// x [8]byte // temp byte array re-used internally for ef***REMOVED***ciency
}
...
func (z *bytesEncWriter) growAlloc(n int, oldcursor int) {
	// appendslice logic (if cap < 1024, *2, ***REMOVED*** *1.25): more expensive. many copy calls.
...
func (f *encFnInfo) getValueForMarshalInterface(rv reflect.Value, indir int8) (v interface{}, proceed bool) {
	if indir == 0 {
		...
		// Take addr if addressable, ***REMOVED*** copy value to an addressable value.
		if rv.CanAddr() {
			v = rv.Addr().Interface()
		} ***REMOVED*** {
...
func (f *encFnInfo) kSlice(rv reflect.Value) {
	...
	if f.seq != seqTypeArray {
		...
		// If in this method, then there was no extension function de***REMOVED***ned.
	...
	if ti.rtid == uint8SliceTypId || rtelem.Kind() == reflect.Uint8 {
		switch f.seq {
		case seqTypeArray:
			// if l == 0 { e.e.encodeStringBytes(c_RAW, nil) } ***REMOVED***
			if rv.CanAddr() {
...
func (f *encFnInfo) kStruct(rv reflect.Value) {
	...
	e := f.e
	cr := e.cr
	tis***REMOVED*** := fti.s***REMOVED***p
	toMap := !(fti.toArray || e.h.StructToArray)
	newlen := len(fti.s***REMOVED***)
...
func (e *Encoder) kMapCanonical(rtkeyid uintptr, rtkey reflect.Type, rv reflect.Value, mks []reflect.Value, valFn *encFn, asSymbols bool) {
	...
	// This is not necessary, as the natural kind is suf***REMOVED***cient for ordering.
	...
	if rtkeyid == uint8SliceTypId {
		...
		default:
			// out-of-band
			// ***REMOVED***rst encode each key to a []byte ***REMOVED***rst, then sort them, then record
...
//
// For ef***REMOVED***ciency, Users are encouraged to pass in a memory buffered writer
// (eg bu***REMOVED***o.Writer, bytes.Buffer).
...
// NewEncoderBytes returns an encoder for encoding directly and ef***REMOVED***ciently
...
func (e *Encoder) Reset(w io.Writer) {
	...
	if ok {
		...
		e.wi.w = sww
		//ww = bu***REMOVED***o.NewWriterSize(w, defEncByteBufSize)
	}
	e.w = &e.wi
	e.e.reset()
}
...
//
// Encoding can be con***REMOVED***gured via the struct tag for the ***REMOVED***elds.
// The "codec" key in struct ***REMOVED***eld's tag value is the key name,
// followed by an optional comma and options.
// Note that the "json" key is used in the absence of the "codec" key.
//
// To set an option on all ***REMOVED***elds (e.g. omitempty on all ***REMOVED***elds), you
// can create a ***REMOVED***eld called _struct, and set flags on it.
//
// Struct values "usually" encode as maps. Each exported struct ***REMOVED***eld is encoded unless:
//    - the ***REMOVED***eld's tag is "-", OR
//    - the ***REMOVED***eld is empty (empty or the zero value) and its tag speci***REMOVED***es the "omitempty" option.
//
// When encoding as a map, the ***REMOVED***rst string in the tag (before the comma)
// is the map key string to use when encoding.
//
// However, struct values may encode as arrays. This happens when:
//    - StructToArray Encode option is set, OR
//    - the tag on the _struct ***REMOVED***eld sets the "toarray" option
//
...
//
// Anonymous ***REMOVED***elds are encoded inline except:
//    - the struct tag speci***REMOVED***es a replacement name (***REMOVED***rst value)
//    - the ***REMOVED***eld is of an interface type
//
// Examples:
//
//      // NOTE: 'json:' can be used as struct tag key, in place 'codec:' below.
//      type MyStruct struct {
//          _struct bool    `codec:",omitempty"`   //set omitempty for every ***REMOVED***eld
//          Field1 string   `codec:"-"`            //skip this ***REMOVED***eld
...
//
//      type MyStruct struct {
//          _struct bool    `codec:",omitempty,toarray"`   //set omitempty for every ***REMOVED***eld
...
//
// Note that struct ***REMOVED***eld names and keys in map[string]XXX will be treated as symbols.
...
func (e *Encoder) getEncFn(rtid uintptr, rt reflect.Type, checkFastpath, checkCodecSelfer bool) (fn *encFn) {
	...
	***REMOVED***.ti = ti
	...
	} ***REMOVED*** if e.e.IsBuiltinType(rtid) {
		fn.f = (*encFnInfo).builtin
	} ***REMOVED*** if xfFn := e.h.getExt(rtid); xfFn != nil {
		***REMOVED***.xfTag, ***REMOVED***.xfFn = xfFn.tag, xfFn.ext
		fn.f = (*encFnInfo).ext
	} ***REMOVED*** if supportMarshalInterfaces && e.be && ti.bm {
		fn.f = (*encFnInfo).binaryMarshal
	} ***REMOVED*** if supportMarshalInterfaces && !e.be && e.js && ti.jm {
		//If JSON, we should check JSONMarshal before textMarshal
		fn.f = (*encFnInfo).jsonMarshal
	} ***REMOVED*** if supportMarshalInterfaces && !e.be && ti.tm {
...
// The cost is that of locking sometimes, but sync.Pool is ef***REMOVED***cient
...
func encStructPoolGet(newlen int) (p *sync.Pool, v interface{}, s []stringRv) {
	...
	} ***REMOVED*** if newlen <= 16 {
		p = &encStructPool[1]
		v = p.Get()
		s = v.(*[16]stringRv)[:newlen]
	} ***REMOVED*** if newlen <= 32 {
		p = &encStructPool[2]
		v = p.Get()
		s = v.(*[32]stringRv)[:newlen]
	} ***REMOVED*** if newlen <= 64 {
		p = &encStructPool[3]
		v = p.Get()
		s = v.(*[64]stringRv)[:newlen]
	} ***REMOVED*** if newlen <= 128 {
		p = &encStructPool[4]
		v = p.Get()
