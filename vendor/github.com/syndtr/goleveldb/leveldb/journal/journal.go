...
// Next ***REMOVED***nishes the current journal. Call Close to ***REMOVED***nish the ***REMOVED***nal journal.
//
// Optionally, call Flush to ***REMOVED***nish the current journal and flush the underlying
...
// ***REMOVED***rst, middle or last chunk of a multi-chunk journal. A multi-chunk journal
// has one ***REMOVED***rst chunk, zero or more middle chunks, and one last chunk.
//
// The wire format allows for limited recovery in the face of data corruption:
// on a format error (such as a checksum mismatch), the reader moves to the
// next block and looks for the next full or ***REMOVED***rst chunk.
package journal
...
type Reader struct {
	...
	// only the ***REMOVED***nal block can have n < blockSize.
	n int
...
func (r *Reader) nextChunk(***REMOVED***rst bool) error {
	for {
		if r.j+headerSize <= r.n {
			...
			} ***REMOVED*** if r.checksum && checksum != util.NewCRC(r.buf[r.i-1:r.j]).Value() {
				// Drop entire block.
				r.i = r.n
				r.j = r.n
				return r.corrupt(unprocBlock, "checksum mismatch", false)
			}
			if ***REMOVED***rst && chunkType != fullChunkType && chunkType != ***REMOVED***rstChunkType {
...
type Writer struct {
	...
	written int
	// ***REMOVED***rst is whether the current chunk is the ***REMOVED***rst chunk of the journal.
	***REMOVED***rst bool
...
// ***REMOVED***llHeader ***REMOVED***lls in the header for the pending chunk.
func (w *Writer) ***REMOVED***llHeader(last bool) {
	...
	if last {
		...
		if w.***REMOVED***rst {
			w.buf[w.i+6] = ***REMOVED***rstChunkType
		} ***REMOVED*** {
...
// writePending ***REMOVED***nishes the current journal and writes the buffer to the
// underlying writer.
...
// Close ***REMOVED***nishes the current journal and closes the writer.
...
// Flush ***REMOVED***nishes the current journal, writes to the underlying writer, and
