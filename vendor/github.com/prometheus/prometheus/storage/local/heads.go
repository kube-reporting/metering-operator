...
// heads.db ***REMOVED***le. It follows a similar semantics as the bu***REMOVED***o.Scanner.
// It is not safe to use a headsScanner concurrently.
type headsScanner struct {
	f                    *os.File
	r                    *bu***REMOVED***o.Reader
...
func newHeadsScanner(***REMOVED***lename string) *headsScanner {
	hs := &headsScanner{}
	defer func() {
	...
	if hs.f, hs.err = os.Open(***REMOVED***lename); hs.err != nil {
		return hs
	}
	hs.r = bu***REMOVED***o.NewReaderSize(hs.f, ***REMOVED***leBufSize)
...
// scan works like bu***REMOVED***o.Scanner.Scan.
func (hs *headsScanner) scan() bool {
	...
	for i := int64(0); i < numChunkDescs; i++ {
		if i < persistWatermark {
			if ***REMOVED***rstTime, hs.err = binary.ReadVarint(hs.r); hs.err != nil {
				return false
			}
			...
			chunkDescs[i] = &chunk.Desc{
				ChunkFirstTime: model.Time(***REMOVED***rstTime),
...
// close closes the underlying ***REMOVED***le if required.
...
// DumpHeads writes the metadata of the provided heads ***REMOVED***le in a human-readable
// form.
func DumpHeads(***REMOVED***lename string, out io.Writer) error {
	hs := newHeadsScanner(***REMOVED***lename)
	defer hs.close()

	if hs.err == nil {
		fmt.Fprintf(
			out,
			">>> Dumping %d series from heads ***REMOVED***le %q with format version %d. <<<\n",
			hs.seriesTotal, ***REMOVED***lename, hs.version,
		)
	}
	for hs.scan() {
		s := hs.series
